...
IMPLEMENT_DYNAMIC(CHexEditView, CView)
...
void CHexEditView::DoDraw(CDC &dc)
{
	...
	CString tstrAddr;
	const BYTE *pbPos = m_data.GetData();
	const BYTE *pbEnd = pbPos + m_data.GetSize();
	...
	for (int iLb = 0; iLb < nLb; iLb++) {
		...
		tcHex[0] = 0;
		int iWalk1 = 0;
		for (; iWalk1 < nWalk1; iWalk1++, pbWalk1++) {
		...
		TCHAR tcAsc[17];
		int iWalk2 = 0;
		for (; iWalk2 < nWalk2; iWalk2++, pbWalk2++) {
			if (isprint(*pbWalk2)) {
		...
		for (iMask = 0; iMask < nWalk1; iMask++) {
			if (bSelMask[iMask]) {
				int iMask2 = iMask;
				for (; iMask2 < nWalk1 && bSelMask[iMask2]; iMask2++);
				CRect rcInv;
...
void CHexEditView::SetRawData(LPCVOID pvAny, UINT cbAny)
{
	m_data.SetSize(cbAny);
	memcpy(m_data.GetData(), pvAny, cbAny);

	BinaryResize();
	RefreshCaretPos();
}

void CHexEditView::GetRawData(SizeBuff &fv)
{
	fv.Alloc(m_data.GetSize(), m_data.GetData());
}

void CHexEditView::SetSink(CHexEditSink *pUniSink)
{
	m_sink = pUniSink;
}
	...
	if (m_bChrPane) {
		...
		if (GetDataSize() <= m_iCursor) {
			...
			if (m_bOverwrite && !bKilled) {
				m_data.SetAt(m_iCursor, c);
				m_iCursor++;
			} else {
				m_data.InsertAt(m_iCursor, c);
				m_iCursor++;
			}
		}
		if (w) {
...
void CHexEditView::OnEditCopy() 
{
	...
	for (; iMin != iMax; iMin++, i++) {
		tstrTemp.Format((i == 0) ? "%02X" : " %02X", (UINT)m_data[iMin]);
		tstr += tstrTemp;
	}
	CSharedFile f;
...
void CHexEditView::OnEditPaste() 
{
	...
	if (OpenClipboard()) {
		...
		if (!tstr.IsEmpty()) {
			...
			KillSelected();
			int iPaste = 0, nPastes = arr.GetSize();
			for (; iPaste < nPastes; iPaste++) {
				m_data.InsertAt(m_iCursor, arr[iPaste]);
				m_iCursor++;
			}
			if (nPastes) {
				m_iSelFrom = m_iCursor;
...
BOOL CHexEditView::SaveModified()
{
	if (!IsModified())
		return TRUE;        // ok to continue

	CString prompt;
	AfxFormatString1(prompt, AFX_IDP_ASK_TO_SAVE, "Unnamed");
	switch (MessageBox(prompt, NULL, MB_ICONEXCLAMATION|MB_YESNOCANCEL))
	{
	case IDCANCEL:
