	...
	
	{
		...
		for (j = 0; j < nRows; j++)
		{
			...
			for (i = 0; i < nCols; i++)
			{
				...
				{
					ObjectID rlDoor = OBJECT_NULL;
					ObjectID_Reference* refDoor;
					//Door* xDoor = NULL;
					Door_Info* xDoor = NULL;
					DUNGEON_SET_TILE(ttFloor, 0, iMapIndex, i, j);
					
					DUNGEON_SET_TILE(ttDoor, 0, iMapIndex, i, j);
					rlDoor = DUNGEON_PUT_NEW_DOOR(0, iMapIndex, i, j);
					refDoor = (ObjectID_Reference*) &rlDoor;
					//printf("Door (%d,%d,%d)\n", refDoor->iDBType, refDoor->iID, refDoor->iDir);
					DUNGEON_SET_TILE_FLAG(iDoorDirFlag, iMapIndex, i, j);
					xDoor = (Door_Info*) GET_ADDRESS_OF_RECORD0(rlDoor);
					//printf("SET DOOR %p\n", xDoor);
					/*
					if (MVALID(xDoor))
					{
