...
#include <XDEFINES.H>	// for default frequency
#include <SkParam.h>
// SPX
...
int iStandardSoundPlaybackFrequency = 6000;
...
namespace sblast_sys {
	...
	void DS::Play(PlayBuff &pb) {
		...
		for (int x=0; x<DSB_MAX; x++) {
			...
			// SPX
			// PC9821/V4 => Playback Frequency = 6000
			// PCDOS/V5 => Playback Frequency = 11020
			int iBasePlaybackFrequency = iStandardSoundPlaybackFrequency;
	...
	void DS::SetPlaybackFrequency(int f) {
		iStandardSoundPlaybackFrequency = f;
	}
