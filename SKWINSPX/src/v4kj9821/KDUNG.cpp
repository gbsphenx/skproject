...
void SkWinCore::SET_SQUARE_ASPECT_RANDOM_WALL_ORNAMENT_ORDINALS(ExtendedTileInfo *ref, Bit16u xx, Bit16u yy, Bit16u zz, Bit16u ww, Bit16u vv, i16 ss, i16 tt)
{
	...
	//^0CEE:1872
	ref->w6[2] = (GET_RANDOM_ORNAMENT_ORDINAL(xx, si, iPosX, (++iPosY) * ((vv & 3) +1), 30));
	//^0CEE:1899
	ref->w6[3] = (GET_RANDOM_ORNAMENT_ORDINAL(yy, si, iPosX, iPosY * (((++vv) & 3) +1), 30));
	//^0CEE:18C2
	ref->w6[0] = (GET_RANDOM_ORNAMENT_ORDINAL(zz, si, iPosX, iPosY * (((++vv) & 3) +1), 30));
	//^0CEE:18EB
	ref->w6[1] = (GET_RANDOM_ORNAMENT_ORDINAL(ww, si, iPosX, (iPosY--) * (((++vv) & 3) +1), 30));
	// 1 = left wall
	// 2 = front wall
	// 3 = right wall
...
void SkWinCore::__LOAD_CREATURE_FROM_DUNGEON()
{
	...
	//^1C9A:3C35
	glbCreaturesCount = 0;
	//printf("__LOAD_CREATURE_FROM_DUNGEON:CAST\n"); getch();
	...
	//printf("__LOAD_CREATURE_FROM_DUNGEON:Nb Creatures = %d\n", si); getch();
	for (; si-- != 0; bp04++) {
		//^1C9A:3C52
		//printf("__LOAD_CREATURE_FROM_DUNGEON:QUERY_CREATURE_AI_SPEC_FROM_TYPE %d\n", si); getch();
	...
	//^1C9A:3C80
	//printf("__LOAD_CREATURE_FROM_DUNGEON:Min / CAST\n"); getch();
	...
	//^1C9A:3CBF
	//printf("__LOAD_CREATURE_FROM_DUNGEON:Return\n"); getch();
	return;
}
