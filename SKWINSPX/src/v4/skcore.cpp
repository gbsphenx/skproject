...
// 0x00
	if (event == UI_EVENTCODE_x01_TURN_LEFT) return "TURN LEFT";
	if (event == UI_EVENTCODE_x02_TURN_RIGHT) return "TURN RIGHT";
	if (event == UI_EVENTCODE_x03_MOVE_FORWARD) return "MOVE FORWARD";
	if (event == UI_EVENTCODE_x04_MOVE_RIGHT) return "MOVE RIGHT";
	if (event == UI_EVENTCODE_x05_MOVE_BACK) return "MOVE BACK";
	if (event == UI_EVENTCODE_x06_MOVE_LEFT) return "MOVE LEFT";

	if (event == UI_EVENTCODE_x07_VIEW_CHAMPION_1) return "CHAMPION INVENTORY 1";
	if (event == UI_EVENTCODE_x08_VIEW_CHAMPION_2) return "CHAMPION INVENTORY 2";
	if (event == UI_EVENTCODE_x09_VIEW_CHAMPION_3) return "CHAMPION INVENTORY 3";
	if (event == UI_EVENTCODE_x0A_VIEW_CHAMPION_4) return "CHAMPION INVENTORY 4";
	if (event == UI_EVENTCODE_x0B_RETURN_VIEWPORT) return "BACK TO VIEWPORT";

// 0x10
	if (event == UI_EVENTCODE_x14_CHAMPION_1_HAND_RIGHT) return "CHAMPION 1 RIGHT HAND";
	if (event == UI_EVENTCODE_x15_CHAMPION_1_HAND_LEFT) return "CHAMPION 1 LEFT HAND";
	if (event == UI_EVENTCODE_x16_CHAMPION_2_HAND_RIGHT) return "CHAMPION 2 RIGHT HAND";
	if (event == UI_EVENTCODE_x17_CHAMPION_2_HAND_LEFT) return "CHAMPION 2 LEFT HAND";
	if (event == UI_EVENTCODE_x18_CHAMPION_3_HAND_RIGHT) return "CHAMPION 3 RIGHT HAND";
	if (event == UI_EVENTCODE_x19_CHAMPION_3_HAND_LEFT) return "CHAMPION 3 LEFT HAND";
	if (event == UI_EVENTCODE_x1A_CHAMPION_4_HAND_RIGHT) return "CHAMPION 4 RIGHT HAND";
	if (event == UI_EVENTCODE_x1B_CHAMPION_4_HAND_LEFT) return "CHAMPION 4 LEFT HAND";

	if (event == UI_EVENTCODE_x1C_INVENTORY_HAND_RIGHT) return "INVENTORY RIGHT HAND";
	if (event == UI_EVENTCODE_x1D_INVENTORY_HAND_LEFT) return "INVENTORY LEFT HAND";

	if (event == UI_EVENTCODE_x1E_INVENTORY_HEAD) return "INVENTORY HEAD";
	if (event == UI_EVENTCODE_x1F_INVENTORY_BODY) return "INVENTORY BODY";

// 0x20 - 0x30
	if (event == UI_EVENTCODE_x20_INVENTORY_LEGS) return "INVENTORY LEGS";
	if (event == UI_EVENTCODE_x21_INVENTORY_FOOT) return "INVENTORY FOOT";
	if (event == UI_EVENTCODE_x22_INVENTORY_POUCH_2) return "INVENTORY POUCH 2";
	if (event == UI_EVENTCODE_x23_INVENTORY_SCABBARD_2) return "INVENTORY SCABBARD 2";
	if (event == UI_EVENTCODE_x24_INVENTORY_SCABBARD_3) return "INVENTORY SCABBARD 3";
	if (event == UI_EVENTCODE_x25_INVENTORY_SCABBARD_4) return "INVENTORY SCABBARD 4";

	if (event == UI_EVENTCODE_x26_INVENTORY_NECK) return "INVENTORY NECK";
	if (event == UI_EVENTCODE_x27_INVENTORY_POUCH_1) return "INVENTORY POUCH 1";
	if (event == UI_EVENTCODE_x28_INVENTORY_SCABBARD_1) return "INVENTORY SCABBARD 1";

	if (event == UI_EVENTCODE_x29_BACKPACK_01) return "INVENTORY BACKPACK 1";
	if (event == UI_EVENTCODE_x2A_BACKPACK_02) return "INVENTORY BACKPACK 2";
	if (event == UI_EVENTCODE_x2B_BACKPACK_03) return "INVENTORY BACKPACK 3";
	if (event == UI_EVENTCODE_x2C_BACKPACK_04) return "INVENTORY BACKPACK 4";
	if (event == UI_EVENTCODE_x2D_BACKPACK_05) return "INVENTORY BACKPACK 5";
	if (event == UI_EVENTCODE_x2E_BACKPACK_06) return "INVENTORY BACKPACK 6";
	if (event == UI_EVENTCODE_x2F_BACKPACK_07) return "INVENTORY BACKPACK 7";
	if (event == UI_EVENTCODE_x30_BACKPACK_08) return "INVENTORY BACKPACK 8";
	if (event == UI_EVENTCODE_x31_BACKPACK_09) return "INVENTORY BACKPACK 9";
	if (event == UI_EVENTCODE_x32_BACKPACK_10) return "INVENTORY BACKPACK 10";
	if (event == UI_EVENTCODE_x33_BACKPACK_11) return "INVENTORY BACKPACK 11";
	if (event == UI_EVENTCODE_x34_BACKPACK_12) return "INVENTORY BACKPACK 12";
	if (event == UI_EVENTCODE_x35_BACKPACK_13) return "INVENTORY BACKPACK 13";
	if (event == UI_EVENTCODE_x36_BACKPACK_14) return "INVENTORY BACKPACK 14";
	if (event == UI_EVENTCODE_x37_BACKPACK_15) return "INVENTORY BACKPACK 15";
	if (event == UI_EVENTCODE_x38_BACKPACK_16) return "INVENTORY BACKPACK 16";
	if (event == UI_EVENTCODE_x39_BACKPACK_17) return "INVENTORY BACKPACK 17";

	if (event == UI_EVENTCODE_x3A_CONTAINER_1) return "CONTAINER SLOT 1";
	if (event == UI_EVENTCODE_x3B_CONTAINER_2) return "CONTAINER SLOT 2";
	if (event == UI_EVENTCODE_x3C_CONTAINER_3) return "CONTAINER SLOT 3";
	if (event == UI_EVENTCODE_x3D_CONTAINER_4) return "CONTAINER SLOT 4";
	if (event == UI_EVENTCODE_x3E_CONTAINER_5) return "CONTAINER SLOT 5";
	if (event == UI_EVENTCODE_x3F_CONTAINER_6) return "CONTAINER SLOT 6";
	if (event == UI_EVENTCODE_x40_CONTAINER_7) return "CONTAINER SLOT 7";
	if (event == UI_EVENTCODE_x41_CONTAINER_8) return "CONTAINER SLOT 8";



// 0x40

	if (event == UI_EVENTCODE_x46_MOUTH) return "INVENTORY MOUTH";
	if (event == UI_EVENTCODE_x47_EYE) return "INVENTORY EYE";


// 0x50
	if (event == UI_EVENTCODE_x50_CLICK_VIEWPORT) return "CLICK ON VIEWPORT";
	if (event == UI_EVENTCODE_x52_VIEW_LEADER) return "LEADER INVENTORY";

	if (event == UI_EVENTCODE_x5D_CHAMPION_TURN_LEFT) return "CHAMPION TURN LEFT";
	if (event == UI_EVENTCODE_x5E_CHAMPION_TURN_RIGHT) return "CHAMPION TURN RIGHT";

// 0x60

	if (event == UI_EVENTCODE_x5F_SPELL_OR_LEADER_1) return "CHOOSE LEADER / INVOKE SPELL 1";
	if (event == UI_EVENTCODE_x60_SPELL_OR_LEADER_2) return "CHOOSE LEADER / INVOKE SPELL 2";
	if (event == UI_EVENTCODE_x61_SPELL_OR_LEADER_3) return "CHOOSE LEADER / INVOKE SPELL 3";
	if (event == UI_EVENTCODE_x62_SPELL_OR_LEADER_4) return "CHOOSE LEADER / INVOKE SPELL 4";

	
	
	if (event == UI_EVENTCODE_x65_ADD_RUNE_1) return "ADD RUNE 1";
	if (event == UI_EVENTCODE_x66_ADD_RUNE_2) return "ADD RUNE 2";
	if (event == UI_EVENTCODE_x67_ADD_RUNE_3) return "ADD RUNE 3";
	if (event == UI_EVENTCODE_x68_ADD_RUNE_4) return "ADD RUNE 4";
	if (event == UI_EVENTCODE_x69_ADD_RUNE_5) return "ADD RUNE 5";
	if (event == UI_EVENTCODE_x6A_ADD_RUNE_6) return "ADD RUNE 6";
	if (event == UI_EVENTCODE_x6B_REMOVE_RUNE) return "REMOVE RUNE";
	if (event == UI_EVENTCODE_x6C_VALIDATE_SPELL) return "VALIDATE SPELL";

// 0x70
	if (event == UI_EVENTCODE_x70_HAND_RUNE_QUIT) return "HAND / RUNE QUIT ACTION";
	if (event == UI_EVENTCODE_x71_HAND_ACTION_1) return "HAND ACTION 1";
	if (event == UI_EVENTCODE_x72_HAND_ACTION_2) return "HAND ACTION 2";
	if (event == UI_EVENTCODE_x73_HAND_ACTION_3) return "HAND ACTION 3";
	if (event == UI_EVENTCODE_x74_CHAMPION1_ACTION_HAND_RIGHT) return "ACTIVATE CHAMPION 1 RIGHT HAND";
	if (event == UI_EVENTCODE_x75_CHAMPION1_ACTION_HAND_LEFT) return "ACTIVATE CHAMPION 1 LEFT HAND";
	if (event == UI_EVENTCODE_x76_CHAMPION2_ACTION_HAND_RIGHT) return "ACTIVATE CHAMPION 2 RIGHT HAND";
	if (event == UI_EVENTCODE_x77_CHAMPION2_ACTION_HAND_LEFT) return "ACTIVATE CHAMPION 2 LEFT HAND";
	if (event == UI_EVENTCODE_x78_CHAMPION3_ACTION_HAND_RIGHT) return "ACTIVATE CHAMPION 3 RIGHT HAND";
	if (event == UI_EVENTCODE_x79_CHAMPION3_ACTION_HAND_LEFT) return "ACTIVATE CHAMPION 3 LEFT HAND";
	if (event == UI_EVENTCODE_x7A_CHAMPION4_ACTION_HAND_RIGHT) return "ACTIVATE CHAMPION 4 RIGHT HAND";
	if (event == UI_EVENTCODE_x7B_CHAMPION4_ACTION_HAND_LEFT) return "ACTIVATE CHAMPION 4 LEFT HAND";

	if (event == UI_EVENTCODE_x7D_TAKE_CHAMPION_TOP_LEFT) return "SELECTION TOP LEFT CHAMPION";
	if (event == UI_EVENTCODE_x7E_TAKE_CHAMPION_TOP_RIGHT) return "SELECTION TOP RIGHT CHAMPION";
	if (event == UI_EVENTCODE_x7F_TAKE_CHAMPION_BOTTOM_RIGHT) return "SELECTION BOTTOM RIGHT CHAMPION";
	if (event == UI_EVENTCODE_x80_TAKE_CHAMPION_BOTTOM_LEFT) return "SELECTION BOTTOM LEFT CHAMPION";
	if (event == UI_EVENTCODE_x81_UPDATE_CHAMPION_POSITION) return "UPDATE CHAMPION POSITION";

// 0x80
	if (event == UI_EVENTCODE_x8C_DISK_OP) return "DISK OPERATION";

// 0x90
	if (event == UI_EVENTCODE_x91_SLEEP) return "INVOKE SLEEP";
	if (event == UI_EVENTCODE_x92_WAKE) return "WAKE UP FROM SLEEP";
	if (event == UI_EVENTCODE_x93_PAUSE) return "INVOKE PAUSE";
	if (event == UI_EVENTCODE_x94_END_PAUSE) return "RESUME GAME FROM PAUSE";

	if (event == UI_EVENTCODE_x96_MAGIC_MAP) return "MAGIC MAP";
	if (event == UI_EVENTCODE_x97_MAGIC_MAP_RUNE_1) return "MAGIC MAP RUNE 1";
	if (event == UI_EVENTCODE_x98_MAGIC_MAP_RUNE_2) return "MAGIC MAP RUNE 2";
	if (event == UI_EVENTCODE_x99_MAGIC_MAP_RUNE_3) return "MAGIC MAP RUNE 3";
	if (event == UI_EVENTCODE_x9A_MAGIC_MAP_RUNE_4) return "MAGIC MAP RUNE 4";

// 0xA0
	if (event == UI_EVENTCODE_xA0_REVIVE_CHAMPION) return "REVIVE CHAMPION";
	if (event == UI_EVENTCODE_xA1_EXIT_CRYOCELL) return "CANCEL REVIVE CHAMPION";

// 0xD0
	if (event == UI_EVENTCODE_xD7_START_NEW_GAME) return "START NEW GAME";
	if (event == UI_EVENTCODE_xD8_LOAD_GAME) return "0D8? LOAD GAME";
	if (event == UI_EVENTCODE_xD9_RESUME_GAME) return "RESUME GAME";
	if (event == UI_EVENTCODE_xDA_SHOW_CREDITS) return "SHOW CREDITS";

	if (event == UI_EVENTCODE_xDB_DIALOG_BUTTON_1) return "BUTTON 1";
	if (event == UI_EVENTCODE_xDC_DIALOG_BUTTON_2) return "BUTTON 2";
	if (event == UI_EVENTCODE_xDD_DIALOG_BUTTON_3) return "BUTTON 3";
	if (event == UI_EVENTCODE_xDE_DIALOG_BUTTON_4) return "BUTTON 4";

// 0xE0		
	if (event == UI_EVENTCODE_xE0_QUIT_GAME) return "QUIT GAME";
	if (event == UI_EVENTCODE_xE1_MOVE_OBJECT) return "MOVE OBJECT";
	if (event == UI_EVENTCODE_xE3_RELEASE_MOUSE_BUTTON) return "RELEASE MOUSE BUTTON";

	if (event == UI_EVENTCODE_xE4_MONEYBOX_1) return "MONEY BOX SLOT 1 (BLUE GEMS)";
	if (event == UI_EVENTCODE_xE5_MONEYBOX_2) return "MONEY BOX SLOT 2 (RED GEMS)";
	if (event == UI_EVENTCODE_xE6_MONEYBOX_3) return "MONEY BOX SLOT 3 (GREEN GEMS)";
	if (event == UI_EVENTCODE_xE7_MONEYBOX_4) return "MONEY BOX SLOT 4 (GOLD COINS)";
	if (event == UI_EVENTCODE_xE8_MONEYBOX_5) return "MONEY BOX SLOT 5 (SILVER COINS)";
	if (event == UI_EVENTCODE_xE9_MONEYBOX_6) return "MONEY BOX SLOT 6 (BRONZE COINS)";
	
	if (event == UI_EVENTCODE_xEA_CLICK_STATS_BAR_1) return "CLICK CHAMPION 1 BAR STATS";
	if (event == UI_EVENTCODE_xEB_CLICK_STATS_BAR_2) return "CLICK CHAMPION 2 BAR STATS";
	if (event == UI_EVENTCODE_xEC_CLICK_STATS_BAR_3) return "CLICK CHAMPION 3 BAR STATS";
	if (event == UI_EVENTCODE_xED_CLICK_STATS_BAR_4) return "CLICK CHAMPION 4 BAR STATS";


	if (event == UI_EVENTCODE_xEF_QUIT_CREDITS) return "QUIT CREDITS";
...
//^0759:0310
// SPX: _0759_0310 renamed _0759_0310
void SkWinCore::IBMIO_BLIT_0759_0310(U16 offset, U16 size)
{
	ENTER(0);
	IBMIO_BLIT_SOURCE_4TO8BPP(_089c_0354, offset, offset, size) CALL_IBMIO;
	return;
}
//^0759:02C6
// _0759_02c6 renamed ANIM_SOMETHING_0759_02c6
void SkWinCore::ANIM_SOMETHING_0759_02c6(U16 xx, U16 yy, U16 zz)
{
	//^0759:02C6
	ENTER(0);
	//^0759:02CB
...
void SkWinCore::GRAPHICS_DATA_OPEN()
{
	...
	if ((glbGDatOpenCloseFlag++) == 0) {
		...
		if (glbFloppyDiskFlag != 0 && glbGDatFloppyFlag == 0) {
...
//^0B36:0D67
// SPX: _0b36_0d67 rename SOME_RECT_0b36_0d67
void SkWinCore::SOME_RECT_0b36_0d67(sk3f6c *ref, SRECT *rc)
{
	//^0B36:0D67
	//^0B36:0D6C
	if (rc == NULL)
...
//^0B36:0C52
// SPX: _0b36_0c52 renamed SOME_RECT_PICT_0b36_0c52
sk3f6c *SkWinCore::SOME_RECT_PICT_0b36_0c52(sk3f6c *ref, U16 iRectNo, U16 ww)
{
	if (iRectNo != 0xFFFF) {
...
U16 SkWinCore::IS_MISSILE_VALID_TO_LAUNCHER(U16 player, i16 hand, ObjectID rlWhatYouLaunch)
{
	...
	return (true
		&& hand != -1
		&& (bp02 = tblChampionSquad[player].Possess(hand)) != OBJECT_NULL // any objects in hand?
...
//^1031:0675
// SPX: _1031_0675 renamed SQUAD_SOMETHING_1031_0675
void SkWinCore::SQUAD_SOMETHING_1031_0675(U16 xx)
{
	ENTER(0);
...
U16 SkWinCore::PT_IN_RECT(SRECT *rc, i16 xx, i16 yy)
{
	...
	if (SkCodeParam::bUsePowerDebug && !CheckSafePointer(rc))
		return 0;
...
//^1031:01D5
// SPX: _1031_01d5 renamed GET_SPECIAL_RECT_1031_01d5
SRECT *SkWinCore::GET_SPECIAL_RECT_1031_01d5(U16 iRectNo, SRECT *pRectZone)
{
	ENTER(4);
	U16 iLocRectNo = iRectNo;	// si
	pRectZone = QUERY_EXPANDED_RECT(iLocRectNo & 0x3FFF, pRectZone);
	if (pRectZone != NULL) {
		if ((iLocRectNo & 0x8000) != 0) {
			iLocRectNo = 7;
			goto _020f;
		}
		if ((iLocRectNo & 0x4000) != 0) {
			iLocRectNo = 18;
_020f:
			i16 bp02;
			i16 bp04;
			QUERY_TOPLEFT_OF_RECT(iLocRectNo, &bp02, &bp04);
			pRectZone->x += bp02;
			pRectZone->y += bp04;
		}
	}
...
//^1031:0A88
// SPX: _1031_0a88 renamed RECT_WITHIN_THEN_EVENT_1031_0a88
U16 SkWinCore::RECT_WITHIN_THEN_EVENT_1031_0a88(sk0d9e *ref, U16 xx, U16 yy, U16 ww)
{
	ENTER(4);
	if (ref == NULL)
		return 0;
	...
	do {
		if (true
			&& (ref->w4 & 0x0800) == 0 
			&& (ww & (255 & ref->w4)) != 0 
			&& GET_SPECIAL_RECT_1031_01d5(ref->w2, &_4976_4e9e) != NULL
...
U16 SkWinCore::_1031_030a(sk1891 *ref, U16 xx, U16 yy, U16 zz)
{
	...
	do {
		...
		if ((this->*_4976_0cba_PFN_12[ref->b0_0_6()])(bp04) != 0) {
			...
			else if (PT_IN_RECT(GET_SPECIAL_RECT_1031_01d5(_4976_16ed[bp04->w2].w0, &bp10), xx, di) != 0) {
				if ((si = RECT_WITHIN_THEN_EVENT_1031_0a88(_1031_024c(bp04), xx, di, zz)) != 0)
					return si;
			}
		}
		bp08++;
	...
	if (ref != NULL) {
		...
		for (; (si = 0x07ff & ref->w0) != 0; ref++) {
			if ((ref->w4 & 0x0800) == 0 && si == xx) {
				GET_SPECIAL_RECT_1031_01d5(ref->w2, &_4976_4e9e);
				_4976_4df8 = ref->w2;
...
//^1031:10C8
// SPX: _1031_10c8 renamed SOME_COPY_RECT_1031_10c8
void SkWinCore::SOME_COPY_RECT_1031_10c8(sk3f6c *ref, SRECT* xRect, U16 cx, U16 cy)
{
	...
	CALC_CENTERED_RECT_IN_RECT(xRect, &ref->rc2, cx, cy);
	return;
}
...
void SkWinCore::_2405_00ec_RECT(U16 iRectNo, SRECT* xRect)
{
	ENTER(4);
...
void SkWinCore::_2405_011f_RECT(U16 iRectNo, SRECT* xRect)
{
	ENTER(0);
	_2405_00ec_RECT(iRectNo, xRect);
	INFLATE_RECT(xRect, _4976_0112, _4976_0112);
	return;
}
...
//^2759:0155
// SPX: _2759_0155 renamed CHECK_ITEM_CAN_USE_CHARGES
U16 SkWinCore::CHECK_ITEM_CAN_USE_CHARGES(ObjectID rl)
{
	ENTER(4);
	ObjectID iObjectID = rl;	// si
	if (iObjectID == OBJECT_NULL) {
		return 0;
	}
	U8 iCls1Category = QUERY_CLS1_FROM_RECORD(iObjectID);	// bp01
	U8 iCls2MainID = QUERY_CLS2_FROM_RECORD(iObjectID);	// bp02
	U8 iCls4EntryID = 0x08;	// bp03
	for (; iCls4EntryID < 12; iCls4EntryID++) {	// check item commands from 8 through 11 (interesting though items should only have 3 possible commands, is there room for a hidden command?)
		if (true
			&& QUERY_GDAT_ENTRY_IF_LOADABLE(iCls1Category, iCls2MainID, dtText, iCls4EntryID) != 0
			&& QUERY_CMDSTR_ENTRY(iCls1Category, iCls2MainID, iCls4EntryID, CnCM_Command) != 0
			&& QUERY_CMDSTR_ENTRY(
				QUERY_CLS1_FROM_RECORD(iObjectID),
				QUERY_CLS2_FROM_RECORD(iObjectID),
				iCls4EntryID,
				CnNC_NumberCharge
				) != 0
		) {
...
void SkWinCore::INIT_BACKBUFF()
{
	ENTER(0);
	if (_4976_5be6 != 0) {
		FIRE_FILL_BACKBUFF_RECT(&_4976_5938, glbPaletteT16[C00_COLOR_BLACK]);
	}
//	WRITE_UI16(_4976_4c16,-4,_4976_00f6); // width
//	WRITE_UI16(_4976_4c16,-2,_4976_00f8); // height
//	WRITE_UI16(_4976_4c16,-6,8); // bpp

	WRITE_IMGBUFF_HEIGHT(glbBackBuffViewport,glbViewportHeight);
	WRITE_IMGBUFF_WIDTH(glbBackBuffViewport,glbViewportWidth);
...
//^2C1D:0E23
// SPX: _2c1d_0e23 renamed SOMETHING_ITEM_WEIGHT_2c1d_0e23
U16 SkWinCore::SOMETHING_ITEM_WEIGHT_2c1d_0e23(U16 xx)
{
	ENTER(0);
	i16 di;
...
_0b01:
	...
	if (_12b4_099e(bp08) == 0) {
		...
		for (bp02 = 0; bp02 < cd.pi.glbChampionsCount; bp02++) {
			//^12B4:0B59
			if (tblChampionSquad[bp02].curHP() != 0) {
				//^12B4:0B6D
...
void SkWinCore::QUERY_GDAT_IMAGE_METRICS(U8 cls1, U8 cls2, U8 cls4, i16 *pcx, i16 *pcy)
{
	...
	if (SkCodeParam::bUsePowerDebug && iRawDatIndex == 0xFFFF) { // not expected
		printf("WARNING! can't find RawDatId for %02X-%02X-%02X !\n", cls1, cls2, cls4);
		*pcx = 0;
		*pcy = 0;
		return;
	}
...
i16 SkWinCore::DIALOG_SELECT_SAVEGAMES()
{
	...
	if (_4976_5bf6 != 0) {
		...
		COPY_MEMORY(&tblSaveGamesInfo[iSavegameIdx], &glbCurrentSaveGameInfo, sizeof(sksave_header_asc));	// size = 42
		// but glbCurrentSaveGameInfo is not used anywhere ?!
	}
	else {
	...
	do {
		...
		if (_4976_4c0a != 0 && glbUIEventCode >= UI_EVENTCODE_xA5 && glbUIEventCode <= UI_EVENTCODE_xD6) {
			if (_4976_4e9c == 14) {
				if (di > 0) {
					di--;
					bp04[di] = 0;
...
//^0CD5:0039
void SkWinCore::FILL_U16(i16 *pDataBuffer, X16 iSizeCount, i16 iFillValue, i16 iDeltaStep)
{
	ENTER(0);
	X16 iBuffCursor = 0;	// si
	iDeltaStep >>= 1;	// because working on X16 buffer
	for (U16 iIndex = 0; iIndex < iSizeCount; iIndex++) {	// di
		pDataBuffer[iBuffCursor] = iFillValue;
		iBuffCursor += iDeltaStep;
	}
	return;
}
	...
	if (bp02 != ttWall) {
		...
		if ((bp04 & 0x10) != 0) {
			...
			for (ObjectID si = GET_WALL_TILE_ANYITEM_RECORD(xx, yy); si != OBJECT_END_MARKER; si = GET_NEXT_RECORD_LINK(si)) {
				...
				//^19F0:0177
				if (bp06 == dbCreature) {
					ObjectID bp08 = GET_CREATURE_1c9a_03cf(&xx, &yy, 255);
					if (bp08 != OBJECT_NULL) {
						//^19F0:019A
							...
							if (_19f0_0559(ww) == 0) {
								...
								if (bp04 <= 1 && (si & 7) != 0 && ((si & 0xff8) == 0 || RAND01() != 0)) {
									...
									if (zz <= 1) {
										...
										if (bp06 != 0xffff) {
											//^19F0:0B31
											bp06 = tblChampionSquad[bp06].playerPos();
											goto _0bae;
										}
									}
									//^19F0:0B40
...
//^19F0:000A
// SPX: _19f0_000a renamed IS_TILE_BLOCKING_VISION
U16 SkWinCore::IS_TILE_BLOCKING_VISION(i16 iTileX, i16 iTileY)
{
	ENTER(6);
	U16 iTileValue = GET_TILE_VALUE(iTileX, iTileY);	// bp06
	U16 iTileType = iTileValue >> 5;	// si
	if (iTileType == ttDoor) {
		Door *pDoor = GET_ADDRESS_OF_TILE_RECORD(U8(iTileX), U8(iTileY))->castToDoor();	// bp04
		if ((iTileValue & 7) == 4 && GET_DOOR_STAT_0D(GET_GRAPHICS_FOR_DOOR(pDoor)) == 0)	// == 4 => closed door
			return 1;
		return 0;
	}
	if (iTileType != ttWall) {
		if (iTileType != ttTrickWall)
			return 0;
		if ((iTileValue & 4) != 0)	// trickwall & 4 => wall opened
			return 0;
	}
	return 1;
}
							...
							switch (bp0c->w8) {
								...
								case 2:
									if (CALC_VECTOR_DIR(glbCreaturePosX, glbCreaturePosY, bp12, bp14) != glbCreatureDir)
										break;
									bp3c = _19f0_0207(glbCreaturePosX, glbCreaturePosY, bp12, bp14, &SkWinCore::IS_TILE_BLOCKING_VISION);
									if (bp3c <= 0)
										break;
									if (bp0c->w10 < bp3c)
										break;
...
_0d50:
	...
	do {
		...
		if (bp0e == 2) {
			...
			if (_4976_49a0 != 0 || UNIMPLEMENTED_476d_04af(0) == 1) {
				bp16 = 1;
			}
			else {
				bp16 = 0;
...
_1045:
	...
	if (SUPPRESS_WRITER(glbIngameGlobVarWords, _4976_3956, 2, 64) != 0)
		goto _14fa;
	DEBUG_HELP_WRITER("Champion Squad", tblChampionSquad, 261, cd.pi.glbChampionsCount);
	if (SUPPRESS_WRITER(tblChampionSquad, _4976_3992, 261, cd.pi.glbChampionsCount) != 0)
		goto _14fa;
	...
	for (bp0e = 0; bp0e < cd.pi.glbChampionsCount; bp0e++) {
		ObjectID *bp04 = tblChampionSquad[bp0e].inventory;
...
//#endif
	...
	(this->*_sys_getvect(0x08))();	// this is required to proceed mouse button (DOS)

	//printf("MESSAGE LOOP %05d: End\n", iInternalMLCount);
...
X16 SkWinCore::_12b4_0881_CHECK_MOVE_BETWEEN_TILES(X16 aa, U16 oldTile, U16 newTile, i16 xx, i16 yy, ObjectID *rl)
{
	ENTER(0);
	*rl = OBJECT_NULL;
	X16 iOldTileIsStairs = ((oldTile >> 5) == ttStairs) ? 1 : 0;	// di
	if (iOldTileIsStairs != 0 && aa == 2)
		return C1_CHECKMOVE_OLD_TILE_STAIRS;
	if ((newTile >> 5) == ttStairs) {
		return C2_CHECKMOVE_TILE_STAIRS;
	}
	if (IS_TILE_BLOCKED(U8(newTile)) != 0) {
		return C3_CHECKMOVE_TILE_BLOCKED;
	}
	if (!SkCodeParam::bDebugNoCreatureCollision) {
	...
	if (!SkCodeParam::bDebugNoCreatureCollision) {
		ObjectID si = GET_CREATURE_1c9a_03cf(&xx, &yy, 255);	// si
		if (si != OBJECT_NULL) {
			if ((QUERY_CREATURE_AI_SPEC_FLAGS(si) & 0x8000) == 0)
				return C5_CHECKMOVE_STOP;
		}
	}
	return C6_CHECKMOVE_TILE_PASSABLE;
}
...
X16 SkWinCore::CHECK_MOVE_BETWEEN_TILES_AND_INTERWALLS(X16 aa, U16 iInterwall, U16 oldTile, U16 newTile, i16 xx, i16 yy, ObjectID *rl)
{
	...
	if (iInterwall == 1 )//|| iInterwall == 3)
		return C3_CHECKMOVE_TILE_BLOCKED;	// consider tile is blocked (as wall)
	...
	for (i16 si = 0; si < 2; si++) {
		...
		if (bp02 >= 0) {
			if (WOUND_PLAYER(bp02, 1, 0x18, 2) != 0) {
				di = 1;
				QUEUE_NOISE_GEN2(GDAT_CATEGORY_x16_CHAMPIONS, tblChampionSquad[bp02].HeroType(), SOUND_CHAMPION_BUMP, 0xFE, xx, yy, 1, 0x64, 0xC8);
			}
		}
	}
	return di;
...
X16 SkWinCore::_12b4_099e(ObjectID rl)
{
	...
	for (si = 0; si < cd.pi.glbChampionsCount; si++) {
		//^12B4:09C3
		Champion *bp04 = &tblChampionSquad[si];
		if (bp04->curHP() != 0) {
	...
	for (si = 0; si < cd.pi.glbChampionsCount; si++) {
		//^12B4:0A44
		if (tblChampionSquad[si].curHP() != 0) {
			//^12B4:0A54
			if (tblChampionSquad[si].curStamina() > tblChampionSquad[si].maxStamina()) {
				ADJUST_STAMINA(
					si,
...
//^1031:06B3
// SPX: _1031_06b3 renamed SOMETHING_CALL_PFN12_1031_06b3
sk0d9e *SkWinCore::SOMETHING_CALL_PFN12_1031_06b3(sk1891 *ref, X16 xx)
{
	ENTER(12);
	...
	do {
		...
		if ((this->*_4976_0cba_PFN_12[ref->b0_0_6()])(bp08) != 0) {
			if (bp08->b0_7_7() == 1) {
				sk0d9e *bp04 = SOMETHING_CALL_PFN12_1031_06b3(bp08, xx);
				if (bp04 != 0)
					return bp04;
			}
			else {
...
//^476D:04E8
// SPX: _476d_04e8 renamed UNIMPLEMENTED_476d_04e8
U8 SkWinCore::UNIMPLEMENTED_476d_04e8(U16 xx) { // TODO: Unr
	Unr(); return 0;
}

//^476D:04AF
// SPX: _476d_04af renamed UNIMPLEMENTED_476d_04af
U16 SkWinCore::UNIMPLEMENTED_476d_04af(U16 xx) { // TODO: Unr
	Unr(); return 0;
}


//^1031:06A5
// SPX: _1031_06a5 renamed SOMETHING_1031_06a5_CALL_1031_0541
void SkWinCore::SOMETHING_1031_06a5_CALL_1031_0541()
{
	ENTER(0);
...
//^2FCF:0B8B
// SPX: _2fcf_0b8b renamed TELEPORTER_2fcf_0b8b
void SkWinCore::TELEPORTER_2fcf_0b8b(U16 xx, U16 yy, U16 zz)
{
	ENTER(6);
...
//^3929:0044
SkTextFont *SkWinCore::QUERY_CHAR_METRICS(U8 cls2, U8 yy, U8 chr, U8 *tableIdxOut)
{
	ENTER(6);
	SkTextFont *bp04 = _4976_5bfa[cls2];
...
U16 SkWinCore::QUERY_STR_METRICS(const U8 *buff, i16 *xx, i16 *yy)
{
	...
	if (QUERY_MBCS_PRESENCE(buff) != 0) {
		...
		for (; *buff != 0; ) {
			...
			U8 bp08;
			SkTextFont *bp04 = QUERY_CHAR_METRICS(bp05, bp06, bp07, &bp08);
			if (bp04 != NULL) {
...
void SkWinCore::_3a15_020f()
{
	...
	for (si = 0; si < MAX_CHAMPIONS; si++) {
		tblChampionSquad[si].timerIndex = TIMER_NONE;
	}
	si = 0;
	...
	for (; si < timerCnt; si++, timer++) {
		switch (timer->TimerType()) {
			case tty0C:
				tblChampionSquad[timer->actor].timerIndex = si;
				break;
			case tty1D:
			case tty1E:
...
//^098D:1208
// SPX: _098d_1208 renamed INIT_LOAD_RECTS_ZONES
void SkWinCore::INIT_LOAD_RECTS_ZONES() //#DS=4976?
{
	// TODO: 本ズレ函数
	// SPX: Gdat item 01-00-00 contains the rectangle zones.
	LOAD_RECTS_AND_COMPRESS(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x00_BASE_DATA, 0);	// (1, 0, 0)
	glbRectZonesLoaded = 1;
	return;
}
...
//^098D:1185
void SkWinCore::LOAD_RECTS_AND_COMPRESS(U8 iGDatCls1Category, U8 iGDatCls2MainItemId, U8 iGDatCls4EntryId) //#DS=4976?
{
	// TODO: 本ズレ函数
	U32 iDataLength = QUERY_GDAT_ENTRY_DATA_LENGTH(iGDatCls1Category, iGDatCls2MainItemId, fmtRect, iGDatCls4EntryId);	// bp08
	U8* pDataBuffer = ALLOC_MEMORY_RAM(iDataLength, afUseLower, 1024);	// bp04
	LOAD_GDAT_ENTRY_DATA_TO(iGDatCls1Category, iGDatCls2MainItemId, fmtRect, iGDatCls4EntryId, pDataBuffer);
	COMPRESS_RECTS((i16 *)pDataBuffer, iDataLength, &glbRectNoTable, &SkWinCore::ALLOC_UPPER_MEMORY);
	DEALLOC_LOWER_MEMORY(iDataLength);

	// SPX: for Beta, this item does not exist. Load preextracted V4 bin instead
	if (SkCodeParam::bDM2BetaGDATDetected && iDataLength == 0) {
		i16 hRectTabHandle = -1;
		hRectTabHandle = FILE_OPEN((const U8*)"bin/v4/0646rect.bin");
		iDataLength = 17280;
		FILE_READ(hRectTabHandle, iDataLength, pDataBuffer);
		FILE_CLOSE(hRectTabHandle);
		COMPRESS_RECTS((i16 *)pDataBuffer, iDataLength, &glbRectNoTable, &SkWinCore::ALLOC_UPPER_MEMORY);
	}
}
...
//^098D:0E78
void SkWinCore::COMPRESS_RECTS(i16 *data, Bit32u size, RectTable* pRectTable, U8 *(SkWinCore::*allocFunc)(Bit32u size))
{
...
#define	WRITE_WORD(PTR,VAL) **(U16 **)&(PTR) = (VAL);
	...
	// TODO: 本ズレ函数
	if (*(data++) != (i16)0xFC0D)	// looks like a magic number for the RectZone raw data item, it's the same through the different GDAT versions.
		return;
	i16 bp0e = *(data++);
	i16 *bp08 = data;
	data += bp0e << 1;
	while (pRectTable->pNextTable != NULL) {
	...
	for (; --bp0e >= 0; ) {
		...
		U32 bp12 = CALC_SIZE_OF_COMPRESSED_RECT(bp17) * si + sizeof(RectTable);

		if ((bp17 & 1) != 0) {
			bp12 += 2;
		}
		RectTable* pNewRectTable = reinterpret_cast<RectTable *>((this->*allocFunc)(bp12));	// bp04
		pRectTable = (pRectTable->pNextTable = pNewRectTable);
		pNewRectTable->pNextTable = NULL;
		pNewRectTable->iRectNoMin = bp0a;
		pNewRectTable->iRectNoMax = bp0c;
		pNewRectTable->b8 = (U8)bp17;
		pNewRectTable->b9 = (U8)bp1a;
		SkD((DLV_RCT, "RCT: COMPRESS_RECTS (%p,(%d,%d,%d,%d))\n"
			, (RectTable*)pNewRectTable->pNextTable, (U16)pNewRectTable->iRectNoMin, (U16)pNewRectTable->iRectNoMax, (U8)pNewRectTable->b8, (U8)pNewRectTable->b9));
		pNewRectTable++;
		if ((bp17 & 1) != 0) {
			WRITE_WORD(pNewRectTable,bp1c);
			PTR_ADVANCE(pNewRectTable,+2);
		}
		data = bp16;

		do {
			if ((bp17 & 4) != 0) {
				WRITE_BYTE(pNewRectTable,(U8)*data); data++;
				PTR_ADVANCE(pNewRectTable,+1);
				WRITE_BYTE(pNewRectTable,(U8)*data); data++;
				PTR_ADVANCE(pNewRectTable,+1);
			}
			else {
				if ((bp17 & 2) == 0) {
					WRITE_WORD(pNewRectTable,*data);
					PTR_ADVANCE(pNewRectTable,+2);
				}
				data++;
				if ((bp17 & 1) == 0) {
			...
			if ((bp17 & 24) != 0) {
				...
				WRITE_WORD(pNewRectTable,*data); data++;
				PTR_ADVANCE(pNewRectTable,+2);
				WRITE_WORD(pNewRectTable,*data); data++;
				PTR_ADVANCE(pNewRectTable,+2);
			}
...
//^098D:030F
SRECT *SkWinCore::QUERY_RECT(RectTable* entry, U16 iRectNo) //#DS=4976?
{
#define	PTR_ADVANCE(PTR,DELTA) *((U8 **)&(PTR)) += (DELTA);
...
#define	READ_WORD(PTR) **((U16 **)&(PTR))
	...
	U16 iRequestedRectNo = iRectNo;	// U16 si
	if (iRequestedRectNo == 0)
		return NULL;
	for (; entry != NULL; entry = entry->pNextTable) {
		if (entry->iRectNoMin > iRequestedRectNo || entry->iRectNoMax < iRequestedRectNo)
			continue;
		iRequestedRectNo -= entry->iRectNoMin;
		glbRectRingBufferIndex++;
		if (glbRectRingBufferIndex >= 4) {
			glbRectRingBufferIndex = 0;
		}
		sk4b84 *bp04 = &tblRectRingBuffer[glbRectRingBufferIndex];	// bp04
		U8 bp05 = entry->b8;
		...
		SkD((DLV_RCT, "RCT: QUERY_RECT(%4d,(%3d,%3d,%3d,%3d))\n", (Bitu)iRectNo, (Bitu)bp04->w0, (Bitu)bp04->w2, (Bitu)bp04->w4, (Bitu)bp04->w6));
...
//^00EB:0845
//SPX: _00eb_0845 renamed BLIT_IMAGE_00eb_0845
void SkWinCore::BLIT_IMAGE_00eb_0845(U8 *buff, SRECT *rc, U16 ww) //#DS=04BF
{
	LOADDS(0x0c48);
...
RawEntry *SkWinCore::QUERY_GDAT_ENTRYPTR(U8 iGDatCategory, U16 iGDatItemId, U8 iGDatEntryType, U16 iGDatEntryId)
{
	...
	if (glbGDatEntries.tblCls1toCls2[iGDatCategory + 1] - si < iGDatEntryType) {
		return NULL;
	}
	si += iGDatEntryType;
...
//^0AAF:01DB
// SPX: _0aaf_01db renamed RECT_0aaf_01db
void SkWinCore::RECT_0aaf_01db(U16 iRectNo, U16 yy)
{
	if (glbGDatStructureRead != 0) {
		if (yy == 0)
			return;
		...
		U8* bp08;
		if (_4976_5d76 != 0) {
			bp08 = QUERY_GDAT_IMAGE_LOCALPAL(glbGDATItemCls1Category, glbGDATItemCls2MainItemId, glbGDATItemCls4EntryId);
		}
		else {
			bp08 = xImageBuffer + CALC_IMAGE_BYTE_LENGTH(xImageBuffer);
		}
		DRAW_DIALOGUE_PARTS_PICT(xImageBuffer, &bp10, 7, bp08);
		FREE_PICT_ENTRY(xImageBuffer);
	}
	else {
		U8 si = (yy != 0) ? (glbPaletteT16[C00_COLOR_BLACK]) : (glbPaletteT16[C01_COLOR_DARK_GRAY]);
		SRECT bp10;
		FIRE_FILL_BACKBUFF_RECT(QUERY_EXPANDED_RECT(iRectNo, &bp10), si);
	}
...
U16 SkWinCore::QUERY_GDAT_ENTRY_DATA_INDEX(U8 iGDatCls1Category, U16 iGDatCls2MainItemId, U8 iGDatCls3DataType, U16 iGDatCls4EntryId)
{
//LOGX(("%40s: C%02d=I%02X=E%02X=T%03d to %08X", "QUERY_GDAT_ENTRY_DATA_INDEX from ", iCategory, iItem, iEntry, iType ));
	RawEntry* xRawData = QUERY_GDAT_ENTRYPTR(iGDatCls1Category, iGDatCls2MainItemId, iGDatCls3DataType, iGDatCls4EntryId);	// bp04
	if (xRawData == NULL) {
		switch (iGDatCls3DataType) {
...
//^3E74:0422
U16 SkWinCore::QUERY_GDAT_RAW_DATA_LENGTH(U16 iRawDatIndex)
{
	ENTER(4);
	...
	if (iRawDatIndex > glbGDatNumberOfData && SkCodeParam::bUsePowerDebug)
	{
	//	ATLASSERT(index < glbGDatNumberOfData);
		return 0;
	}
	
	xShelfMem = glbShelfMemoryTable[iRawDatIndex];
	//--- SPX END
...
//^3E74:1586
U16 SkWinCore::IS_CLS1_CRITICAL_FOR_LOAD(U8 iGDatCls1Category)
{
	switch (iGDatCls1Category) {
...
void __DECLSPEC_NORETURN_ SkWinCore::RAISE_SYSERR(U16 syserr)
{
	...
	if (glbFontLoaded != 0) {
		...
		if (_4976_474a != 0) {
			...
			SLEEP_SEVERAL_TIME(0x012C);	// 0x012C = 300
		}
		SK_PREPARE_EXIT();
	}
#endif
...
void SkWinCore::GRAPHICS_DATA_READ(U32 location, U32 size, X8 *buff)
{
	...
	if (glbFloppyDiskFlag != 0) {
		bp14[1] = glbFileHandleGraphics2;
		bp08[1] = location - glbSizeOfGDATFile;
		if (bp08[1] >= 0) {
			bp10[1] = size;
		}
		else {
...
//LOGX((" + LOAD_GDAT_RAW_DATA loads GDAT item %04d to ptr %08x", index, &ps ));
	...
	while (bp04 > 0) {
		if (si != _4976_480d || _4976_5d34 != 0) {
			_4976_480d = si;
			U32 bp18 = glbSizeOfGDATFull - bp08;
...
void SkWinCore::DUMP_5CA4() {
	...
	for (int x=0; x<glbNumberOfMements; x++) {
		if (tblMementsPointers[x] == NULL) continue;
		SkD((DLV_DBG_EXTRACT,
			"#%02d %p (%6d,%04X,%04X,%04X,%04X,%04X,%04X,%04X)\n"
			, x, tblMementsPointers[x]
			, tblMementsPointers[x]->_dw0
			, tblMementsPointers[x]->_w4
			, tblMementsPointers[x]->_w6
			, tblMementsPointers[x]->_w8
			, tblMementsPointers[x]->_w10
			, tblMementsPointers[x]->_w12
			, tblMementsPointers[x]->_w14
			, tblMementsPointers[x]->_w16
			));
	}
	SkD((DLV_DBG_EXTRACT,
		"=====\n"));
...
U8* SkWinCore::QUERY_GDAT_IMAGE_ENTRY_BUFF(U8 iGDatCategory, U8 iGDatItemId, U8 iGDatEntryId)	// (U8 cls1, U8 cls2, U8 cls4)
{
	...
	RawEntry* pEntry = QUERY_GDAT_ENTRYPTR(iGDatCategory, iGDatItemId, dtImage, iGDatEntryId);
	U16 iGDATItemID;
	if (pEntry == NULL) {
		iGDATItemID = 0xFFFF;
	}
		...
		if ((bp11 == '.' && format[1] == 'Z') || bp11 == 0x01) {
			...
			*bp04 = 0;
			...
			switch (si) {
				...
				case 0x0007:	// .Z007
					{
						if (glbChampionBonesIndex == 0xFFFF)
							break;
						bp0c = (U8 *)&tblChampionSquad[glbChampionBonesIndex];
						break;
					}
				case 0x0008:	// .Z008
					{
				...
				case 0x000c:	// .Z012
					{
						U32 iValue = glbChampWeightLoadDisplay;	// bp10
						SK_LTOA10(iValue, bp0c);
						break;
					}
				case 0x000d:	// .Z013
					{
						Bit32u bp10 = glbChampWeightLoadTenthDisplay;
						SK_LTOA10(bp10, bp0c);
						break;
					}
				case 0x000e:	// .Z014
					{
						Bit32u bp10 = glbChampMaxLoadDisplay;
						SK_LTOA10(bp10, bp0c);
						break;
					}
				case 0x000f:	// .Z015
...
/*
	...
	*/
	...
	U16 iTotalMapColumns = 0;	// bp0e total map column
	for (iMapIndex = 0; iMapIndex < nMaps; iMapIndex++) {
		dunMapColumnsSumArray[iMapIndex] = iTotalMapColumns;
		iTotalMapColumns += dunMapsHeaders[iMapIndex].Column();
	...
	if (SKLOAD_READ(dunMapTilesObjectIndexPerColumn, iTotalMapColumns << 1) == 0)
		return 0;
	if (iSwapEndian)
		SWAPWPX(dunMapTilesObjectIndexPerColumn, iTotalMapColumns);
...
U16 SkWinCore::READ_SKSAVE_DUNGEON()
{
	...
	for (iChampionIndex = 0; iChampionIndex < cd.pi.glbChampionsCount; iChampionIndex++) {
		ObjectID* xInvObject = &tblChampionSquad[iChampionIndex].inventory[0];	// bp08
...
_2e5b:
		if (_4976_49a0 == 0 && UNIMPLEMENTED_476d_04af(1) != 1) {
			switch (_4976_5eb8) {
				case 0x0000:
					bp01 = 0x1d;
					break;
	...
	if (bp6a.wTimerFlag == 1) // native DM2 savegame
	{
		do {
			...
			if (SUPPRESS_READER(glbIngameGlobVarWords, _4976_3956, 2, 64, 1) != 0)
				break;
			if (SUPPRESS_READER(tblChampionSquad, _4976_3992, 261, cd.pi.glbChampionsCount, 1) != 0) // player ents
				break;
...
_31b8:		// we jump there from loading a dungeon from new game
			...
			TELEPORTER_2fcf_0b8b(cd.pi.glbPlayerPosX, cd.pi.glbPlayerPosY, cd.pi.glbPlayerMap);
	...
	else	// try read as DM1 savegame
	{
		do {
			...
			for (iChampionIndex = 0; iChampionIndex < 4; iChampionIndex++)
			{
				...
				*((U16*)(&cd.dm1.xDM1PortraitsData[iChampionIndex])) = IMG_4_BPP; // 4bpp
...
{
	Text* pText = GET_ADDRESS_OF_RECORD2(recordLink);	// bp04
	if (pText->TextMode() == C01_ACT_SIMPLE__MODE_ACTIVE_DECORATION) {
		U16 iExtUsage = pText->SimpleTextExtUsage();	// si
		switch (iExtUsage) {
...
//^0CD5:0176
// SPX: _0cd5_0176 renamed GET_SCALED_PRODUCT
// ReDMCSB: F0030_MAIN_GetScaledProduct
// CSBWin: TAG001098/ScaledMultiply
i16 SkWinCore::GET_SCALED_PRODUCT(U16 iValue, U16 iDivideScale, U16 iMultiplier)
{
	ENTER(0);
	return (U32(iValue) * U32(iMultiplier)) >> iDivideScale;
}

//^098D:000F
// SPX: _098d_000f renamed BUILD_5x5_POS
void SkWinCore::BUILD_5x5_POS(i16 xx, i16 yy, U16 ww, U16 *x2, U16 *y2)
{
	ENTER(0);
	*x2 = (xx << 2) + (ww % 5);
	*y2 = (yy << 2) + (ww / 5);
...
{
	ENTER(4);
	ATLASSERT(glbSkEntriesCount < C400_MAX_DYN_ENTRIES);	// 400
	SkLoadEnt* xSkEntry = &tblSkEntries[glbSkEntriesCount];	// bp04
	glbSkEntriesCount++;
	xSkEntry->w0(0);
	xSkEntry->xEnt4.cls1(U8(aa >> 24));
	xSkEntry->xEnt4.cls2(U8(aa >> 16));
	xSkEntry->xEnt4.cls3(U8(aa >>  8));
	xSkEntry->xEnt4.cls4(U8(aa      ));
	return;
}

//^2676:008F
void SkWinCore::MARK_DYN_EXCLUDE_RANGE(U32 ent4, U8 iGDatCls4EntryId)
{
	ENTER(0);
	MARK_DYN_LOAD(ent4);
	tblSkEntries[glbSkEntriesCount -1].MarkContinuousExcluded();
	MARK_DYN_LOAD((ent4 & 0xffffff00) | iGDatCls4EntryId);
	return;
}
...
void SkWinCore::_2676_006a(U32 aa)
{
	ENTER(0);
	MARK_DYN_LOAD(aa);
	tblSkEntries[glbSkEntriesCount -1].MarkExcluded();
	return;
}

//^2676:00D0
// SPX: _2676_00d0 renamed MARK_WORDVAL_DYN_LOAD
void SkWinCore::MARK_WORDVAL_DYN_LOAD(U8 iGDatCls1Category, U8 iGDatCls2MainItemId, U8 iGDatCls4EntryId)
{
	ENTER(2);
	X16 bp02 = QUERY_GDAT_ENTRY_DATA_INDEX(iGDatCls1Category, iGDatCls2MainItemId, dtWordValue, iGDatCls4EntryId);
	if (bp02 == 0)
		return;
	iGDatCls2MainItemId = U8(bp02);
	if (iGDatCls2MainItemId != 0) {
		MARK_DYN_LOAD(iGDatCls2MainItemId + 0xffff0d00);
	}
	bp02 >>= 8;
	if (bp02 != 0) {
...
_19c0:
			...
			ref->cls1base(ref->x8.xEnt4.cls1());
...
_1a92:
				...
				ref->cls3base(ref->x8.xEnt4.cls3());
...
//^3E74:33A4
void SkWinCore::LOAD_DYN4(SkLoadEnt *ref, i16 iLoadEntCount)	// aa
{
	ENTER(92);
	X16 bp38 = 0;
	X16 bp3a = 0;
	U8 *pTabGDatRawDat = ALLOC_MEMORY_RAM(glbGDatNumberOfData, afZeroMem, 1024);	// bp04
	// flags: XY-#####
	//           ref. cnt
	//         pool flag?
	//        pool flag?
	...
	SkLoadEnt *pLoadEntMarks = ref;	// bp08	// pointer to load-ent, 400 max
	i16 iLoadEntIndex = 0;	// bp12
	i16 bp12;	// SPX: another bp12 for rawdat index
	SkEntIter bp5c;
	X16 iCurIndex;	// si
	X16 bp2e;
...
//LOGX(("LOAD_DYN4: %02d / %02d", iLoadEntIndex, aa));
//LOGX(("LOAD_DYN4: MASK: %s", DEBUG_SKLOADENT((U8*)pLoadEntMarks) ));

		SkD((DLV_DYN, "Dyn: Test(%04X,%02X,%02X,%02X,%02X)\n"
			, 0U +pLoadEntMarks->w0(), 0U +pLoadEntMarks->xEnt4.cls1(), 0U +pLoadEntMarks->xEnt4.cls2(), 0U +pLoadEntMarks->xEnt4.cls3(), 0U +pLoadEntMarks->xEnt4.cls4()));
		...
		while (QUERY_NEXT_GDAT_ENTRY(&bp5c) != 0) {
			...
			if ((bp2e & 0x8000) != 0) {
				...
				if (bp2b > 0 && bp2b != RAWDAT_DYN_MARK_x1F) {
					--pTabGDatRawDat[iCurIndex];
				}
				continue;
			}
			if (bp2b != 0) {
				if ((bp2b & RAWDAT_DYN_MARK_x1F) != RAWDAT_DYN_MARK_x1F) {
					++pTabGDatRawDat[iCurIndex];
				}
				continue;
			}
			if (bp2c == 2) {
				if (AUDIO_482b_015c(iCurIndex) != 0 || _4976_5bee[2] != 0)
					continue;
			}
			pTabGDatRawDat[iCurIndex] = 1;
			X16 bp36 = _4976_481a[RCJ(15,bp2c)];
			if ((bp36 & glbTiamatPoolInfo1.iMemPoolFlags) != 0 && (bp36 & glbTiamatPoolInfo2.iMemPoolFlags) == 0) {
				pTabGDatRawDat[iCurIndex] |= RAWDAT_DYN_MARK_x40;
			}
			else if ((bp36 & glbTiamatPoolInfo2.iMemPoolFlags) != 0 && (bp36 & glbTiamatPoolInfo1.iMemPoolFlags) == 0) {
				pTabGDatRawDat[iCurIndex] |= RAWDAT_DYN_MARK_x80;
			}
		}
		pLoadEntMarks++;
		if (iLoadEntCount -1 == iLoadEntIndex && bp3a == 0) {
			bp3a = 1;
			iLoadEntIndex--;
			pLoadEntMarks = &bp40;
			bp40.w0(0);
			bp40.xEnt4 = MAKE_ENT4(GDAT_CATEGORY_x15_MISCELLANEOUS,0xFE,0x01,0xFE); // add our smily ":P" once
		}
	}
	for (iCurIndex = 0; iCurIndex < glbGDatNumberOfData; iCurIndex++) {
		bp12 = pTabGDatRawDat[iCurIndex];
		if (bp12 != 0) {
			if ((bp12 & 0x1f) != 0) {
				pTabGDatRawDat[iCurIndex] = (bp12 & RAWDAT_DYN_MARK_xE0) | RAWDAT_DYN_MARK_x21;
			}
			else {
				pTabGDatRawDat[iCurIndex] &= RAWDAT_DYN_MARK_x20;
			}
		}
	}
	pLoadEntMarks = ref;
	...
	if (_4976_5d78 != 0) {
		for (iCurIndex = 0; iCurIndex < glbGDatNumberOfData; iCurIndex++) {
			if ((pTabGDatRawDat[iCurIndex] & 1) == 0)
				continue;
			X16 di;
			if ((di = TRACK_UNDERLAY(iCurIndex)) != 0xFFFF) {
				while ((pTabGDatRawDat[di] & 1) == 0) {
					pTabGDatRawDat[di] |= pTabGDatRawDat[iCurIndex] & 0xc1;
...
_37e9:
		;
	} while (glbMement5 != NULL);
	for (iCurIndex = 0; iCurIndex < glbGDatNumberOfData; iCurIndex++) {
		shelf_memory iShelfMem = glbShelfMemoryTable[iCurIndex];	// bp08
		if (iShelfMem.Absent())
			continue;
		if (CHECK_SHELF_IN_BOUNDS_3e74_00ed(&glbTiamatPoolInfo1, iShelfMem) == 0 && CHECK_SHELF_IN_BOUNDS_3e74_00ed(&glbTiamatPoolInfo2, iShelfMem) == 0)
			continue;
		bp12 = pTabGDatRawDat[iCurIndex];
		bp12 &= ((bp12 & 1) != 0) ? 0xFFF9 : 0x0020;
		pTabGDatRawDat[iCurIndex] = U8(bp12);
		_3e74_3200(iCurIndex);
	}
	TIAMAT_POOL_3e74_30bd(&glbTiamatPoolInfo1);
	TIAMAT_POOL_3e74_30bd(&glbTiamatPoolInfo2);
	glbFreeCPXMemory += tiamat::Size(glbTiamatPoolInfo1.t8, glbTiamatPoolInfo1.memRefLower);
	SkD((DLV_CPX,"CPX: glbFreeCPXMemory += %u\n", (Bitu)tiamat::Size(glbTiamatPoolInfo1.t8, glbTiamatPoolInfo1.memRefLower)));
	glbTiamatPoolInfo1.memRefLower = glbTiamatPoolInfo1.t8;

_38db:
	X32 iTotalMemPool1a = 0;	// bp1a
	X32 iTotalMemPool2 = 0;	// bp16
	X32 bp1e = GET_TIAMAT_3e74_0017(&glbTiamatPoolInfo2);
	U32 iDiffMemSize = tiamat::Size(glbTiamatPoolInfo1.t0, ptr2t(reinterpret_cast<U8 *>(_4976_5ce6)));	// bp22	size between 1st and 2nd
	i32 iRequiredMemSize;	// bp10
	for (iCurIndex = 0; iCurIndex < glbGDatNumberOfData; iCurIndex++) {
		bp12 = pTabGDatRawDat[iCurIndex] = pTabGDatRawDat[iCurIndex] & RAWDAT_DYN_MARK_xE1;
		shelf_memory iShelfMem = glbShelfMemoryTable[iCurIndex];	// bp08
		if (iShelfMem.Present()) {
			iRequiredMemSize = ((READ_RAW_DATA_LENGTH(REALIZE_GRAPHICS_DATA_MEMORY(iShelfMem)) +1) & 0xFFFE) +4;	// bp10 = ((READ_UI16(REALIZE_GRAPHICS_DATA_MEMORY(iShelfMem),-2) +1) & 0xfffe) +4;
			if (CHECK_SHELF_IN_BOUNDS_3e74_00ed(&glbTiamatPoolInfo1, iShelfMem) != 0) {
				pTabGDatRawDat[iCurIndex] |= RAWDAT_DYN_MARK_x0A;
				if ((bp12 & 1) == 0) {
					pTabGDatRawDat[iCurIndex] |= RAWDAT_DYN_MARK_x40;
				}
_39ec:
				iTotalMemPool2 += iRequiredMemSize;
			}
			else {
				if (CHECK_SHELF_IN_BOUNDS_3e74_00ed(&glbTiamatPoolInfo2, iShelfMem) != 0) {
					pTabGDatRawDat[iCurIndex] |= RAWDAT_DYN_MARK_x14;
					if ((bp12 & 1) == 0) {
						pTabGDatRawDat[iCurIndex] |= RAWDAT_DYN_MARK_x80;
					}
					goto _3abe;
				}
				pTabGDatRawDat[iCurIndex] = 0;
			}
			continue;
		}
		if ((bp12 & 1) != 0) {
			iRequiredMemSize = ((iShelfMem.SizeIfAbsent() +1) & 0xFFFE) +4;
			if (bp1e == 0 || (bp12 & RAWDAT_DYN_MARK_x40) != 0) {
				pTabGDatRawDat[iCurIndex] |= RAWDAT_DYN_MARK_x08;
				goto _39ec;
			}
			else {
				pTabGDatRawDat[iCurIndex] |= RAWDAT_DYN_MARK_x10;
_3abe:
				iTotalMemPool1a += iRequiredMemSize;
			}
		}
	}

	if (iTotalMemPool1a > bp1e) {
		for (iCurIndex = 0; (iDiffMemSize -iTotalMemPool2 > _4976_480f) && (iTotalMemPool1a > bp1e) && (iCurIndex < glbGDatNumberOfData); iCurIndex++) {
			if ((pTabGDatRawDat[iCurIndex] & RAWDAT_DYN_MARK_x94) == RAWDAT_DYN_MARK_x10) {
				pTabGDatRawDat[iCurIndex] ^= RAWDAT_DYN_MARK_x18;
				iRequiredMemSize = ((QUERY_GDAT_RAW_DATA_LENGTH(iCurIndex) +1) & 0xFFFE) +4;
				iTotalMemPool2 += iRequiredMemSize;
				iTotalMemPool1a -= iRequiredMemSize;
			}
		}
		for (iCurIndex = 0; (iDiffMemSize -iTotalMemPool2 > _4976_480f) && (iTotalMemPool1a > bp1e) && (iCurIndex < glbGDatNumberOfData); iCurIndex++) {
			if ((pTabGDatRawDat[iCurIndex] & RAWDAT_DYN_MARK_x90) == RAWDAT_DYN_MARK_x10) {
				pTabGDatRawDat[iCurIndex] ^= RAWDAT_DYN_MARK_x18;
				iRequiredMemSize = ((QUERY_GDAT_RAW_DATA_LENGTH(iCurIndex) +1) & 0xFFFE) +4;
				iTotalMemPool2 += iRequiredMemSize;
				iTotalMemPool1a -= iRequiredMemSize;
		...
		for (iCurIndex = 0; (iTotalMemPool1a > bp1e) && (iCurIndex < glbGDatNumberOfData); iCurIndex++) {
			if ((pTabGDatRawDat[iCurIndex] & RAWDAT_DYN_MARK_x11) == RAWDAT_DYN_MARK_x10) {
				pTabGDatRawDat[iCurIndex] = pTabGDatRawDat[iCurIndex] & 0xef;
				iTotalMemPool1a -= ((QUERY_GDAT_RAW_DATA_LENGTH(iCurIndex) +1) & 0xFFFE) +4;
			}
		}
		if (_4976_5bee[3] == 0) {
			for (iCurIndex = 0; (iTotalMemPool1a > bp1e) && (iCurIndex < glbGDatNumberOfData); iCurIndex++) {
	...
	for (iCurIndex = 0; (bp1e - iTotalMemPool1a >= 6) && (iCurIndex < glbGDatNumberOfData); iCurIndex++) {
		if ((pTabGDatRawDat[iCurIndex] & RAWDAT_DYN_MARK_x48) == RAWDAT_DYN_MARK_x08) {
			iRequiredMemSize = ((QUERY_GDAT_RAW_DATA_LENGTH(iCurIndex) +1) & 0xFFFE) +4;
			if (i32(bp1e - iTotalMemPool1a) >= iRequiredMemSize) {
				pTabGDatRawDat[iCurIndex] ^= RAWDAT_DYN_MARK_x18;
				iTotalMemPool1a += iRequiredMemSize;
				iTotalMemPool2 -= iRequiredMemSize;
			}
	...
	for (iCurIndex = 0; (iDiffMemSize -iTotalMemPool2 <= _4976_480f) && (iCurIndex < glbGDatNumberOfData); iCurIndex++) {
		if ((pTabGDatRawDat[iCurIndex] & RAWDAT_DYN_MARK_x09) == RAWDAT_DYN_MARK_x08) {
			pTabGDatRawDat[iCurIndex] = pTabGDatRawDat[iCurIndex] & RAWDAT_DYN_MARK_xF7;
			iTotalMemPool2 += ((QUERY_GDAT_RAW_DATA_LENGTH(iCurIndex) +1) & 0xFFFE) +4;
		}
	}

	X32 bp2a = iDiffMemSize - iTotalMemPool2;
	_4976_5d62 = bp1e - iTotalMemPool1a;
	...
	if (glbNewMap_4976_5c8a == 0) {
		...
		bp26 = tiamat::Size(glbTiamatPoolInfo1.t8, glbTiamatPoolInfo1.t0 -iTotalMemPool2);
		for (iCurIndex = 0; iCurIndex < glbGDatNumberOfData; iCurIndex++) {
			bp12 = pTabGDatRawDat[iCurIndex];
			if ((bp12 & 6) != 0 && (bp12 & 0x18) == 0) {
				pTabGDatRawDat[iCurIndex] &= RAWDAT_DYN_MARK_x20;
				FREE_INDEXED_MEMENT(iCurIndex);
				_3e74_3200(iCurIndex);
			}
		}
		if (bp26 > 0) {
		...
		if (bp26 > 0) {
			glbFreeCPXMemory -= bp26;
			SkD((DLV_CPX,"CPX: glbFreeCPXMemory -= %u\n", (Bitu)bp26));
			glbTiamatPoolInfo1.memRefLower = glbTiamatPoolInfo1.t0 - iTotalMemPool2;
		}
		X16 bp32;
		do {
			bp32 = 0;
			for (iCurIndex = 0; iCurIndex < glbGDatNumberOfData; iCurIndex++) {
				bp12 = pTabGDatRawDat[iCurIndex];
				tiamat tmMemRef;	// bp08
				if ((bp12 & RAWDAT_DYN_MARK_x0C) == RAWDAT_DYN_MARK_x0C) {
					iRequiredMemSize = ((QUERY_GDAT_RAW_DATA_LENGTH(iCurIndex) +1) & 0xFFFE) +4;
					if (TIAMAT_POOL_NOT_IMPLEMENTED(&glbTiamatPoolInfo1) < iRequiredMemSize) {
						TIAMAT_POOL_3e74_30bd(&glbTiamatPoolInfo1);
						if (TIAMAT_POOL_NOT_IMPLEMENTED(&glbTiamatPoolInfo1) < iRequiredMemSize) {
							bp32 = 1;
							continue;
					...
					tmMemRef = _3e74_32fb(&glbTiamatPoolInfo1, REALIZE_GRAPHICS_DATA_MEMORY(glbShelfMemoryTable[iCurIndex]), iRequiredMemSize);
					_3e74_3200(iCurIndex);
					goto _4173;
				}
				else if ((bp12 & RAWDAT_DYN_MARK_x12) != RAWDAT_DYN_MARK_x12) {
					continue;
				}
				else {
					iRequiredMemSize = ((QUERY_GDAT_RAW_DATA_LENGTH(iCurIndex) +1) & 0xFFFE) +4;
					if (TIAMAT_POOL_NOT_IMPLEMENTED(&glbTiamatPoolInfo2) < iRequiredMemSize) {
						TIAMAT_POOL_3e74_30bd(&glbTiamatPoolInfo2);
						if (TIAMAT_POOL_NOT_IMPLEMENTED(&glbTiamatPoolInfo2) < iRequiredMemSize) {
							bp32 = 1;
							continue;
					...
					tmMemRef = _3e74_32fb(&glbTiamatPoolInfo2, REALIZE_GRAPHICS_DATA_MEMORY(glbShelfMemoryTable[iCurIndex]), iRequiredMemSize);
					_3e74_3200(iCurIndex);
					if ((glbTiamatPoolInfo2.iMemPoolFlags & 0x8000) != 0) {
						glbShelfMemoryTable[iCurIndex] = t2s(tmMemRef);	// tiamat => shelf	// is for CRAM ?
					}
					else {
_4173:
						glbShelfMemoryTable[iCurIndex] = CONVERT_PHYS_TO_SHELF_FORM(t2ptr(tmMemRef));	// tiamat => pointer => shelf
					}
					pTabGDatRawDat[iCurIndex] ^= RAWDAT_DYN_MARK_x06;
				}
			}
		} while (bp32 != 0);
		TIAMAT_POOL_3e74_30bd(&glbTiamatPoolInfo1);
		...
		for (iCurIndex = 0; iCurIndex < glbGDatNumberOfData; iCurIndex++) {
			...
			if (bp12 == 0 || (bp12 & RAWDAT_DYN_MARK_x06) != 0)
				continue;
			X16 iRawDatLength;	// bp30
			iRequiredMemSize = (((iRawDatLength = QUERY_GDAT_RAW_DATA_LENGTH(iCurIndex)) +1) & 0xFFFE) +4;
			sk5d12 *bp0c = ((bp12 & RAWDAT_DYN_MARK_x08) != 0) ? &glbTiamatPoolInfo1 : &glbTiamatPoolInfo2;	// like choosing a EMS or notEMS ?
			tiamat memRef = MEM_3e74_32a2(bp0c, iRequiredMemSize);	// (bp08) moves pointer backward - iRequiredMemSize
			if (bp0c->Is4EMS() != 0) {
				WRITE_UI16(REALIZE_GRAPHICS_DATA_MEMORY(t2s(memRef) + (iRequiredMemSize) - (2)),+0,iCurIndex);
				memRef = ptr2t(REALIZE_GRAPHICS_DATA_MEMORY(glbShelfMemoryTable[iCurIndex] = t2s(memRef + (2))));
			}
			else {
				glbShelfMemoryTable[iCurIndex] = CONVERT_PHYS_TO_SHELF_FORM(t2ptr(memRef +2));	// tiamat => real pointer => shelf
				WRITE_UI16(t2ptr(memRef),+iRequiredMemSize -4,iCurIndex);	// WRITE_UI16(t2ptr(bp08),+bp10 -4,si);
			}
			// Write the real raw data len at -2
			WRITE_RAW_DATA_LENGTH(t2ptr(memRef),iRawDatLength);	// WRITE_UI16(t2ptr(bp08),-2,bp30);
			if (bp34 == 0) {
				DIALOG_BOX_2066_03e0(0);
				GRAPHICS_DATA_OPEN();
				bp34 = 1;
...
//LOGX(("LOAD_GDAT_RAW_DATA call from LOAD_DYN4"));
			LOAD_GDAT_RAW_DATA(iCurIndex, glbShelfMemoryTable[iCurIndex]);
			SkD((DLV_GLD, "GLD: Dyn4-load Raw#%4d at S(%08lX)\n", (Bitu)iCurIndex, (Bitu)glbShelfMemoryTable[iCurIndex].val));
		}
		if (bp34 != 0)
			GRAPHICS_DATA_CLOSE();
		glbCpxHeapLimitPtr = t2ptr(glbTiamatPoolInfo1.memRefLower);
		AUDIO_482b_0684();
		if (_4976_4bd8 != 0) {
...
//^098D:04C7
// SPX: _098d_04c7 renamed SOMETHING_WITH_SCALED_RECTANGLES
X16 SkWinCore::SOMETHING_WITH_SCALED_RECTANGLES(X16 iRectNo1, X16 iRectNo2, X16 scale, X16 *ss, X16 *tt)
{
	ENTER(10);
	i16 iScale = scale;	// si
	SRECT *pRect1 = QUERY_RECT(&glbRectNoTable, iRectNo1);	// bp04
	if (pRect1 == NULL) {
		return -1;
	}
	SRECT *pRect2 = QUERY_RECT(&glbRectNoTable, iRectNo2);	// bp08
	if (pRect2 == NULL) {
		return -1;
	}
	iScale = BETWEEN_VALUE(0, iScale, 0x64);	// 0 <= iScale <= 100
	if (iScale == 0) {
		*ss = 0;
		*tt = 0;
		return iRectNo1;
	}
	i16 bp0a = pRect2->cx - pRect1->cx;
	*ss = (bp0a * iScale) / 100;
	i16 di = pRect2->cy - pRect1->cy;
	*tt = (di * iScale) / 100;
	return iRectNo1;
}
...
void SkWinCore::LOAD_LOCALLEVEL_DYN()
{
	...
	tblSkEntries = reinterpret_cast<SkLoadEnt *>(ALLOC_MEMORY_RAM(sizeof(SkLoadEnt) * C400_MAX_DYN_ENTRIES, 0,  1024));
	...
	if (_4976_5bee[0] != 0) {
		...
		tblSkEntries[glbSkEntriesCount -1].MarkContinuous();
	...
	tblSkEntries[glbSkEntriesCount -1].MarkIncluded();
	SkD((DLV_DBG_INIT, "LOAD_LOCALLEVEL_DYN: pCurrentTileMap :%08X\n", glbCurrentTileMap));
	...
	if (_4976_5bee[0] == 0) {
		for (bp02 = 0; bp02 < cd.pi.glbChampionsCount; bp02++) {
			MARK_DYN_LOAD(0x16000100 + (tblChampionSquad[bp02].HeroType() << 16)); // Mark: Champions, xxx, image, 0x00(wake)
			MARK_DYN_LOAD(0x160002ff + (tblChampionSquad[bp02].HeroType() << 16)); // Mark: Champions, xxx, snd, all
			MARK_DYN_EXCLUDE_RANGE(0x16000508 + (tblChampionSquad[bp02].HeroType() << 16), 0xb); // Mark: Champions, xx, text, 0x08 to 0x0B
			tblSkEntries[glbSkEntriesCount -1].MarkIncluded();
		}
	}
	...
	if (_4976_5bee[0] != 0) {
		...
		for (bp02 = 0; bp02 < 0xFA; bp02++) {
			if (bp18[bp02] != 0) {
				...
				MARK_WORDVAL_DYN_LOAD(GDAT_CATEGORY_x09_WALL_GFX, U8(bp02), 0x40);	// (0x9, U8(bp02), 0x40)
			}
		}
	}
	...
	if (_4976_5bee[0] != 0) {
		...
		for (bp02 = 0; bp02 < 0xfa; bp02++) {
			...
			MARK_WORDVAL_DYN_LOAD(GDAT_CATEGORY_x0A_FLOOR_GFX, U8(bp02), 0x40);	// (0xa, U8(bp02), 0x40);
		}
	}
	if (_4976_5bee[0] != 0) {
	...
	for (bp03 = 0; bp03 < 0xfa; bp03++) {
		...
		if ((bp14[bp03] & 1) != 0 || _4976_5bee[1] != 0) {
			...
			MARK_WORDVAL_DYN_LOAD(GDAT_CATEGORY_x0F_CREATURES, bp03, 0x40);	// (0xf, bp03, 0x40)
		}
		else {
	...
	LOAD_DYN4(tblSkEntries, glbSkEntriesCount);
	...
	if (_4976_5be6 > bp32) {
		_4976_5be6 -= bp32;
		SET_SRECT(&_4976_5938, 0, bp36, glbViewportWidth, glbViewportHeight -(bp36 +bp3a));
	}
	else {
		_4976_5be6 = 0;
	}
...
void SkWinCore::CHANCE_TABLE_OPERATION()
{
	...
	if (bp02 < 0 || di < 0 || bp02 >= glbViewportWidth || glbScreenHeight - _4976_00ea <= di)
		return;
	bp02 -= glbViewportWidth >> 1; // horz pos in viewport
	di -= glbViewportHeight >> 1; // vert pos in viewport
	X16 si;
	X16 bp04;
	X16 bp06;
	X16 bp08;
...
//^32CB:5C67
// SPX: _32cb_5c67 renamed GET_SOME_VIEWPORT_INFO_TELEPORTER
void SkWinCore::GET_SOME_VIEWPORT_INFO_TELEPORTER()
{
	ENTER(8);
	i16 si;
	for (si = 0; si < 0x15; si++) {
...
//^32CB:5A8F
// SPX: _32cb_5a8f renamed SET_SOME_CELL_INFO_32cb_5a8f
void SkWinCore::SET_SOME_CELL_INFO_32cb_5a8f()
{
	ENTER(14);
	X32 bp04 = 0;
	X32 bp08 = 0;
...
//^32CB:2D8C
// SPX: _32cb_2d8c renamed _32cb_2d8c_DRAW_TILE
void SkWinCore::_32cb_2d8c_DRAW_TILE(ObjectID rl, X16 iViewportCell, X32 aa)
{
	//^32CB:2D8C
	ENTER(46);
	//^32CB:2D92
	X16 di = iViewportCell;	// di
	ObjectID bp0c = rl;
...
_2eda:
				...
				if (bp0a > 0) {
					...
					if (bp26 != 0 && glbTabYAxisDistance[0] == bp1c)
						...
						bp0a = 0x1e;
					//^32CB:30AE
					glbTempPicture.iYDisp += (glbViewportHeight << 3) / bp0a;
					if (bp08->b2_7_7() == 0) {
						//^32CB:30CD
						bp0a = (glbViewportWidth << 3) / bp0a;
						if (si == 1 || si == 4) {
							//^32CB:30E4
							bp0a = -bp0a;
						}
						//^32CB:30EC
						glbTempPicture.iXDisp += bp0a;
					}
				}

				//^32CB:30F3
				glbTempPicture.iXDisp = CALC_STRETCHED_SIZE(glbTempPicture.iXDisp, bp22);
				glbTempPicture.iYDisp = CALC_STRETCHED_SIZE(glbTempPicture.iYDisp, bp22);
				DRAW_TEMP_PICST();
				if (bp26 != 0) {
					goto _2eda;
				}
...
// SPX: _1c9a_03cf renamed GET_CREATURE_1c9a_03cf
ObjectID SkWinCore::GET_CREATURE_1c9a_03cf(i16 *xx, i16 *yy, U16 dir)
{
	//^1C9A:03CF
	//^1C9A:03D5
	i16 bp0c = *xx;
		...
		case ttTrickWall://-2:	// TRICKWALL
			{
				...
				if (bp16 != 0) {
					//^075F:0DB2
					if ((((tblChampionSquad[bp12].playerDir() == (glbTimersTable[bp08->TimerIndex()].Direction() +2)) ? 1 : 0) & 3) != 0) {
						//^075F:0DF2
...
_0e37:
				...
				bp3a = ypos;
				//^075F:0E43
				ObjectID bp2e = GET_CREATURE_1c9a_03cf(&bp38, &bp3a, dir);
				//^075F:0E5B
				if (bp2e == OBJECT_NULL) {
					//^075F:0E60
					if (tileType == ttTeleporter)
				...
				if ((*(bp04++) & 0x10) != 0) {
					...
					while (true) {
						...
						if (bp1c == dbActuator) {
							...
							Text* xTextObject = GET_ADDRESS_OF_RECORD(si)->castToText();	// bp08
							if (xTextObject->TextMode() != 1) {
								goto _1314;
							}
							if (xTextObject->SimpleTextExtUsage() != 4) {
								goto _1314;
							}
							goto _1324;
						}
							...
							GenericRecord* xGenericItem = GET_ADDRESS_OF_RECORD(si);	// bp08
							switch (itemdb) {
								case dbCreature:	// 4
									{
										Creature* xCreature = xGenericItem->castToCreature();	// _bp08

										if (xCreature->b15_2_2() != 0)
											goto _1314;
										if (xCreature->possession != OBJECT_END_MARKER)	// 0xFFFE
											goto _1314;
										if (itemtype != 0xff) {
											if (xCreature->CreatureType() != itemtype)
												goto _1314;
										}
								...
								case dbMissile:		//14
									{
										Missile* xMissile = xGenericItem->castToMissile();	// _bp08

										if (itemtype != 0xff) {
											if (QUERY_CLS2_FROM_RECORD(xMissile->w2) != itemtype)
												goto _1314;
										}
										_075f_056c(si);
								...
								case dbCloth:		// 6
									{
										Cloth* xClothingItem = xGenericItem->castToCloth();	// _bp08

										if (xClothingItem->Important() != 0)
											goto _1314;
										U16 bp1e = xClothingItem->ItemType();
										if (bp1e == bp27)
											goto _1314;
								...
								case dbWeapon:		// 5
									{
										Weapon* xWeaponItem = xGenericItem->castToWeapon();	// _bp08

										if (xWeaponItem->Important() != 0)
											goto _1314;
										U16 bp1e = xWeaponItem->ItemType();
										if (bp1e == bp27)
											goto _1314;
										if (itemtype != 0xff && itemtype != bp1e)
											goto _1314;
								...
								case dbPotion:		// 8
									{
										Potion *_bp08 = xGenericItem->castToPotion();
								...
								case dbMiscellaneous_item:	//10
									{
										Miscellaneous_item* xMiscItem = xGenericItem->castToMisc();	// _bp08

										if (xMiscItem->Important() != 0)
											goto _1314;
										U16 bp1e = xMiscItem->ItemType();
										if (bp1e == bp27)
											goto _1314;
										if (itemtype != 0xFF && itemtype != bp1e)
											goto _1314;
							...
							Creature* xCreature = GET_ADDRESS_OF_RECORD(si)->castToCreature();	// bp08
							if (xCreature->possession == OBJECT_END_MARKER) {
								goto _1314;
							}
							if (QUERY_CREATURE_AI_SPEC_FROM_TYPE(xCreature->CreatureType())->IsStaticObject() == 0) {
								goto _1314;
...
//^0CEE:1409
ObjectID SkWinCore::ALLOC_NEW_RECORD(U16 iItemDatabase)
{
	SkD((DLV_DBM, "DBM: ALLOC_NEW_RECORD(%2d)\n", (Bitu)iItemDatabase));

	U16 iLocalItemDatabase = iItemDatabase;	// si
	ObjectID oidAllocatedObject = OBJECT_NULL;	// di
	U16 bp08 = dunHeader->nRecords[(iLocalItemDatabase & 0x7FFF)];	// bp08
	if (iLocalItemDatabase == 0x800A) { // 0x8000 is for bone?
		iLocalItemDatabase = 0x000A;
	}
	else {
		if (iLocalItemDatabase == 0x000A) {
			bp08 -= 3;
		}
	}
	U16 bp06 = bp08;
	U16 iObjectSize = glbItemSizePerDB[iLocalItemDatabase];		// bp0a
	U8* xObjectData = glbDBObjectData[iLocalItemDatabase];	// bp04

	while (true) {
		if (reinterpret_cast<GenericRecord *>(xObjectData)->w0 == OBJECT_NULL) {
			oidAllocatedObject = ObjectID(0, iLocalItemDatabase, bp08 - bp06);
		}
		else {
			if (bp06-- != 0) {
				xObjectData += (iObjectSize >> 1) << 1;
				continue;
			}
			else {
				oidAllocatedObject = RECYCLE_A_RECORD_FROM_THE_WORLD(iLocalItemDatabase, 0xFF);
				if (oidAllocatedObject == OBJECT_NULL) {
					return OBJECT_NULL;
				}
				xObjectData = reinterpret_cast<U8 *>(GET_ADDRESS_OF_RECORD(oidAllocatedObject));
			}
		}
		ZERO_MEMORY(xObjectData, iObjectSize);
		reinterpret_cast<GenericRecord *>(xObjectData)->w0 = OBJECT_END_MARKER;
		if (iLocalItemDatabase == dbContainer) {
			reinterpret_cast<Container *>(xObjectData)->w2 = OBJECT_END_MARKER;
		}
		return oidAllocatedObject;
	}
}
...
void SkWinCore::INIT_FREE_POOL(sk5d00 *ref, i32 size, U16 poolflag)
{
	...
	ref->w16 = poolflag;
	if (ref->Is4EMS()) {
		glbShelfMemTopEMS = CONVERT_PHYS_TO_SHELF_FORM(reinterpret_cast<U8 *>(ref));
...
//^3E74:059E
// SPX: _3e74_059e renamed TAG_LARGEST_FREE_POOL
sk5d00 *SkWinCore::TAG_LARGEST_FREE_POOL(X16 mask)
{
	ENTER(12);
	X16 iPoolMask = mask;	// si
	sk5d00 *bp08 = NULL;
	U32 bp0c = 0;
	...
	do {
		if (bp04->w18 != 0)
			continue;
		if ((bp04->w16 & iPoolMask) != iPoolMask)
			continue;
		if ((bp04->w16 & 0x800) != (iPoolMask & 0x800))
			continue;
		if (bp04->dw4 <= bp0c)
			continue;
		bp08 = bp04;
...
U16 SkWinCore::ADD_MEM_TO_FREE_POOL(sk5d00 *pool, i32 size, X16 poolflag)
{
	...
	//^3E74:11C0
	_4976_5c7a = 1;
	sk5d00 *bp04 = TAG_LARGEST_FREE_POOL(0x7f8);	// 0x7f8 = 2040
	if (bp04 == NULL) {
		//^3E74:11DF
	...
	//^3E74:11E7
	glbCpxHeapCurrentMementPtr = _4976_5ce6 = reinterpret_cast<mement *>(&bp04[+1]);
	_4976_5cf0 = _4976_5cf4 = PTR_PADD(bp04,+bp04->dw4 + sizeof(sk5d00));
	glbFreeRAMMemPool = bp04->dw4;
	_4976_5cee = bp04->w16;
	//^3E74:124C
	bp04 = TAG_LARGEST_FREE_POOL(0xc00);
	if (bp04 != NULL || (bp04 = TAG_LARGEST_FREE_POOL(0x400)) != NULL) {
		//^3E74:127E
		_4976_5d6e = bp04->w16;
...
//LOGX(("LOAD_GDAT_RAW_DATA call from LOAD_ENT1"));
	...
	/// SPX: Special code for DM2 beta
	if (glbGDatNumberOfRawEntries == 0x8F29 && glbGDatNumberOfData == 0xF7D)
		SkCodeParam::bDM2BetaGDATDetected = true;
...
void SkWinCore::DEBUG_DISPLAY_GDAT_MAIN_INFO()
{
	...
	for (iCategoryIndex = 0; iCategoryIndex < GDAT_CATEGORY_MAX; iCategoryIndex++)
	{
		...
		iMaxItemsForCategory = GDAT_GET_ENTRIES_NUMBER(iCategoryIndex, 0x0B);
...
void SkWinCore::READ_GRAPHICS_STRUCTURE()
{
	...
	U16 *pTableDataSizes;	// bp04
	X32 iTableDataSizesSize;	// bp08	, the size of the TableDataSizes .. 2 bytes (U16) for each raw data item
	...
	iTableDataSizesSize = U32(glbGDatNumberOfData) << 1;	// nb items * 2 = nb of bytes for all item sizes
	pTableDataSizes = reinterpret_cast<U16 *>(ALLOC_MEMORY_RAM(iTableDataSizesSize, afDefault, 0x400));
	glbGDatCursorDataCumulatedLength = iTableDataSizesSize + 4;	// nb items sizes + (magic number + nb items) = offset of first raw data which must be the ENT1 (4 bytes after the magic number + nb items)
	if (glbGDATVersion < 3) {
		if (READ_FILE(glbFileHandleGraphics1, iTableDataSizesSize, pTableDataSizes) == 0)
			goto _read_graphics_structure__raise_error;

		glbGDat1stEntRawDatSize = *pTableDataSizes;
		...
		if (READ_FILE(glbFileHandleGraphics1, 4, &glbGDat1stEntRawDatSize) == 0)	// Read the next 4 bytes of GDAT which hold the size for the first item entry which must be the ENT1 item
			goto _read_graphics_structure__raise_error;
		if (SkCodeParam::bUseBigEnd)
			glbGDat1stEntRawDatSize = SWAP32(glbGDat1stEntRawDatSize);
		...
		if (READ_FILE(glbFileHandleGraphics1, iTableDataSizesSize - 2, &pTableDataSizes[1]) == 0)	// here read the size table before the ENT1 item (except the first item already read, which is exceptionnally on 4 bytes; all others are on 2 bytes max)
			goto _read_graphics_structure__raise_error;
		if (SkCodeParam::bUseBigEnd)
			SWAPWPX(&pTableDataSizes[1], iTableDataSizesSize - 2);
	}
	pTableDataSizes[0] = 0;
	glbSizeOfGDATFull = glbGDatCursorDataCumulatedLength;
	U16 iShelfIndex;	// si
	// Init the ShelfMem Table with all sizes for each raw data.
	for (iShelfIndex = 0; iShelfIndex < glbGDatNumberOfData; pTableDataSizes++, iShelfIndex++) {
		glbSizeOfGDATFull += *pTableDataSizes;
		glbShelfMemoryTable[iShelfIndex] = shelf_memory::FromSizeOnUnloaded(*pTableDataSizes);
		//printf("Shelf memory %05d: size = %06d\n", si, glbShelfMemoryTable[si].val & 0x7fff);
	}
	glbSizeOfGDATFile = GET_FILE_SIZE(glbFileHandleGraphics1);
	if (glbSizeOfGDATFile < glbSizeOfGDATFull) {	// if size of this GDAT is < theorical size of GDAT, then we would need a second part of GDAT file
		glbGDatFloppyFlag = 1;
		glbFloppyDiskFlag = 1;
	}
	DEALLOC_UPPER_MEMORY(iTableDataSizesSize);
	LOAD_ENT1();
...
//DEBUG_DISPLAY_GDAT_MAIN_INFO();
	...
	LOAD_GLOBAL_TABLE_RAWDAT_SOUNDS();	// SPX: quick test, invert this and glbGDatStructureRead after it for some Beta test; as it seem QUERY_GDAT_ENTRYPTR works differently before and after init
...
void SkWinCore::LOAD_GDAT_INTERFACE_00_0A()
{
	...
	iItemSize = QUERY_GDAT_ENTRY_DATA_LENGTH(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x00_BASE_DATA, dt07, 0x0A);
	if (iItemSize <= 0) {
	...
	tblCreatureFrameInfo14 = reinterpret_cast<U8 (*)[14]>(ALLOC_MEMORY_RAM(bp04 = QUERY_GDAT_ENTRY_DATA_LENGTH(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x00_BASE_DATA, dt07, 0x0A), afUseUpper, 0x400));
	LOAD_GDAT_ENTRY_DATA_TO(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x00_BASE_DATA, dt07, 0x0A, reinterpret_cast<U8 *>(tblCreatureFrameInfo14));
	}
	/*
	if (SkCodeParam::bUsePowerDebug) {
...
{
	ENTER(6);
	U8* xMemRawItemData = ALLOC_MEMORY_RAM(QUERY_GDAT_ENTRY_DATA_LENGTH(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x00_BASE_DATA, dt07, 0x2), afUseUpper, 0x400);	// bp04
	// SPX: this item is x411 bytes long (1041) and looks like several parts
	// x00 - x10 : 17 * value x10	// 1st value (16) is for number of sk4bde structures, the 16 next values feed b0 for each item of these sk4bde structures.
	//		these values (16) tell the step for each item on the next table parts to read
	// x11 - x110 : 256 values, 16 * 16
	// x111 - x210 : 256 values, litterally 0 to 255
	// x211 - x410 : 512 bytes, 256 * 2, again 0 to 255 but with split nibbles.
	LOAD_GDAT_ENTRY_DATA_TO(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x00_BASE_DATA, dt07, 0x2, xMemRawItemData);
	X8 iNumberOfRows = *xMemRawItemData++;	// this would get value 16, the number of sk4bde structures	// bp05
	//_4976_4bde = reinterpret_cast<sk4bde *>(ALLOC_MEMORY_RAM(bp05 * 9UL, afUseUpper, 0x400));
	tblOfsk4bdePalette = reinterpret_cast<sk4bdePalette*>(ALLOC_MEMORY_RAM(iNumberOfRows * sizeof(sk4bdePalette), afUseUpper, 0x400));
	i16 iCurIdx;	// si
	for (iCurIdx = 0; iNumberOfRows > iCurIdx; xMemRawItemData++, iCurIdx++) {	// read byte by byte
		tblOfsk4bdePalette[iCurIdx].iRowSize = *xMemRawItemData;
	}
	for (iCurIdx = 0; iNumberOfRows > iCurIdx; iCurIdx++) {
		tblOfsk4bdePalette[iCurIdx].pTabRow1 = xMemRawItemData;
		xMemRawItemData += tblOfsk4bdePalette[iCurIdx].iRowSize;
	}
	for (iCurIdx = 0; iNumberOfRows > iCurIdx; iCurIdx++) {
		tblOfsk4bdePalette[iCurIdx].pTabRow2 = xMemRawItemData;
		xMemRawItemData += tblOfsk4bdePalette[iCurIdx].iRowSize;
	}
	tblOfsk4be2Palette = reinterpret_cast<sk4be2Palette*>(xMemRawItemData);
	return;
}

// SPX: Added LOAD_CD_DAT_FILE to handle CD.DAT file and musics triggered on specific location.
void SkWinCore::LOAD_CD_DAT_FILE()
{
	void* pDataTable = cd.sc.glbTabCDMusicTriggers;
	i16 hCDFileHandle = -1;
	hCDFileHandle = FILE_OPEN((const U8*)"CD.DAT");
...
void SkWinCore::LOAD_GDAT_INTERFACE_00_00()
{
	...
	iItemSize = QUERY_GDAT_ENTRY_DATA_LENGTH(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x00_BASE_DATA, dt06, 0x0);
	if (iItemSize <= 0) {
		...
		pDataTable = ALLOC_MEMORY_RAM(
			iItemSize,
			afUseUpper, 0x400);
		LOAD_GDAT_ENTRY_DATA_TO(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x00_BASE_DATA, dt06, 0x0, pDataTable);
	}
...
void SkWinCore::_38c8_00c8_ALLOC_PICT()
{
	...
	if (glbBackBuffViewport == NULL) {
		glbBackBuffViewport = ALLOC_PICT_BUFF(glbViewportWidth, glbViewportHeight, afUseUpper, IMG_8_BPP);
	}
	_4976_5ca0 = 0xB798;
	return;
}
...
//^3E74:2439
// _3e74_2439 renamed GDAT_GET_ENTRIES_NUMBER
U8 SkWinCore::GDAT_GET_ENTRIES_NUMBER(X8 iGDatCategory, X8 iGDatEntryId)
{
...
void SkWinCore::SHOW_MENU_SCREEN()
{
	...
	if (SkCodeParam::bDM2CDMusic && !SkCodeParam::bDM2V5Mode)
		REQUEST_PLAY_MUSIC(5, 50);
		//REQUEST_PLAY_CD_MUSIC(5, 100);	// SPX: CD title music
	else
		REQUEST_PLAY_MUSIC(100, 50);	// SPX: title music
	...
	if (QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x05_TITLE, 0x0, dt07, GDAT_IMG_x04_MAIN_MENU_SCREEN != 0xFFFF) && !SkCodeParam::bDM2BetaGDATDetected)	// 64000 bytes raw data for menu screen ?
	{
		...
		cd.mo.glbImageMenuScreen = QUERY_GDAT_IMAGE_ENTRY_BUFF(GDAT_CATEGORY_x05_TITLE, 0x0, GDAT_IMG_x04_MAIN_MENU_SCREEN);
/*		_4976_52ba = cd.mo.glbImageMenuScreen + (
		...
		glbMenuScreenLocalPal16 = cd.mo.glbImageMenuScreen + (
...
/*	_4976_52b6 = cd.mo.glbImageCreditScreen +((READ_UI16(cd.mo.glbImageCreditScreen,-6) == 4) // 4bits image, meaning 2 pixel per byte
	...
	glbCreditScreenLocalPal16 = cd.mo.glbImageCreditScreen +((READ_IMGBUFF_BPP(cd.mo.glbImageCreditScreen) == IMG_4_BPP) // 4bits image, meaning 2 pixel per byte
		...
		;
	// then glbCreditScreenLocalPal16 = p(cd.mo.glbImageCreditScreen) + 0x7D00 => end of this image
	GRAPHICS_DATA_CLOSE();
	_1031_0541(0);
	do {
...
//		_4976_5c7e_cache_ici = reinterpret_cast<U16 *>(ALLOC_MEMORY_RAM(glbCacheRecyclerMax << 1, afUseUpper, 0x400));
		_4976_5c7e_cache_ici = reinterpret_cast<U16 *>(ALLOC_MEMORY_RAM(glbCacheRecyclerMax * sizeof (U16), afUseUpper, 0x400));
//		_4976_5c86_cache_hash = reinterpret_cast<U32 *>(ALLOC_MEMORY_RAM(glbCacheRecyclerMax << 2, afZeroMem|afUseUpper, 0x400));
		_4976_5c86_cache_hash = reinterpret_cast<U32 *>(ALLOC_MEMORY_RAM(glbCacheRecyclerMax * sizeof (U32), afZeroMem|afUseUpper, 0x400));
		glbCpxHeapLimitPtr = _4976_5cf4 - _4976_00ec;
		SkD((DLV_DBG_INIT, "ALLOC_CPX_SETUP:INIT_CPXHEAP\n"));
		INIT_CPXHEAP(&glbTiamatPoolInfo1, ptr2t(glbCpxHeapLimitPtr), 0, _4976_5cee);
		if (glbFreeEMSMemPool < 12) {
			//^3E74:14E7
			INIT_CPXHEAP(&glbTiamatPoolInfo2, ptr2t(NULL), 0, _4976_5d6e);
		}
		else {
			INIT_CPXHEAP(&glbTiamatPoolInfo2, _4976_5d5a, glbFreeEMSMemPool, _4976_5d6e);
		}
...
#endif
		glbFreeCPXMemory = tiamat::Size(ptr2t(glbCpxHeapLimitPtr), ptr2t(reinterpret_cast<U8 *>(_4976_5ce6)));
		//^3E74:153C
...
UINT SkWinCore::SKLIB_INIT()
{
	U16 iCompatibilityFlag = 0;
	U16 iSpecialItem = 0;

	SKLIB_IBMIO_INIT();	// IBMIO must be called first to set up some callback functions used for timers/tick steps, etc ..., but it must not call the game INIT() from there.
	...
	cd.gg.glbCreaturesMaxCount = GDAT_GET_ENTRIES_NUMBER(GDAT_CATEGORY_x0F_CREATURES, fmtWordVal);	// 0xf, 0xb
	...
	printf("SK-INIT COMPLETE\n");

	return 0;
}

// SPX Special lib callable GAME LOAD
UINT SkWinCore::SKLIB_GAMELOAD()
{
	CHANGE_CONSOLE_COLOR(BRIGHT, LIGHT_YELLOW, BLACK);
	printf("SK GAME LOAD\n");
	CHANGE_CONSOLE_COLOR(BRIGHT, LIGHT_GRAY, BLACK);
...
UINT SkWinCore::INIT()
{
	...
	LOAD_GDAT_INTERFACE_00_0A(); // game will fail if this item is not loaded, but it does not exist in PC-DOS version
	U8 *pPaletteBuffer = ALLOC_MEMORY_RAM(0x400, afUseLower, 1024);	// pb04	// 0x400 = 256 * 4
//DEBUG_DUMP_ULP();
...
void SkWinCore::DRAW_MESSAGE_HINT_TEXTS()
{
	...
	for (iMessageNo = 0; iMessageNo < 1; iMessageNo++) {
		...
		X16 bp06 = _4976_0130;
		FIRE_HIDE_MOUSE_CURSOR();
		FILL_RECT_ANY(&sRect, glbPaletteT16[C00_COLOR_BLACK]);	// SPX: Fill low text panel with BLACK
		FIRE_SHOW_MOUSE_CURSOR();
...
//^0CEE:04E5
// SPX: _0cee_04e5 renamed GET_TILE_FROM_VECTOR_W_DIR
X16 SkWinCore::GET_TILE_FROM_VECTOR_W_DIR(X16 xx, X16 yy, X16 zz, i16 ss, i16 tt)
{
	ENTER(0);
...
void SkWinCore::_2759_12e6()
{
	...
	for (; si < cd.pi.glbChampionsCount; si++) {
		...
		for (; di <= 1; di++) {
			...
			if (bp04->w0 == 0)
				continue;
			//^2759:1323
			Champion *bp08 = &tblChampionSquad[si];
	...
	if (si != 0 && bp0a >= 0 && bp0a <= 1) {
		//^2759:13CF
		si--;
		IS_ITEM_HAND_ACTIVABLE(si, tblChampionSquad[si].Possess(glbSelectedHandAction), glbSelectedHandAction);
	}
	//^2759:13F1
	return;
}
...
//		sprintf((char*)sMessage, "RAM = %08d / EMS = %08d\n", glbFreeRAMMemPool, glbFreeEMSMemPool);
		...
		DISPLAY_HINT_TEXT(C11_COLOR_YELLOW, sMessage);
		CHANGE_CONSOLE_COLOR(BRIGHT, LIGHT_YELLOW, BLACK);
...
_01f7:
		...
		do {
			...
			if (_4976_4c3e != 0) {
				X16 di;
				di = GET_TILE_FROM_VECTOR_W_DIR(cd.pi.glbPlayerDir, 1, 0, cd.pi.glbPlayerPosX, cd.pi.glbPlayerPosY);
...
//^13AE:005C
UINT SkWinCore::SKLIB_GAME_STEP()
{
	int iLoopCount = 0;
/*
	{
...
//
	...
	_4976_013e = 320;
	_4976_0140 = 15;
	glbRectRingBufferIndex = 0; // temp index always 0 (tblRectRingBuffer)
	zeroMem(&glb_4976_4bca_Rect, sizeof(glb_4976_4bca_Rect));
	_4976_4bc8 = 0;
	glbRectX_0106 = 16; // cx
	glbRectY_0108 = 16; // cy
	...
	glbGameTickHoldFromMement = -1;
	_4976_5d76 = 0; //?
	gblLinkedImageRoot.pNextImage = NULL; //?
	glbFreeCPXMemory = 0;
...
//	glbMap_4976_4c12 = 0;
	...
	zeroMem(&glbTiamatPoolInfo1, sizeof(glbTiamatPoolInfo1));
	zeroMem(&glbTiamatPoolInfo2, sizeof(glbTiamatPoolInfo2));
	...
	glbChampionColor[0] = C07_COLOR_LIGHT_GREEN;
	glbChampionColor[1] = C11_COLOR_YELLOW;
	glbChampionColor[2] = C08_COLOR_RED;
	glbChampionColor[3] = C14_COLOR_BLUE;
	...
	glbRectFullScreen.x = 0; glbRectFullScreen.y = 0; glbRectFullScreen.cx = 320; glbRectFullScreen.y = 200;
	tickThen = -1;
}

void SkWinCore::SKExt_FORCE_RENDER_SCREEN()
{
