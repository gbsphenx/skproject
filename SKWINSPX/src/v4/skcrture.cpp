					...
					if ((bp0a & _4976_4eec) == bp0a) {
						...
						if (bp16 != 0 || si == 0x400) {
							...
							if (bp0e != _4976_581c) {
								...
								//^19F0:1CE7
								CHANGE_GLOBAL_CREATURE_POS(_4976_581c);
							}
							//^19F0:1CF1
...
U16 SkWinCore::CREATURE_WALK_NOW()
{
	...
	if (CREATURE_GO_THERE(bp04->b32|0x80, glbCreatureTimerGetX, glbCreatureTimerGetY, glbCreatureTempTargetX, glbCreatureTempTargetY, bp04->b27) != 0) {
		...
		CHANGE_GLOBAL_CREATURE_POS(glbSomeMap_4976_4ee7);
		if (si != 0) {
			//^1887:01DB
...
X16 SkWinCore::CREATURE_USES_LADDER_HOLE()
{
	ENTER(18);
	CreatureInfoData* xCreatureInfo = glbCurrentThinkingCreatureData;	// bp04
	if (CREATURE_GO_THERE(xCreatureInfo->b32|0x80, glbCreatureTimerGetX, glbCreatureTimerGetY, glbCreatureTempTargetX, glbCreatureTempTargetY, xCreatureInfo->b27) == 0)
		return 1;
	if ((tlbCreatureCommandsFlags[RCJ(MAX_CREATURE_COMMANDS,xCreatureInfo->Command)] & 4) == 0)
		return 1;
	X16 si = glbCurrentThinkingCreatureRec->b15_0_1();
	glbCreatureTempTargetX = xCreatureInfo->w24.GetX();
	glbCreatureTempTargetY = xCreatureInfo->w24.GetY();
	if (xCreatureInfo->Command == ccm39 || xCreatureInfo->Command == ccm3A) {
		Actuator *bp0e;
		i16 di;
		si = FIND_LADDER_AROUND(glbCreatureTimerGetX, glbCreatureTimerGetY, di = (xCreatureInfo->Command == ccm39) ? -1 : 1, reinterpret_cast<Actuator **>(&bp0e));
		if (bp0e != NULL) {
	...
	CHANGE_GLOBAL_CREATURE_POS(xCreatureInfo->w24.GetMap());
	if (xCreatureInfo->Command == ccm35 || xCreatureInfo->Command == ccm36) {
	...
	CHANGE_GLOBAL_CREATURE_POS(glbSomeMap_4976_4ee7 = (U8)_4976_581c);
	if (bp06 != 0) {
		OPERATE_PIT_TELE_TILE(glbCreatureTimerGetX, glbCreatureTimerGetY, 0);
	}
	if (_4976_514e.b1 > 0)
...
void SkWinCore::THINK_CREATURE(X8 xx, X8 yy, X16 timerType)
{
	...
	// SPX: protection
	if (!CheckSafePointer(xCreatureData))
		return;
...
void SkWinCore::ANIMATE_CREATURE(X16 iXPos, X16 iYPos, X16 ww)
{
	ENTER(4);
	//X16 iYPos = yy;	// di
	ObjectID oCreature = GET_CREATURE_AT(iXPos, iYPos);	// si
	if (oCreature == OBJECT_NULL)
		return;
	if ((QUERY_CREATURE_AI_SPEC_FLAGS(oCreature)&1) != 0 && GET_ADDRESS_OF_RECORD4(oCreature)->InternalID() == 0xFF) {
		ALLOC_CAII_TO_CREATURE(oCreature, iXPos, iYPos);
	}
	U8* bp04 = PREPARE_LOCAL_CREATURE_VAR(oCreature, iXPos, iYPos, 0x21);	// bp04
...
//^1C9A:121D
ObjectID SkWinCore::ALLOC_NEW_CREATURE(U16 creaturetype, U16 healthMultiplier_1to31_baseIs8, U16 dir, U16 iXPos, U16 iYPos)
{
	...
	ENTER(12);
	U16 iCreatureHP = healthMultiplier_1to31_baseIs8;	// si
	...
	Creature* xCreature = GET_ADDRESS_OF_RECORD4(oCreature);	//*bp04
	...
	iCreatureHP = min_value(iCreatureHP, 31);
	// SPX: bp08->w4 = Hit Points
	iCreatureHP = (iCreatureHP * bp08->BaseHP) >> 3;

	xCreature->HP1(RAND16((iCreatureHP >> 3) + 1) + iCreatureHP);
	xCreature->HP3(0);
	xCreature->SetTriggerXPos(iXPos);
	xCreature->SetTriggerYPos(iYPos);
	xCreature->SetTriggerMap(glbCurrentMapIndex);
	xCreature->iAnimSeq = 0xFFFF;
	if (MOVE_RECORD_TO(oCreature, -4, 0, iXPos, iYPos) == 1) {
		return OBJECT_NULL;
	}
