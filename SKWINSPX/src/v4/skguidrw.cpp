//------------------------------------------------------------------------------
// SK-GUI-DRAW
//	Draw functions for Graphical UI
//------------------------------------------------------------------------------
...
//	DEBUG_SHOW_ICON_PICT_BUFF(buff, tt, rc, srcx, srcy, colorkey, flipmirror, localpal);
	ENTER(8);
	if (rc != NULL) {
		SRECT bp08;
		FIRE_BLIT_PICTURE(
			buff,
			QUERY_MEMENT_BUFF_FROM_CACHE_INDEX(tt->iCacheIndex0),
			OFFSET_RECT(tt, rc, &bp08),
			srcx,
			srcy,
			READ_IMGBUFF_WIDTH(buff),	// READ_UI16(buff,-4)
			tt->rc2.cx,
			colorkey,
			flipmirror,
			READ_IMGBUFF_BPP(buff),	// READ_UI16(buff,-6)
			IMG_8_BPP,
			localpal
			);
		SOME_RECT_0b36_0d67(tt, rc);
	}
	return;
}
...
{
	ENTER(20);
	printf("DRAW_ICON_PICT_ENTRY for iRectNo = %d\n");
	i16 iSourceY = 0; // bp0c
	i16 iSourceX = 0; // bp0a
...
void SkWinCore::SHOW_ATTACK_RESULT(i16 yourValue)
{
	...
	DRAW_SOME_PICTURE_0b36_11c0(QUERY_PICST_IT(&bp0144), &glbScreenBufferRightPanel, 57, -1);
	if (di >= 0) {
#if UseAltic
		...
		si--;
		bp0a[si] = 0x02;
		DRAW_SIMPLE_STR(&glbScreenBufferRightPanel, 0x39, glbPaletteT16[C04_COLOR_CYAN], glbPaletteT16[C00_COLOR_BLACK], &bp0a[si]);
	}
	//else if (di == -2) {
	else if (di == ATTACK_REQUIRES_HAND_ITEM) {
		DRAW_ICON_PICT_ENTRY(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0x00, (glbWeaponMissileHand == 0) ? GDAT_INTERFACE_BODY_HAND_RIGHT : GDAT_INTERFACE_BODY_HAND_LEFT, &glbScreenBufferRightPanel, 126, -1);
		bp01 = 0;
		// SPX: Search through all weapons to find which projectile fits the current shooter item
		for (; bp01 < 255; bp01++) {
			U16 si = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x10_WEAPONS, bp01, dtWordValue, GDAT_ITEM_WEAPON_PROJECTILE_FLAG); // 0x05
			...
			if (si != 0 && (si & WEAPON_FLAGS_SHOOTER) == 0 && (glbWeaponShooterNum & si) != 0) {
				DRAW_ICON_PICT_ENTRY(GDAT_CATEGORY_x10_WEAPONS, bp01, 0x18, &glbScreenBufferRightPanel, 125, -1);
				return;
			}
		}
	}
...
U8 *SkWinCore::DRAW_ITEM_ON_WOOD_PANEL(U16 iChampionIndex, U16 possessionIndex, Picture *ref)
{
	...
	if (bp12 != 0) {
		DRAW_DIALOGUE_PICT(
			...
			bp04,
			ALLOC_TEMP_ORIGIN_RECT(xExtPict.iWidth, xExtPict.iHeight),
			0,
			0,
			12,
			xExtPict.iPal256
...
void SkWinCore::DRAW_HAND_ACTION_ICONS(U16 iChampionIndex, U16 iChampionHand, U16 iActivatedHand)
{
	ENTER(48);
	SOMETHING_RIGHT_PANEL_29ee_00a3(0);
	Champion* xChampion = &tblChampionSquad[iChampionIndex];
	U16 iRectNo = (iChampionHand == 1) ? RECTZONE_070_RIGHT_PANEL_CHAMPION_1_HAND_1 : RECTZONE_074_RIGHT_PANEL_CHAMPION_1_HAND_0;	// si
	iRectNo += (xChampion->playerPos() + 4 - cd.pi.glbPlayerDir) & 3;
	SRECT xRect1;	// bp10
	QUERY_EXPANDED_RECT(iRectNo, &xRect1);
	if (xChampion->curHP() == 0) {	// if the champion is dead or absent, display a black zone instead
		SRECT xRect1;	// bp10
		FILL_RECT_SUMMARY(&glbScreenBufferRightPanel, &xRect1, glbPaletteT16[C00_COLOR_BLACK]);
	}
	else {
		...
		if (xImgBuffer != NULL) {
			...
			DRAW_ICON_PICT_BUFF(
				...
				//CALC_CENTERED_RECT_IN_RECT(&xRect2, &xRect1, READ_UI16(xImgBuffer,-4), READ_UI16(xImgBuffer,-2)),
				CALC_CENTERED_RECT_IN_RECT(&xRect2, &xRect1, READ_IMGBUFF_WIDTH(xImgBuffer), READ_IMGBUFF_HEIGHT(xImgBuffer)),
				0,
				0,
				glbPaletteT16[C08_COLOR_RED],
				0,
				NULL
				);
			FREE_PICT_MEMENT(&xPicture);
		}
		if (xChampion->handCooldown[iChampionHand] != 0 || cd.pi.glbIsPlayerSleeping != 0) {
			DRAW_GRAY_OVERLAY(&glbScreenBufferRightPanel, &xRect1, 0);
		}
	}
	return;
}
...
void SkWinCore::DRAW_SQUAD_SPELL_AND_LEADER_ICON(U16 player, U16 yy)
{
    ENTER(324);
	SOMETHING_RIGHT_PANEL_29ee_00a3(0);
	U16 si = (tblChampionSquad[player].playerPos() +4 - cd.pi.glbPlayerDir) & 3;
	SRECT bp0a;
	FILL_RECT_SUMMARY(
		&glbScreenBufferRightPanel,
	...
	if (tblChampionSquad[player].curHP() != 0) {
		...
		DRAW_SOME_PICTURE_0b36_11c0(
			QUERY_PICST_IT(&bp0144),
			&glbScreenBufferRightPanel,
			si + 0x57,
			4
			);
		if (cd.pi.glbIsPlayerSleeping != 0 || tblChampionSquad[player].herob44 != 0) {
			bp0144.rc36.x += glbScreenBufferRightPanel.rc2.x;
			bp0144.rc36.y += glbScreenBufferRightPanel.rc2.y;
			DRAW_GRAY_OVERLAY(&glbScreenBufferRightPanel, &bp0144.rc36, 0);
		}
...
// Draws into the right panel: the 4 squares for each champions, then draw each colored champion + aura
void SkWinCore::DRAW_SQUAD_POS_INTERFACE()
{
	ENTER(350);
	SOMETHING_RIGHT_PANEL_29ee_00a3(0);
	DRAW_ICON_PICT_ENTRY(GDAT_CATEGORY_x08_GRAPHICSSET, glbSquadInterfaceMapGfxSet, GDAT_GFXSET_xF5_SQUAD_4X, &glbScreenBufferRightPanel, RECTZONE_047_RIGHT_PANEL_CHAMPION__FULL_ZONE, -1);	// gfx = 0xF5
	U8 *pPictBuff = ALLOC_PICT_BUFF(glbPictIconBufferX, glbPictIconBufferY, afDefault, IMG_4_BPP);	// bp04
	SRECT iRectZoneChampion;	// bp14
	for (U16 iChampionIdx = 0; iChampionIdx < cd.pi.glbChampionsCount; iChampionIdx++) {	// si
		U16 bp06 = (tblChampionSquad[iChampionIdx].playerPos() + 4 - cd.pi.glbPlayerDir) & 3;
		if (tblChampionSquad[iChampionIdx].curHP() == 0 || bp06 +1 == _4976_5dbc)
			continue;
		i16 bp0c = 0;
		i16 bp0a = 0;
		U16 bp08;
		if (QUERY_BLIT_RECT(pPictBuff, &iRectZoneChampion, bp08 = bp06 + 53, &bp0a, &bp0c, -1) == NULL)
			continue;
		if (glbGlobalSpellEffects.AuraOfSpeed != 0) {	// with speed, the champions silhouette will shake
			i16 iRandomDisplacement = RAND02();	// di
			if (iRandomDisplacement != 0) {
				iRectZoneChampion.y += iRandomDisplacement -2;
				iRandomDisplacement = RAND02();
				if (iRandomDisplacement != 0) {
					iRectZoneChampion.x += iRandomDisplacement -2;
				}
			}
		}
		...
		if (tblChampionSquad[iChampionIdx].enchantmentPower == 0)
			continue;
		ExtendedPicture xExtPicture;	//	bp014e
		// SPX: Check in INTERFACE (0x01) 07 => Party protection animations
		QUERY_PICST_IT(QUERY_GDAT_SUMMARY_IMAGE(&xExtPicture, GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x07_CHAMPION_AURA, tblChampionSquad[iChampionIdx].enchantmentAura));
		xExtPicture.w4 |= 0x10;
		xExtPicture.iWidth >>= 2;
		xExtPicture.w14 = xExtPicture.iWidth * ((tblChampionSquad[iChampionIdx].playerDir() + 4 - cd.pi.glbPlayerDir) & 3);
		xExtPicture.iHeight >>= 2;
		xExtPicture.w16 = xExtPicture.iHeight * (i16(glbGameTick) & 3);
		DRAW_SOME_PICTURE_0b36_11c0(&xExtPicture, &glbScreenBufferRightPanel, bp08, 12);
	}
	FREE_PICT_BUFF(pPictBuff);
	if (cd.pi.glbIsPlayerSleeping != 0) {
		DRAW_GRAY_OVERLAY(
			&glbScreenBufferRightPanel, 
			QUERY_EXPANDED_RECT(RECTZONE_047_RIGHT_PANEL_CHAMPION__FULL_ZONE, &iRectZoneChampion),
			0
			);
	}
	return;
...
void SkWinCore::DRAW_STRONG_TEXT(U8 *buff, U16 ww, U16 cx, U16 xx, U16 yy, U16 clr1, U16 fill, U8 *str)
{
	...
	if (QUERY_STR_METRICS(str, &bp02, &bp04) != 0) {
		...
		DRAW_STRING(buff, ww, cx, di   , si +1, glbPaletteT16[C00_COLOR_BLACK], glbPaletteT16[C15_COLOR_WHITE] | 0x4000, str, 8); // shadow text, black
		DRAW_STRING(buff, ww, cx, di +1, si +1, glbPaletteT16[C00_COLOR_BLACK], glbPaletteT16[C15_COLOR_WHITE] | 0x4000, str, 8);	// shadow text, black
...
{
	ENTER(0);
	DRAW_ICON_PICT_ENTRY(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x04_CHAMPION, 0x14, &glbScreenBufferRightPanel, 60, -1);
	DRAW_ICON_PICT_ENTRY(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x04_CHAMPION, 0x0E, &glbScreenBufferRightPanel, 59, -1);
	DRAW_NAME_STR(
		&glbScreenBufferRightPanel, 
		61, 
		(cd.pi.glbChampionIndex - 1 == glbChampionLeader) ? glbPaletteT16[C09_COLOR_ORANGE] : glbPaletteT16[C15_COLOR_WHITE], 
...
//^0CAF:0008
i16 SkWinCore::SK_STRLEN(const U8 *pszString)
{
	ENTER(4);
	const U8 *pszBegin = pszString;	// bp04
	while (*(pszString++) != 0);
	return U16(pszString - pszBegin - 1);
}
...
void SkWinCore::DRAW_SPELL_TO_BE_CAST(U16 xx)
{
	...
	if (xx != 0) {
		DRAW_ICON_PICT_ENTRY(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x05_SPELLMENU, 0x09, &glbScreenBufferRightPanel, 252, -1);
	}
	Champion *pChampion = &glbChampionTable[cd.pi.glbChampionIndex];
	U8 szRune[2];	// bp08
    szRune[1] = 0;
	U16 iRunesLen = SK_STRLEN(pChampion->GetRunes());	// bp06
	U16 di = RECTZONE_261_SPELL_RUNE_1;	// 261 rect zone
	for (U16 iRuneIdx = 0; iRuneIdx < iRunesLen; iRuneIdx++) {
		szRune[0] = pChampion->GetRunes()[iRuneIdx];
		// SPX: Spell runes color
		DRAW_SIMPLE_STR(&glbScreenBufferRightPanel, di++, glbPaletteT16[C00_COLOR_BLACK], glbPaletteT16[C13_COLOR_LIGHTER_GRAY] | 0x4000, szRune);
	}
	return;
}
...
void SkWinCore::DRAW_SPELL_PANEL()
{
	ENTER(10);
	Champion *pChampion = &glbChampionTable[cd.pi.glbChampionIndex];
	U16 bp06 = pChampion->runesCount;
	// SPX: rune class interface
	DRAW_ICON_PICT_ENTRY(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x05_SPELLMENU, bp06 +1, &glbScreenBufferRightPanel, 92, -1);
	if (bp06 < 4) {
		U8 bp0a[2];
		bp0a[1] = 0;
		...
		for (bp06 = 0; bp06 < 6; bp06++) {
			...
			DRAW_SIMPLE_STR(
				...
				si++, 
				glbPaletteT16[C00_COLOR_BLACK],
				glbPaletteT16[C13_COLOR_LIGHTER_GRAY] | 0x4000,
				bp0a
				);
		}
	}
...
void SkWinCore::DRAW_CMD_SLOT(U16 cmdSlot, U8 ww)
{
	...
	if (glbMagicalMapFlags != 0) {
		DRAW_ICON_PICT_ENTRY(GDAT_CATEGORY_x14_CONTAINERS, glbHoldedContainerType, ((glbItemSelected[si].entry +0xf8) << 1) +ww +0x41, &glbScreenBufferRightPanel, si +110, -1);
	}
	else {
		DRAW_ICON_PICT_ENTRY(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x04_CHAMPION, ww +0x15, &glbScreenBufferRightPanel, si +63, -1);
		DRAW_NAME_STR(
			&glbScreenBufferRightPanel,
			si +66,
...
//^29EE:1D03
// _29ee_1d03 renamed DRAW_SOME_MAGICAL_MAP_STRING_29ee_1d03
void SkWinCore::DRAW_SOME_MAGICAL_MAP_STRING_29ee_1d03(U16 xx)
{
	ENTER(6);
	U16 si = xx;
	SOMETHING_RIGHT_PANEL_29ee_00a3(0);
	if (cd.pi.glbChampionIndex > 0 && (glbMagicalMapFlags & 0x0800) != 0) {
		...
		for (; bp02 < 4; bp02++) {
			...
			si >>= 1;
			DRAW_ICON_PICT_ENTRY(GDAT_CATEGORY_x14_MAGICAL_MAPS, glbHoldedContainerType, bp03, &glbScreenBufferRightPanel, bp02 +101, -1);
			bp06[0] = bp02 +0x72;
			DRAW_SIMPLE_STR(&glbScreenBufferRightPanel, bp02 +105, glbPaletteT16[C00_COLOR_BLACK], glbPaletteT16[C02_COLOR_GRAY] | 0x4000, bp06);
		}
	}
	return;
}
...
void SkWinCore::DRAW_MAP_CHIP(ObjectID recordLink, i16 ss, i16 tt, i16 ww, U16 xx, U16 yy, U8 cc, U16 flags)
{
	...
	//^29EE:0D2A
	U8 bp5c[16];
	QUERY_DUNGEON_MAP_CHIP_PICT(GDAT_CATEGORY_x08_GRAPHICSSET, cc, &bp2e, bp5c);
	//^29EE:0D41
	if ((flags & 0x0010) != 0) {
		//^29EE:0D48
		SRECT bp3c;
		...
		//^29EE:0D91
		FILL_RECT_SUMMARY(&glbScreenBufferRightPanel, &bp3c, glbPaletteT16[C08_COLOR_RED]);
	}
	//^29EE:0DAD
		...
		case 7://^0DDF
			//^29EE:0DDF
			{
				...
				for (bp0a = 0; bp0a <= 3; bp0a++) {
					//^29EE:0E27
					bp0c = bp4c.tfoi[bp0a] & 0xFF;
					//^29EE:0E3A
					if (bp0c != 0xff) {
						//^29EE:0E41
						if ((flags & 0x0002) != 0 || QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x09_WALL_GFX, bp0c, dtWordValue, GDAT_IMG_MAP_CHIP) == 0) {
							//^29EE:0E5F
							bp0c = QUERY_DUNGEON_MAP_CHIP_PICT(GDAT_CATEGORY_x09_WALL_GFX, U8(bp0c), &bp2a, bp6c);
							//^29EE:0E79
							if (bp0c != 0) {
								//^29EE:0E7D
								bp0e = bp4c.tfoi[bp0a] >> 10;
								//^29EE:0E90
		...
		case 1://^0EE3
			{
				...
				if ((bp4c.w2 & 0x0008) != 0) {
					//^29EE:0EEA
					QUERY_DUNGEON_MAP_CHIP_PICT(GDAT_CATEGORY_x08_GRAPHICSSET, _4976_4c5c[dunMapLocalHeader->Level()], &bp2e, bp5c);
					//^29EE:0F12
					//^29EE:18AA
					DRAW_CHIP_OF_MAGIC_MAP(bp2e, 0, xx, di, 0, bp5c);
					//^29EE:18B7
...
_0f22:
				...
				//^29EE:0F3C
				bp0a = bp4c.tfoi[2] & 0xFF;
				//^29EE:0F45
				if (bp0a != 0xff) {
					//^29EE:0F49
					if (true
						&& ((flags & 0x0002) != 0 || QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0A_FLOOR_GFX, bp0a, dtWordValue, 0xf9) == 0)
						&& (bp0c = QUERY_DUNGEON_MAP_CHIP_PICT(GDAT_CATEGORY_x0A_FLOOR_GFX, U8(bp0a), &bp2a, bp6c)) != 0
					) {
						//^29EE:0F85
				...
				if (bp4c.w0 == 5) {
					//^29EE:0FCB
					bp0a = QUERY_DUNGEON_MAP_CHIP_PICT(GDAT_CATEGORY_x18_TELEPORTERS, 0x00, &bp2a, bp6c);
					//^29EE:0FE3
					if (bp0a != 0) {
						//^29EE:0FEA
		...
		case 17://^1020	// DOOR TILE
			{
				...
				//^29EE:1025
_1025:
				Door *bp04 = GET_ADDRESS_OF_RECORD0(bp4c.tfoi[1]);
				//^29EE:1034
...
_10b7:
	...
	if ((flags & 0x0010) != 0) {
		...
		for (bp0a = 0; bp0a < cd.pi.glbChampionsCount; bp0a++) {
			//^29EE:10E4
			if (tblChampionSquad[bp0a].curHP() != 0) {
				//^29EE:10F8
				...
				DRAW_CHIP_OF_MAGIC_MAP(
					...
					xx,
					di,
					_4976_3fa4[(tblChampionSquad[bp0a].playerPos() +4 -ww) & 3],
			...
			if (si.DBType() == dbMissile) {
				...
				if (bp0a != 0) {
					...
					if (bp0a > 3) {
						...
						bp0a = 3;
						//^29EE:1670
						switch (GET_MISSILE_FLYING_IMAGE_METHOD(si) +1) {
							case 0:
								//^29EE:1686
								//^29EE:16B0
								bp0a = 0;
...
void SkWinCore::DRAW_MAJIC_MAP(ObjectID recordLink)
{
	...
	if ((glbMagicalMapFlags & 0x0400) == 0) {
		DRAW_ICON_PICT_ENTRY(GDAT_CATEGORY_x14_MAGICAL_MAPS, glbHoldedContainerType, 0x10, &glbScreenBufferRightPanel, 92, -1);
		for (U16 si = 0; si < glbCommandSlotsNum; si++) {
			DRAW_CMD_SLOT(si, 0);
		}
		DRAW_SOME_MAGICAL_MAP_STRING_29ee_1d03(0);
		glbMagicalMapFlags |= 0x0400;
	}
	//^29EE:1E67
	U16 bp10 = cd.pi.glbPlayerPosX;
	U16 di = cd.pi.glbPlayerPosY;
	U16 bp12 = cd.pi.glbPlayerMap;
	//^29EE:1E77
	i16 bp16 = 0; // shift-y
	i16 bp14 = 0; // shift-x
	//^29EE:1E7F
	if (bp0e != 0 && bp0e != 3) {
		...
		if (bp0e == 1) {
			...
			if (bp08 == NULL) {
				...
				//^29EE:1EFC
				i16 si = QUERY_CREATURE_5x5_POS(bp0c, (cd.pi.glbPlayerDir + _4976_3fc2[bp0c->b15_0_1()]) & 3);
				//^29EE:1F28
				bp14 = si % 5;
				//^29EE:1F31
				bp16 = si / 5;
	...
	//^29EE:1FF6
	_29ee_1946(recordLink, bp10, di, bp12, cd.pi.glbPlayerDir, bp14, bp16, glbMagicalMapFlags);
	//^29EE:2015
...
void SkWinCore::DRAW_PLAYER_ATTACK_DIR()
{
	ENTER(346);
	SOMETHING_RIGHT_PANEL_29ee_00a3(0);
	DRAW_ICON_PICT_ENTRY(GDAT_CATEGORY_x08_GRAPHICSSET, glbSquadInterfaceMapGfxSet, GDAT_GFXSET_xF6_SQUAD_SINGLE, &glbScreenBufferRightPanel, 93, -1);	// gfx = 0xF6
	U8 *pPictBuffer = ALLOC_PICT_BUFF(glbPictIconBufferX, glbPictIconBufferY, afDefault, IMG_4_BPP);	// bp04
	i16 bp06 = 0;
	i16 bp08 = 0;
	SRECT xRectZone;	// bp10
	if (QUERY_BLIT_RECT(pPictBuffer, &xRectZone, 94, &bp06, &bp08, -1) != 0) {
		if (glbGlobalSpellEffects.AuraOfSpeed != 0) {
			U16 iRandomDisplacement = RAND02();	// si
			if (iRandomDisplacement != 0) {
				xRectZone.y -= 2;
				iRandomDisplacement = RAND02();
				if (iRandomDisplacement != 0) {
		...
		U8 iLocalPal[16];	// bp015a
		U16 di;
		QUERY_GDAT_SQUAD_ICON(pPictBuffer, U8(di = cd.pi.glbChampionIndex - 1), iLocalPal);
		DRAW_ICON_PICT_BUFF(pPictBuffer, &glbScreenBufferRightPanel, &xRectZone, bp06, bp08, 12, 0, iLocalPal);
		if (tblChampionSquad[di].enchantmentPower != 0) {
			ExtendedPicture xExtPicture;	// bp014a
			QUERY_PICST_IT(QUERY_GDAT_SUMMARY_IMAGE(&xExtPicture, GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x07_CHAMPION_AURA, tblChampionSquad[di].enchantmentAura));
			xExtPicture.w4 |= 0x0010;
			xExtPicture.iWidth >>= 2;
			xExtPicture.w14 = xExtPicture.iWidth * ((tblChampionSquad[di].playerDir() +4 - cd.pi.glbPlayerDir) & 3);
			xExtPicture.iHeight >>= 2;
			xExtPicture.w16 = xExtPicture.iHeight * (U16(glbGameTick) & 3);
			DRAW_SOME_PICTURE_0b36_11c0(&xExtPicture, &glbScreenBufferRightPanel, 94, 12);
		}
	}
	FREE_PICT_BUFF(pPictBuffer);
	DRAW_ICON_PICT_ENTRY(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x04_CHAMPION, 0x10, &glbScreenBufferRightPanel, 96, -1);	// arrow to rotate-left champion
	DRAW_ICON_PICT_ENTRY(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_SUBCAT_x04_CHAMPION, 0x12, &glbScreenBufferRightPanel, 97, -1);	// arrow to rotate-right champion
	return;
}
...
void SkWinCore::HIGHLIGHT_ARROW_PANEL(U16 cls4, U16 rectno, U16 bright)
{
	...
	sk3f6c bp34;
	SOME_RECT_PICT_0b36_0c52(&bp34, si, 1);
	FILL_ENTIRE_PICT(QUERY_MEMENT_BUFF_FROM_CACHE_INDEX(bp34.iCacheIndex0), glbPaletteT16[C00_COLOR_BLACK]);
...
void SkWinCore::DRAW_PICST_X_SRECT(SRECT xRectangle, ExtendedPicture *ref)
{
	...
	if (ref->colorKeyPassThrough != -2) {
		FIRE_BLIT_PICTURE(
			...
			bp08,								// srcx
			bp0a,								// srcy
			READ_IMGBUFF_WIDTH(bp04),					// srcPitch
			si,									// dstPitch
			ref->colorKeyPassThrough,			// colorkey
			ref->mirrorFlip,					// mirrorFlip
			READ_IMGBUFF_BPP(bp04),					// srcBpp
			bp0c,								// dstBpp
...
void SkWinCore::DRAW_CHARSHEET_OPTION_ICON(U8 cls4, U16 rectno, U16 zz)
{
	...
	if ((_4976_581a & zz) != 0)
		cls4++;
	DRAW_STATIC_PIC(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0x00, cls4, rectno, -1);
	return;
}
...
void SkWinCore::DRAW_PLAYER_3STAT_PANE(U16 player, U16 xx)
{
	...
	if (_4976_3ff0.iCacheIndex0 != 0xFFFF)
		return;
	// SPX: Choose between 0 (normal champion panel) or 1 (dead champion panel) or 9 (portrait panel)
	//U8 bp01 = (tblChampionSquad[si].curHP() == 0) ? 1 : (((iChampIdx +1) == glbChampionInventory) ? 9 : 0);
	U8 iChampSlabGfxId = (tblChampionSquad[iChampIdx].curHP() == 0) ? GDAT_x01_CHAMPION_SLAB_DEAD :
		(((iChampIdx +1) == glbChampionInventory) ? GDAT_x09_CHAMPION_SLAB_PORTRAIT : GDAT_x00_CHAMPION_SLAB_HANDS); // bp01
	SOME_RECT_PICT_0b36_0c52(&_4976_3ff0, iChampIdx + RECT_161_CHAMPION_SLAB_AREA_1, xx);
	DRAW_ICON_PICT_ENTRY(
		GDAT_CATEGORY_x01_INTERFACE_GENERAL,
		GDAT_INTERFACE_SUBCAT_x02_CHAMPION_TOPSLAB,
		iChampSlabGfxId,
		&_4976_3ff0,
		iChampIdx + RECT_161_CHAMPION_SLAB_AREA_1,
		-1
		);
	if (xx == 0)
		_4976_3ff0.iCacheIndex = 0;
	return;
}
...
{
	ENTER(0);
	if (!cd.dm1.bDM1PortraitsActivated[iChampionIndex])	// Regular DM2
	{
	DRAW_ICON_PICT_ENTRY(
		GDAT_CATEGORY_x16_CHAMPIONS,	// 0x16
		tblChampionSquad[iChampionIndex].HeroType(),
		0x00,
        &_4976_3ff0,
	...
	if (cd.dm1.bDM1PortraitsActivated[iChampionIndex])	 // SPX attempt to draw savegame portraits
	{
		i16 iSourceY = 0;
		i16 iSourceX = 0;
...
//^2E62:011B
void SkWinCore::DRAW_PLAYER_3STAT_HEALTH_BAR(U16 iChampionIndex)
{
	ENTER(32);
	SRECT bp0c;
	SOME_RECT_0b36_0d67(
		&_4976_3ff0,
		QUERY_EXPANDED_RECT(iChampionIndex + RECTZONE_185_CHAMPION_1_STATBARS, &bp0c)
		);
	Champion *pChampion = &tblChampionSquad[iChampionIndex];	// bp04
	i16 iStatBarVal[3][2];	// bp20
	iStatBarVal[0][0] = pChampion->curHP();
	iStatBarVal[0][1] = pChampion->maxHP();
	iStatBarVal[1][0] = pChampion->curStamina();
	iStatBarVal[1][1] = pChampion->maxStamina();
	iStatBarVal[2][0] = pChampion->curMP();
	iStatBarVal[2][1] = max_value(pChampion->maxMP(), pChampion->curMP());	// SPX Max because of MANA modifier
	U16 iBarRectNo = iChampionIndex + RECTZONE_193_CHAMPION_1_STATBAR_1;	// di
	for (i16 iBarIdx = 0; iBarIdx < 3; iBarIdx++, iBarRectNo += 4) {	// si
		if (iStatBarVal[iBarIdx][1] != 0) {
			if (SCALE_RECT(
				iBarRectNo,
				&bp0c,
				C10000_RECTZONE_SCALE_FULL,
				(iStatBarVal[iBarIdx][0] * i32(C10000_RECTZONE_SCALE_FULL)) / iStatBarVal[iBarIdx][1]
				) != NULL
			){
				SRECT bp14;
				...
				FILL_RECT_SUMMARY(
					&_4976_3ff0,
					&bp0c,
					QUERY_3STAT_BAR_COLOR(iChampionIndex, glbPaletteT16[glbChampionColor[iChampionIndex]])
					);
#else
				FILL_RECT_SUMMARY(
					&_4976_3ff0,
					&bp0c,
					glbPaletteT16[glbChampionColor[iChampionIndex]]
					);
#endif
			}
		}
...
void SkWinCore::DRAW_PLAYER_DAMAGE(U16 player)
{
	...
	DRAW_SIMPLE_STR(
		...
		glbPaletteT16[C15_COLOR_WHITE],
		glbPaletteT16[C08_COLOR_RED],
		FMT_NUM(tblChampionSquad[iChampIdx].damageSuffered, 0, 3)
		);
	return;
}
...
// SPX: original code below
	...
	{	
		...
		DRAW_LOCAL_TEXT(iRectNo, glbPaletteT16[C13_COLOR_LIGHTER_GRAY], glbPaletteT16[C01_COLOR_DARK_GRAY] | 0x4000, sStatString);
		return;
	}
...
void SkWinCore::DRAW_EYE_MOUTH_COLORED_RECTANGLE(U8 iGDatEntryId, U16 iRectNo)
{
	ENTER(8);
	SRECT xRect;	// bp08
	_2405_011f_RECT(iRectNo, &xRect);
	DRAW_DIALOGUE_PARTS_PICT(
		QUERY_GDAT_IMAGE_ENTRY_BUFF(GDAT_CATEGORY_x01_INTERFACE_GENERAL, 0x02, iGDatEntryId),
        &xRect,
		12,
		QUERY_GDAT_IMAGE_LOCALPAL(GDAT_CATEGORY_x01_INTERFACE_GENERAL, 0x02, iGDatEntryId)
		);
	return;
}
...
void SkWinCore::DRAW_CRYOCELL_LEVER(U16 leverIsOn)
{
	...
	DRAW_STATIC_PIC(GDAT_CATEGORY_x09_WALL_GFX, GDAT_WALL_ORNATE__CRYOCELL, (leverIsOn != 0) ? 0xfb : 0xfa, 0x1ee, -1);
	if (leverIsOn != 0) {
		//^24A5:1082
		CHANGE_VIEWPORT_TO_INVENTORY(0);
		// SPX: If lever is down, play the open cryocell sound
		QUEUE_NOISE_GEN1(GDAT_CATEGORY_x09_WALL_GFX, GDAT_WALL_ORNATE__CRYOCELL, 0xFB, 0xFF, 0xc8, cd.pi.glbPlayerPosX, cd.pi.glbPlayerPosY, 0);
	}
	else {
		//^24A5:10AB
...
//^24A5:0E82
void SkWinCore::DRAW_POWER_STAT_BAR(i16 curval, U16 iRectNo, U16 color, i16 vv, U16 ww)
{
	// color
...
#if DM2_EXTENDED_MODE == 1	// custom: fetch color within GDAT
	i16 iBarColor = (di < M0512_FOOD_WARNING) ? QUERY_FOOD_WATER_BAR_COLOR(0x0A, C08_COLOR_RED) : ((di < 0) ? QUERY_FOOD_WATER_BAR_COLOR(0x09, C11_COLOR_YELLOW) : color);
#else
	i16 iBarColor = (di < M0512_FOOD_WARNING) ? C08_COLOR_RED : ((di < 0) ? C11_COLOR_YELLOW : color);	// si
#endif
	di -= vv;
	SRECT bp08;
	if (SCALE_RECT(
		...
		(i32(C10000_RECTZONE_SCALE_FULL) * di) / (C2048_FOOD_MAX - vv),
		C10000_RECTZONE_SCALE_FULL
		) != NULL
	) {
		SRECT bp18;
		SRECT bp10;
		if (ww != 0) {
			QUERY_EXPANDED_RECT(iRectNo, &bp10);
		...
		bp18.x += _4976_0142;
		bp18.y += _4976_0142;
		FIRE_FILL_BACKBUFF_RECT(&bp18, glbPaletteT16[C00_COLOR_BLACK]);
#if DM2_EXTENDED_MODE == 1
		FIRE_FILL_BACKBUFF_RECT(&bp08, ((iBarColor & 0x0100) == 0x0000) ? glbPaletteT16[iBarColor] : (iBarColor & 0x00FF));
#else
		FIRE_FILL_BACKBUFF_RECT(&bp08, glbPaletteT16[iBarColor]);
#endif
		if (ww != 0) {
...
#if DM2_EXTENDED_MODE == 1
// SPX: note: there is this thread from "emetar" on DM forum who asks plenty of questions about customization in DM2; hence this custom addition below
// https://www.dungeon-master.com/forum/viewtopic.php?t=29804
// kkdf2: ext for emetar
i16 SkWinCore::QUERY_FOOD_WATER_BAR_COLOR(U8 iGDatCls4EntryId, i16 iDefaultColor) {
	RawEntry* xRawData = QUERY_GDAT_ENTRYPTR(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0, dtWordValue, iGDatCls4EntryId);	// bp04
	if (xRawData != NULL) {
		return 256 + xRawData->data;
	}
	return iDefaultColor;
}

// kkdf2: ext for emetar
i16 SkWinCore::QUERY_3STAT_BAR_COLOR(U8 iGDatCls4EntryId, i16 iDefaultColor) {
	RawEntry* xRawData = QUERY_GDAT_ENTRYPTR(GDAT_CATEGORY_x01_INTERFACE_GENERAL, GDAT_INTERFACE_CLASS_CHAMPION_FACING, dtWordValue, iGDatCls4EntryId);	// bp04
	if (xRawData != NULL) {
		return xRawData->data;
	}
	return iDefaultColor;
}
#endif
...
void SkWinCore::DRAW_FOOD_WATER_POISON_PANEL()
{
	...
	DRAW_STATIC_PIC(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0x00, GDAT_INTERFACE_CHAR_FOOD_WATER_PANEL, 494, -1);
#if DM2_EXTENDED_MODE == 1
	DRAW_POWER_STAT_BAR(pChampion->curFood(), 496, QUERY_FOOD_WATER_BAR_COLOR(0x06, C05_COLOR_BROWN), -1024, 0);
	DRAW_POWER_STAT_BAR(pChampion->curWater(), 497, QUERY_FOOD_WATER_BAR_COLOR(0x07, C14_COLOR_BLUE), -1024, 0);
#else
	DRAW_POWER_STAT_BAR(pChampion->curFood(), 496, C05_COLOR_BROWN, -1024, 0);
	DRAW_POWER_STAT_BAR(pChampion->curWater(), 497, C14_COLOR_BLUE, -1024, 0);
#endif
	DRAW_STATIC_PIC(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0x00, GDAT_INTERFACE_FOOD_TEXT, 500, 12);	// food
	DRAW_STATIC_PIC(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0x00, GDAT_INTERFACE_WATER_TEXT, 501, 12);	// water
	if (pChampion->PoisonValue != 0) {
		DRAW_STATIC_PIC(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0x00, GDAT_INTERFACE_POISON_TEXT, 502, 12); // poison
		...
		//if (bUseSuperMode)
		//	DRAW_POWER_STAT_BAR(pChampion->PoisonValue, 497, C08_COLOR_RED, -1024, 0);
	}
#if (XDMX_EXTENDED_FEATURES == 1)
	if (pChampion->PlagueValue != 0) 
	{
		DRAW_STATIC_PIC(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0x00, GDAT_INTERFACE_PLAGUED_TEXT, 502, 12); // plagued
	}
#endif
	return;
}
...
void SkWinCore::DRAW_GUIDED_STR(const U8 *ref)
{
	...
	if (QUERY_MBCS_PRESENCE(ref) != 0) {
		//^24A5:0050
		DRAW_VP_STR(_4976_52d8, _4976_52da, glbPaletteT16[C13_COLOR_LIGHTER_GRAY], ref);
		//^24A5:0071
	...
	for (i16 si = 0; *bp04 != 0; ) {
		...
		//^24A5:00E0
		DRAW_VP_STR(_4976_52d8, _4976_52da, glbPaletteT16[C13_COLOR_LIGHTER_GRAY], bp04);
		//^24A5:0101
...
void SkWinCore::DRAW_STUFF_STRINGS(i16 xx, i16 yy, U8 *str)
{
	...
	U16 bp04 = 0;
	U16 bp02;
	if (QUERY_STR_METRICS_U16(str, &bp02, &bp04) == 0)
		return;
	U8 bp00d0[200];
	...
	DRAW_VP_STR(xx -(bp02 >> 1), yy, glbPaletteT16[C00_COLOR_BLACK], str);
	return;
}
...
U16 SkWinCore::DRAW_ITEM_SURVEY(ObjectID recordLink, U16 xx)
{
	...
	DRAW_STATIC_PIC(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0x00, 0x01, 0x01ee, -1);
	U8 bp09;
	U8 bp0a;
	...
	U8 bp0094[128];
    DRAW_GUIDED_STR(QUERY_GDAT_TEXT(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0x00, 0x1f, bp0094));			
	if (bp08 == dbContainer) {
	...
	U16 iDisplayRow = 0;	// bp10
	U8 rune;		// bp11 This holds the rune to be displayed along the power bar
	U16 color = C04_COLOR_CYAN;

	if (CHECK_ITEM_CAN_USE_CHARGES(si) != 0) {
		i16 iNbCharges = ADD_ITEM_CHARGE(si, 0);	/// di
		if (iNbCharges != 0) {
			rune = RUNE_GOR;
			// SPX addition for hard coded waterskin and tankard
			if (si.orif.db == dbMiscellaneous_item) {
				rune = RUNE_VI;
				color = C14_COLOR_BLUE;
			}
			DRAW_ITEM_STATS_BAR(
				bp0e.pos[iDisplayRow++],
				iNbCharges,
...
//	An item with charges will have a CYAN GOR bar with the number of remaining charges
	...
	if (!SkCodeParam::bUseDM2ExtendedMode) {
		if (value != 0) {
			DRAW_ITEM_STATS_BAR(bp0e.pos[iDisplayRow], value, max, rune, C05_COLOR_BROWN);
		}
	}
	// SPX: little modif to have different colors
	else if (SkCodeParam::bUseDM2ExtendedMode && value != 0)
	{
		color = C05_COLOR_BROWN;
		switch (rune)
		{
			...
			case RUNE_ROS:	// Projectile weapon
				color = C09_COLOR_ORANGE;
				break;
			case RUNE_NETA:	// Potion
				color = C14_COLOR_BLUE;
				break;
		}
		DRAW_ITEM_STATS_BAR(bp0e.pos[iDisplayRow], value, max, rune, color);
	}
...
//--- NEW CODE -------------------------------------
	...
	if (SkCodeParam::bUseSuperInfoEye)
	{
		...
		*pstr = '$';
		...
		DRAW_VP_RC_STR(
			569,
			glbPaletteT16[C11_COLOR_YELLOW],
			strMoneyValue
			);

		if ( iDisplayRow != 0 || value != 0)
			yy += 2;
		yy += ((iDisplayRow + (value != 0 ? 1 : 0))*12);
...
U16 SkWinCore::DRAW_ITEMS_HANDS_OR_INVENTORY(U16 iChampIdx, U16 itemNo, U16 yy)
{
	...
	U16 bp08 = yy;
	U16 si;
	if (iChampIdx + 1 != glbChampionInventory) {
		if (itemNo > C01_INVENTORY_HAND_LAST || iChampIdx + 1 == cd.pi.glbNextChampionNumber)
			return 0;
		si = (iChampIdx << 1) + itemNo;
	}
	else {
		si = itemNo + 8;
	}
	ObjectID di = tblChampionSquad[iChampIdx].Possess(itemNo);
	if (itemNo <= C01_INVENTORY_HAND_LAST) {	// <= 1
		if (iChampIdx + 1 == cd.pi.glbChampionIndex && itemNo == glbSelectedHandAction)
			bp04 = 1;
	...
	if (itemNo <= C05_INVENTORY_BODYPART_LAST) {	// <= 5
		if ((tblChampionSquad[iChampIdx].bodyFlag & (1 << itemNo)) != 0)
			bp06 = 1;
	...
	if (yy != 0 || bp08 != 0 || _4976_3b74[si].w6 != di) {
		if (di == 0xffff || si < 8)
			bp08 = 1;
		if (si < 8) {
			DRAW_PLAYER_3STAT_PANE(iChampIdx, 0);
		}
		_4976_3b74[si].w6 = di;
...
//^0CD5:0018
void SkWinCore::FILL_STR(U8* xBuffer, U16 iMaxCount, U8 iValue, U16 iDeltaStep)
{
	for (U16 iBuffIdx = 0, iCountIdx = 0; iCountIdx < iMaxCount; iBuffIdx += iDeltaStep, iCountIdx++) {	// si, di
		xBuffer[iBuffIdx] = iValue;
	}
}
...
void SkWinCore::DRAW_SKILL_PANEL()
{
	...
	for (bp06 = 0; bp06 <= 3; bp06++) {
		...
		if (si != SkLvlNone) {	// (si != 1)
			_4976_52e4 = bp06;
			QUERY_GDAT_TEXT(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0x00
				, tSkillLevelNameGDAT2Entry[RCJ(15,si -SkLvlNeophyte)], bp0090);	// [RCJ(15,si -2)]
			DRAW_VP_STR(bp0c, bp10, glbPaletteT16[(glbChampionMajorSkillsLevel[di][bp06] != 0) ? C07_COLOR_LIGHT_GREEN : C13_COLOR_LIGHTER_GRAY], bp0090);
	...
	for (bp06 = attributeStart; bp06 <= ATTRIBUTE_LAST; bp06++) {
		DRAW_VP_STR(
			...
			QUERY_GDAT_TEXT(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0x00, bp06 +0x20, bp0090)
			);
...
//^2E62:0670
void SkWinCore::REFRESH_PLAYER_STAT_DISP(i16 iChampIdx)
{
	ENTER(164);
	U16 iInterfaceIconsFlags = 0;	// bp14
	U16 bp16 = 0;
	U16 bp18 = 0;
	U16 bp1a = 0;
	U16 bp1c = 0;
	Champion *pChampion = &tblChampionSquad[iChampIdx];	//*bp04
	U16 si = pChampion->heroFlag;
	U16 bp08 = (iChampIdx + 1 == glbChampionInventory) ? 1 : 0; // 1=draw entire inventory panel
	if (bp08 != 0 && _4976_022c != 0)
		si |= 0x8000;
	if (glbChampionInventory != glbLastChampionInventory || (si & 0x8000) != 0) {
		if (bp08 != 0) {
			DRAW_STATIC_PIC(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0x00, 0x00, 0x04, -1);	// char sheet general screen
			bp1c = 1;
			bp18 = 1;
			glbLastChampionInventory = glbChampionInventory;
			iInterfaceIconsFlags = 0x0F;	// 1 + 2 + 4 + 8 => display all 3 icons + char bar name
			si |= 0x7c00;
		}
		else {
		...
		iInterfaceIconsFlags = _4976_581a ^ _4976_5804;
	}
	if (iInterfaceIconsFlags != 0) {
		if (cd.pi.glbNextChampionNumber == 0) {
			if ((iInterfaceIconsFlags & 1) != 0) {
				DRAW_CHARSHEET_OPTION_ICON(GDAT_INTERFACE_CHARNAME_BAR, 0x0267, 0x0001);	// 0x0013: char name bar
			}
			if ((iInterfaceIconsFlags & 8) != 0) {
				DRAW_CHARSHEET_OPTION_ICON(GDAT_INTERFACE_DISK_BUTTON, 0x0232, 0x0008);	// 0x000F: blue diskette
			}
			if ((iInterfaceIconsFlags & 4) != 0) {
				DRAW_CHARSHEET_OPTION_ICON(GDAT_INTERFACE_SLEEP_BUTTON, 0x0234, 0x0004);	// 0x000D: Zzz
			}
		}
		if ((iInterfaceIconsFlags & 2) != 0) {
			DRAW_CHARSHEET_OPTION_ICON(GDAT_INTERFACE_CANCEL_BUTTON, 0x0236, 0x0002);		// 0x000B: Cancel red cross
	...
	if ((si & 0x4000) != 0) {
		DRAW_PLAYER_3STAT_PANE(iChampIdx, 1);
		bp1a = 1;
		if (pChampion->curHP() != 0) {
			if (bp08 != 0) {
				DRAW_CHAMPION_PICTURE(iChampIdx);
				si |= 0x0800;
			}
			else {
				si |= 0x0c00;
			...
			DRAW_NAME_STR(
				...
				iChampIdx + 165,
				glbPaletteT16[C15_COLOR_WHITE],
				glbPaletteT16[C01_COLOR_DARK_GRAY] |0x4000,
				pChampion->firstName
				);
		}
	}
	if (pChampion->curHP() != 0) {
		if ((si & 0x0800) != 0) {	// Refresh stats bar
			DRAW_PLAYER_3STAT_PANE(iChampIdx, 0);
			bp1a = 1;
			DRAW_PLAYER_3STAT_HEALTH_BAR(iChampIdx);
			if (pChampion->timerIndex != TIMER_NONE) {
				DRAW_PLAYER_DAMAGE(iChampIdx);
			}
			if (bp08 != 0) {
				SRECT bp24;
				...
				DRAW_DIALOGUE_PICT(
					...
					bp24.y,
					-1,
					QUERY_GDAT_IMAGE_LOCALPAL(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0x00, 0x00)
					);
				DRAW_PLAYER_3STAT_TEXT(pChampion);
				// SPX: bp0d holds the grey slab/red frame around the mouth or eye.
				U8 iSlabGfxID; //bp0d;
#if (XDMX_EXTENDED_FEATURES == 0) // original
				if (pChampion->curFood() < 0 || pChampion->curWater() < 0 || pChampion->PoisonValue != 0) {
#elif (XDMX_EXTENDED_FEATURES == 1)
				if (pChampion->curFood() < 0 || pChampion->curWater() < 0 || pChampion->PoisonValue != 0 || pChampion->PlagueValue != 0) {
#endif
					iSlabGfxID = GDAT_x05_CHAR_ICON_SLAB_RED_FRAME; // i am hungry! or poisoned!
				}
				else {
					iSlabGfxID = GDAT_x04_CHAR_ICON_SLAB_GREY;
				}
				// SPX: Display colored rectangle around mouth
				DRAW_EYE_MOUTH_COLORED_RECTANGLE(iSlabGfxID, RECT_545_INVENTORY_MOUTH_COLOR_FRAME);
				iSlabGfxID = GDAT_x04_CHAR_ICON_SLAB_GREY;
				// SPX: review skills from STR to AM
				for (i16 iSkillIdx = ATTRIBUTE_STRENGTH; iSkillIdx <= ATTRIBUTE_LAST; iSkillIdx++) {	// for (i16 di = 1; di <= 6; di++)
					if (pChampion->attributes[iSkillIdx][ATTRIBUTE_CURRENT] < pChampion->attributes[iSkillIdx][ATTRIBUTE_MAX]) {
						iSlabGfxID = GDAT_x05_CHAR_ICON_SLAB_RED_FRAME; // level gained! stats have changed !
						break;
					}
				}
				// SPX: Display colored rectangle around eye
				DRAW_EYE_MOUTH_COLORED_RECTANGLE(iSlabGfxID, RECT_546_INVENTORY_EYE_COLOR_FRAME);
				bp18 = 1;
			}
		}
		ObjectID bp12;
		if (bp08 != 0) {
			if (glbShowItemStats != 0) {
				bp12 = cd.pi.glbLeaderHandPossession.object;
			}
			else if (_4976_4bfe != 0) {
				bp12 = OBJECT_NULL;
			}
			else {
				bp12 = pChampion->Possess(C01_INVENTORY_HAND_LEFT);
			}
			if (false
				|| _4976_3d30 != bp12 
				...
				if (SkCodeParam::bBWMode) {
					SRECT xSymbolRect = {36, 15, 12, 12 }; // allow for positionning any rectangle even not defined in the rectangle DB
...
_0a25:
		...
		U8 sTextBuffer[128];	// bp00a4
		memset(sTextBuffer, 0, 128);	// SPX
		if ((si & 0x0400) != 0) {
			iTextColorPalIdx = glbPaletteT16[(iChampIdx == glbChampionLeader && cd.pi.glbNextChampionNumber == 0) ? C09_COLOR_ORANGE : C15_COLOR_WHITE];
			if (SkCodeParam::bDM1Mode)
				iTextColorPalIdx = glbPaletteT16[(iChampIdx == glbChampionLeader && cd.pi.glbNextChampionNumber == 0) ? C11_COLOR_YELLOW : C09_COLOR_ORANGE];
			if (bp08 != 0) {	// in inventory sheet
				SK_STRCPY(sTextBuffer, &pChampion->firstName[0]);
				U8 cLastNameFirstChar = pChampion->lastName[0]; // bp09
				if (cLastNameFirstChar != '.' && cLastNameFirstChar != ';' && cLastNameFirstChar != '-') {
					SK_STRCAT(sTextBuffer, glbConstCharTxtSpace);
				}
				SK_STRCAT(sTextBuffer, &pChampion->firstName[8]);
				DRAW_LOCAL_TEXT(RECT_553_CHAMPION_TITLE_NAMES, iTextColorPalIdx, (cd.pi.glbNextChampionNumber != 0) ? glbPaletteT16[C01_COLOR_DARK_GRAY] : 0x4000, sTextBuffer);
			}
			else {
				DRAW_PLAYER_3STAT_PANE(iChampIdx, 0);
				bp1a = 1;
				DRAW_NAME_STR(&_4976_3ff0, iChampIdx + RECT_165_CHAMPION_FIRSTNAME_1, iTextColorPalIdx, glbPaletteT16[C01_COLOR_DARK_GRAY] | 0x4000, pChampion->firstName);
			}
		}
		if (bp08 != 0 && (si & 0x1000) != 0) {
			// Display CHAR sheet + LOAD
			SRECT xRectCharSheet;	// bp24
			...
			U16 iChampWeightLoad = GET_PLAYER_WEIGHT(iChampIdx);	// bp10
			U16 iChampMaxLoad = MAX_LOAD(pChampion);	// di
			U16 iChampCurrentLoadDisplay = 0;	// di
			U16 iLoadTextColor = 0;	// bp06
			if (iChampMaxLoad < iChampWeightLoad) {	// overloaded
				iLoadTextColor = C08_COLOR_RED;	// 8
			}
			else {	// loaded = yellow / good = lighter gray
				iLoadTextColor = ((i32(iChampWeightLoad) << 3) > (i16(iChampMaxLoad) * i32(5))) ? C11_COLOR_YELLOW : C13_COLOR_LIGHTER_GRAY;	// 11 : 13
			}
			glbChampWeightLoadDisplay = iChampCurrentLoadDisplay = iChampWeightLoad / 10;
			glbChampWeightLoadTenthDisplay = iChampWeightLoad - (iChampCurrentLoadDisplay * 10); // this gets the unit of current load (or tenth of weight load)
			glbChampMaxLoadDisplay = MAX_LOAD(pChampion) / 10;
            QUERY_GDAT_TEXT(GDAT_CATEGORY_x07_INTERFACE_CHARSHEET, 0x00, 0x2A, sTextBuffer);		// 0x2A = text template XX / XX KG
			DRAW_LOCAL_TEXT(RECT_555_CHAMPION_SHEET_LOAD_STRING, glbPaletteT16[iLoadTextColor], glbPaletteT16[C01_COLOR_DARK_GRAY] | 0x4000, sTextBuffer);
			bp18 = 1;
		}
	}
	if (bp1a != 0) {
	...
	pChampion->heroFlag &= 0x03FF;	// 0x03FF
	return;
}
...
void SkWinCore::UPDATE_GENERAL_CHAMPIONS_STAT_DISP(U16 xx)
{
	...
	if (glbChampionInventory != 0) {
		U16 iChampionIndex = glbChampionInventory - 1;	// si
		Champion* xChampion = &tblChampionSquad[iChampionIndex];
		if (xChampion->enchantmentPower != 0 && xChampion->enchantmentAura >= C03_ENCHANTMENT_AURA_FIRST && xChampion->enchantmentAura <= C06_ENCHANTMENT_AURA_LAST) {
			xChampion->heroFlag |= (glbShowItemStats != 0 && cd.pi.glbLeaderHandPossession.object == OBJECT_NULL) ? CHAMPION_FLAG_3000 : CHAMPION_FLAG_1000;	// 0x3000 or 0x1000
		}
...
{
	ENTER(20);
	U8* xIconBuffer = ALLOC_PICT_BUFF(16, 16, afDefault, IMG_4_BPP);	// bp04
	U8 iBlackPalette[16];	// bp14
	for (i16 iPalIdx = 0; iPalIdx < 16; iPalIdx++) {	// si
		iBlackPalette[iPalIdx] = glbPaletteT16[C00_COLOR_BLACK];
	}
...
void SkWinCore::CHANGE_CURSOR_HAND_ITEM()
{
	...
	if (_4976_5dac != 0) {
		U8* xCursorImage = ALLOC_PICT_BUFF(18, 18, afDefault, IMG_8_BPP);	// bp04
...
void SkWinCore::_32cb_0f82_SHOP_GLASS(Actuator *ref, U8 cls4, i16 bb, i16 cellPos, U16 horzStretch, U16 vertStretch, U16 rectno, U16 gg, U16 colorkey1)
{
	...
	if (ref->ActiveStatus() == 0 || ref->OnceOnlyActuator() != 0) {
		...
		if (QUERY_CREATURES_ITEM_MASK(bp11, U8(bp14), bp04, 0) != 0) {
			U16 *bp10 = reinterpret_cast<U16 *>(QUERY_GDAT_ENTRY_DATA_BUFF(GDAT_CATEGORY_x09_WALL_GFX, cls4, dt08, GDAT_WALL_ORNATE__DATA_13));
			i16 bp28;
			if (bp10 != NULL) {
				bp28 = QUERY_GDAT_ENTRY_DATA_LENGTH(GDAT_CATEGORY_x09_WALL_GFX, cls4, dt08, GDAT_WALL_ORNATE__DATA_13) >> 1;
				for (si = 0; si < bp28; si++) {
					if ((bp10[si] & 511) == bp24)
						break;
				}
	...
	U16 bp18 = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x09_WALL_GFX, cls4, dtImageOffset, GDAT_WALL_ORNATE__ITEM_INSIDE_DISPLACEMENT);	// 0xFD
	...
	if (true
		...
		U16 *bp10 = reinterpret_cast<U16 *>(QUERY_GDAT_ENTRY_DATA_BUFF(GDAT_CATEGORY_x09_WALL_GFX, cls4, dt08, GDAT_WALL_ORNATE__DATA_13));
		if (bp10 != NULL) {
			i16 bp28 = QUERY_GDAT_ENTRY_DATA_LENGTH(GDAT_CATEGORY_x09_WALL_GFX, cls4, dt08, GDAT_WALL_ORNATE__DATA_13) >> 1;
			for (si = 0; si < bp28; si++) {
				if ((bp10[si] & 511) == bp24)
					break;
			}
	...
	if (cellPos == 3) {
		...
		U16 bp18 = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x09_WALL_GFX, cls4, dtImageOffset, 0xf2);
		bp2a += i8(bp18 >> 8);
		bp2c += i8(bp18);
	...
	if (cellPos == 3) {
		...
		for (i16 bp16 = 0; bp16 < 2; bp16++) {
			U16 bp18 = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x09_WALL_GFX, cls4, dtImageOffset, (bp16 != 0) ? GDAT_WALL_ORNATE__DATA_F0 : GDAT_WALL_ORNATE__DATA_F1);
			i16 bp2a = i8(bp18 >> 8);
			i16 bp2c = i8(bp18);
			...
			DRAW_DIALOGUE_PICT(bp08, glbBackBuffViewport, QUERY_BLIT_RECT(bp08, &bp34, rectno|0x8000, &bp2a, &bp2c, gg), 
				0, 0, colorkey1, glbTempPicture.iPal256);
...
{
	// Note: D1 front door frame is not displayed from here.
	ENTER(854);
	...
	if (bp2a == 0) {
		iDoNotFlip = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x09_WALL_GFX, bp1f, dtWordValue, GDAT_WALL_ORNATE__DO_NOT_FLIP);	// Has some role for general graphics flip
	}
	U16 iFlipImage = 0;	// U16 bp0e = 0; use flip or not
	U16 si = 0; // defaulting to 0
	if (bp2a != 0 || (si = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x09_WALL_GFX, bp1f, dtWordValue, GDAT_IMG_COLORKEY_1)) == 0)
		si = glbSceneColorKey;
	U16 iOrnatePos = 0;	// U16 bp1a
	U16 iRefPoint = 0;	// U16 bp1e
	if (bp2a != 0 || (iOrnatePos = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x09_WALL_GFX, bp1f, dtWordValue, GDAT_WALL_ORNATE__POSITION)) == 0) {
	...
	if (alcoveType == C3_WALL_ORNATE_OBJECT__CRYOCELL && yy == 0) {	// bp22 == 3 && yy == 0
		U16 bp34 = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x09_WALL_GFX, bp1f, dtImageOffset, GDAT_GFXSET_xFD_DATA_FD);	// 0x09 .. .. 0xFD
		...
		QUERY_TEMP_PICST(iFlipImage, iStretchHorizontal, iStretchVertical, i8(bp34 >> 8), i8(bp34), iYDist, iRectno, iRefPoint, -1, -1, GDAT_CATEGORY_x16_CHAMPIONS, U8(tblCellTilesRoom[cellPos].xsrd.xvalue), 1);
		if (zz == 0)
	...
	if (bp2a != 0) {
		...
		if (yy == 0) {
			...
			if (QUERY_GDAT_ENTRY_IF_LOADABLE(GDAT_CATEGORY_x08_GRAPHICSSET, glbMapGraphicsSet, dtImage, 0xFE) != 0) { // SPX: GDAT2 never has 0xFE image ???
		...
		QUERY_TEMP_PICST(iFlipImage, iStretchHorizontal, iStretchVertical, 0, 0, iYDist, iRectno, iRefPoint, si, -1, GDAT_CATEGORY_x08_GRAPHICSSET, glbMapGraphicsSet, iTextPanelImageID);
		if (zz == 0)
			glbTempPicture.colorKeyPassThrough = -2;
		...
		QUERY_GDAT_SUMMARY_IMAGE(&bp01d6, GDAT_CATEGORY_x08_GRAPHICSSET, glbMapGraphicsSet, iTextPanelImageID);
		i16 bp2c = bp01d6.iXOffset;
		i16 bp2e = bp01d6.iYOffset;
		i16 bp48width; // bp48
		i16 bp4aheight; // bp4a
		QUERY_GDAT_IMAGE_METRICS(GDAT_CATEGORY_x08_GRAPHICSSET, glbMapGraphicsSet, iTextPanelImageID, &bp48width, &bp4aheight);
		...
		if (bp4c != 0) {
			...
			//// SPX: this gets the letters font per graphicsset, but if not existing, it will crash in FIRE_BLIT_PICTURE
			U8 *bp0c = QUERY_GDAT_IMAGE_ENTRY_BUFF(GDAT_CATEGORY_x08_GRAPHICSSET, glbMapGraphicsSet, GDAT_GFXSET_x03_LETTERS);	// 0x08 .. 0x03
			RawEntry* xRawFontData = QUERY_GDAT_ENTRYPTR(GDAT_CATEGORY_x08_GRAPHICSSET, glbMapGraphicsSet, dtImage, GDAT_GFXSET_x03_LETTERS);	// SPX: added this to check if font exists
			U16 bDataExist = QUERY_GDAT_ENTRY_IF_LOADABLE(GDAT_CATEGORY_x08_GRAPHICSSET, glbMapGraphicsSet, dtImage, GDAT_GFXSET_x03_LETTERS);
			rc44.cx = bp3a;
					...
					if (rc44.x < 0) {
						...
						while (bp36-- != 0) {
							DRAW_DIALOGUE_PICT(
								...
								0,
								si,
								QUERY_GDAT_IMAGE_LOCALPAL(GDAT_CATEGORY_x08_GRAPHICSSET, glbMapGraphicsSet, GDAT_GFXSET_x03_LETTERS)	// 0x08 .. 0x03
								);
							rc44.x += bp3a;
						}
					}
		...
		bp01d6.iRawDataIndex = GDAT_xFFFF_RAW_DATA_INDEX__NONE;
		bp01d6.iXStretch = iStretchHorizontal;
		bp01d6.iYStretch = iStretchVertical;
		if (bp4c != 0) {
			bp01d6.iPal256[1] = glbPaletteT16[C00_COLOR_BLACK];
			bp01d6.iPal256[2] = glbPaletteT16[C05_COLOR_BROWN];
		}
		else {
			COPY_MEMORY(
				QUERY_GDAT_IMAGE_LOCALPAL(GDAT_CATEGORY_x08_GRAPHICSSET, glbMapGraphicsSet, GDAT_GFXSET_x03_LETTERS),	// 08 .. 03
				bp01d6.iPal256,
				16
				);
		}
		bp01d6.iPaletteSize = C016_GDAT_PALETTE_SIZE_16;
		PALETTE_SOMETHING_32cb_0804(bp01d6.iPal256, U8(iYDist), si, -1, bp01d6.iPaletteSize);
	...
	if (yy == 0) {	// Front image
		...
		if (yy >= 1) {
			if (QUERY_GDAT_ENTRY_IF_LOADABLE(GDAT_CATEGORY_x09_WALL_GFX, bp1f, dtImage, U8(bp28) +2) != 0) {
		...
		if (SkCodeParam::bGFXFixModeDM1 && iYDist == 0)		{
			...
			if (QUERY_GDAT_ENTRY_IF_LOADABLE(GDAT_CATEGORY_x09_WALL_GFX, bp1f, dtImage, iTestSideD0Entry) != 0) {
	...
	if (SkCodeParam::bUseExtendedSound && bp28 == 4 /*&& SkCodeParam::bForceOrnateSound == true*/)	// 4 = first anim of a loop (if several)
	{
		...
		{
			if (QUERY_GDAT_ENTRY_IF_LOADABLE(GDAT_CATEGORY_x09_WALL_GFX, bp1f, dtSnd, SOUND_ACTIVATION_LOOP + iMaxLoopSoundsAvailable) != 0)
				break;
...
//		i16 iYDist = glbTabYAxisDistance[RCJ(23,cellPos)];	
		...
		QUEUE_NOISE_GEN2(GDAT_CATEGORY_x09_WALL_GFX, bp1f, SOUND_ACTIVATION_LOOP + iRandNoise, 0xFE, cd.pi.glbPlayerPosX+iXDist, cd.pi.glbPlayerPosY+iYDist, 0, 140, 200);
	}
#endif

	SRECT *bp12;
	if (yy == 0 && QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x09_WALL_GFX, bp1f, dtWordValue, GDAT_WALL_ORNATE__WINDOW) != 0) {
		QUERY_TEMP_PICST(iFlipImage, iStretchHorizontal, iStretchVertical, 0, 0, iYDist, iRectno, iRefPoint, -3, -3, GDAT_CATEGORY_x09_WALL_GFX, bp1f, iImageEntry);
		QUERY_TEMP_PICST(iFlipImage, iStretchHorizontal, iStretchVertical, glbTempPicture.iXOffset, glbTempPicture.iYOffset, iYDist, iRectno, iRefPoint, si, -1, GDAT_CATEGORY_x09_WALL_GFX, bp1f, 200);
		if (zz == 0)
		...
		U16 bp26 = QUERY_MULTILAYERS_PIC(
			&bp0310, GDAT_CATEGORY_x09_WALL_GFX, bp1f, iImageEntry, iStretchHorizontal, iStretchVertical, iYDist, iFlipImage, si,
			QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x09_WALL_GFX, bp1f, dtWordValue, GDAT_IMG_WALL_COLORKEY_2)	// 0x11 is colorkey2 for seeing outside through window (0xC8 image)
			);
		DRAW_RAIN_32cb_0c7d(&bp0310, bp26, si);
		bp0310.pb44 = reinterpret_cast<U8 *>(QUERY_MEMENT_BUFF_FROM_CACHE_INDEX(bp26));
		DRAW_PICST(&bp0310);
		FREE_PICT_MEMENT(&bp0310);
		bp0310.colorKeyPassThrough = bp0310.iPal256[si];
		bp0310.pb44 = glbBackBuffViewport;
		bp0310.rectNo = iRectno;
		bp0310.w26 = iRefPoint;
		bp0310.iPaletteSize = C000_GDAT_PALETTE_SIZE_ZERO;
		...
		QUERY_TEMP_PICST(iFlipImage, iStretchHorizontal, iStretchVertical, 0, 0, iYDist, iRectno, iRefPoint, si, -1, GDAT_CATEGORY_x09_WALL_GFX, bp1f, iImageEntry);
		if (zz == 0)
			glbTempPicture.colorKeyPassThrough = -2;
		DRAW_TEMP_PICST();
	...
	if (alcoveType == C0_WALL_ORNATE_OBJECT__NONE || yy != 0)	// bp22 == 0
		return si;
	COPY_MEMORY(&glbTempPicture, &glbDrawExtPicture, sizeof(ExtendedPicture));	// 314 is size of "ExtendedPicture"
	// SPX: This is an overlay on front of wall decoration. This is only used for the shop panel
	if (QUERY_GDAT_ENTRY_IF_LOADABLE(GDAT_CATEGORY_x09_WALL_GFX, bp1f, dtImage, GDAT_WALL_ORNATE__OVERLAY) != 0) {
		...
		QUERY_TEMP_PICST(iFlipImage, iStretchHorizontal, iStretchVertical, 0, 0, iYDist, iRectno, iRefPoint, si, -1, GDAT_CATEGORY_x09_WALL_GFX, bp1f, GDAT_WALL_ORNATE__OVERLAY);
		if (zz == 0)
			glbTempPicture.colorKeyPassThrough = -2;
		DRAW_TEMP_PICST();
		return si;
...
void SkWinCore::DRAW_ITEM(ObjectID rl, i16 xx, U16 yy, U16 zz, i16 iDisplaceShift, Creature *ref, U16 ww, U16 ss, U16 tt)
{
	...
	if (tt != 0) {
		bp0a = 0;
		iDisplacementXY = QUERY_GDAT_ENTRY_DATA_INDEX(glbDrawExtPicture.iGDatCls1Category, glbDrawExtPicture.iGDatCls2MainItemId, dtImageOffset, GDAT_WALL_ORNATE__ITEM_INSIDE_DISPLACEMENT);	 // 0xFD
		yy = glbDrawExtPicture.iXOffset + i8(iDisplacementXY >> 8) + _4976_41de[RCJ(8,tblItemStackDisplace[RCJ(16,iDisplaceShift)][0])];
		si = _4976_41de[RCJ(8,tblItemStackDisplace[RCJ(16,iDisplaceShift)][1])] + (i8(iDisplacementXY) + glbDrawExtPicture.iYOffset);
		bp06 = glbDrawExtPicture.rectNo;
	}
	else {
		bp0a = 4 -(bp18 / 5);
		if (xx == 0 && bp0a < 2)
	...
	if (iItemGDatCategory == GDAT_CATEGORY_x14_CONTAINERS && iItemGDatItemId < 8) {	// 0x14 && < 8
		if (ss == 0) {
...
{
	int iTextButtonColor = C09_COLOR_ORANGE; // V4
	int iTextVersionColor = C02_COLOR_GRAY; // V4
	ENTER(104);
	...
	DRAW_VP_RC_STR(0x1D2, glbPaletteT16[iTextButtonColor], bp18);	// (C09_COLOR_ORANGE) LOAD text
	DRAW_VP_RC_STR(0x1D3, glbPaletteT16[iTextButtonColor], bp14);	// (C09_COLOR_ORANGE) CANCEL text
...
void SkWinCore::SHOW_CREDITS()
{
	...
	cd.mo.glbSpecialScreen = _MENU_SCREEN__SHOW_CREDITS;;	// 218 (0xDA)
	_1031_0541(0);
	FIRE_HIDE_MOUSE_CURSOR();
	return;
}
...
U8 SkWinCore::_0aaf_02f8_DIALOG_BOX(U8 iDialogBoxID, U8 yy) //#DS=4976
{
	...
	iButtonColors[0] = glbPaletteT16[C11_COLOR_YELLOW];
	iButtonColors[1] = glbPaletteT16[C09_COLOR_ORANGE];
	...
	if ((glbGDatStructureRead != 0) && ((_4976_5d76 != 0) || (_4976_00f4 +8 <= glbFreeRAMMemPool))) {
		...
		FIRE_FILL_BACKBUFF_RECT(SET_ORIGIN_RECT(&bp20, glbViewportWidth, glbViewportHeight), glbPaletteT16[C01_COLOR_DARK_GRAY]);
		FIRE_FILL_BACKBUFF_RECT(INFLATE_RECT(&bp20, -10, -10), glbPaletteT16[C05_COLOR_BROWN]);
		if (bp0c == 0x0003) {
			bp10 = 1;
		}
		else if (bp0c == 0x0004) {
		...
		for (U16 si=0; si < bp0c; si++) {
			RECT_0aaf_01db(_4976_01bc[0][RCJ(8,si + bp10)], 0);
		}
	}
	//^0AAF:050C
	// SPX: Draw string version on dialog box
	int iColorVersion = C02_COLOR_GRAY;
	int iColorButton = C09_COLOR_ORANGE;
	if (SkCodeParam::bDM2V5Mode) {
		iColorVersion = C11_COLOR_YELLOW;
		iColorButton = C11_COLOR_YELLOW;
	}
	...
	if (yy != 0 && iDialogBoxID == 0) {
		...
		if (yy != 0) {
			bp04e4[1].pb0 = QUERY_GDAT_TEXT(GDAT_CATEGORY_x1A_DIALOG_BOXES, 0, yy, bp04d4);
			goto _0641;
		}
	}
	bp04e4[1].pb0 = NULL;
		...
		for (bp16=0; bp16 < 2; bp24++, bp16++) {
			if (bp24->pb0 != NULL) {
				for (bp0e=0; bp24->pb0[bp0e] != 0; ) {
					...
					DRAW_VP_STR((glbViewportWidth -bp0a) >> 1, di, iButtonColors[bp16], bp64);
...
void SkWinCore::DRAW_ARROW_PANEL()
{
	...
	SOME_RECT_PICT_0b36_0c52(&bp36, RECT_009_PANEL_ARROW__ZONE, 1);
	FILL_ENTIRE_PICT(reinterpret_cast<U8 *>(QUERY_MEMENT_BUFF_FROM_CACHE_INDEX(bp36.iCacheIndex0)), glbPaletteT16[C00_COLOR_BLACK]);	// make black the background of area for 6 arrow buttons
	...
	do {
		...
		if (ref != NULL) {
			if ((bp10 & 0x400) == 0 && ((bp10 >> 4) & 15) == 0) {
				...
				tblViewportClickableRectangles[glbViewportClickableRectCount].b10 = X8(cellPos);
...
void SkWinCore::DRAW_DEFAULT_DOOR_BUTTON(U8 cls1, U8 cls2, U8 cls4, i16 iViewportCell)	// U8 cls1, U8 cls2, U8 cls4, i16 ww
{
	...
	QUERY_TEMP_PICST(0, iStretchScale, iStretchScale, 0, 0, iYDistance, 
		QUERY_GDAT_ENTRY_DATA_INDEX(cls1, cls2, dtWordValue, 8) * 5 + iButtonRectno + 1950,	// SPX: replaced hardcoded class with cls1 to allow more control from caller
		//QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0C_DOOR_BUTTONS, cls2, dtWordValue, 8) * 5 + iButtonRectno +1950,
		-1, glbSceneColorKey, -1, cls1, cls2, cls4
		);
	if (SkCodeParam::bGFXFixModeDM1) {
		glbTempPicture.iXOffset -= 2;
		glbTempPicture.iYOffset -= 1;
	}
	DRAW_TEMP_PICST();
	...
	if (QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0E_DOORS, glbMapDoorType[door->DoorType()], dtWordValue, GDAT_DOOR_NO_FRAMES) == 0) {
		if ((iDoorFrameDisplayFlags & DRAW_DOOR_FLAG_CEILING_SLIT) != 0) { // (0x01) ceiling slit
			...
			if (bp05 != 0xFF) {
				...
				//printf("Slit: Rectno: %d\n", tlbRectnoDoorRoofSlits[RCJ(14,iViewportCell)]);
				DRAW_DUNGEON_GRAPHIC(GDAT_CATEGORY_x08_GRAPHICSSET, gfxset, bp05, tlbRectnoDoorRoofSlits[RCJ(14,iViewportCell)], colorkey, DRAW_FLAG_FLIP_NONE);
			}
		}
		...
		if ((iDoorFrameDisplayFlags & DRAW_DOOR_FLAG_LEFT_FRAME) != 0) {	// (0x02) left door frame
			...
			if (iDoorFrameGfx != 0xFF) {
				QUERY_TEMP_PICST(0, 64, 64, 0, 0, 0, QUERY_CREATURE_BLIT_RECTI(iViewportCell, 10, 0), 4, colorkey, -1, GDAT_CATEGORY_x08_GRAPHICSSET, gfxset, iDoorFrameGfx); // door frame left
				// SPX: DM1 frame adjustment for pixel perfect
				if (SkCodeParam::bGFXFixModeDM1) {
					if (iDoorFrameGfx == GDAT_GFXSET_xD3_DOOR_FRAME_SIDE_D0_L 
						|| iDoorFrameGfx == GDAT_GFXSET_xD4_DOOR_FRAME_SIDE_D0_R) {	// D3 & D4 distance 0
						glbTempPicture.iXOffset -= 2;
						glbTempPicture.iYOffset -= 3;
					}
					else if (iDoorFrameGfx == GDAT_GFXSET_x07_DOOR_FRAME_SIDE_D1_L 
						|| iDoorFrameGfx == GDAT_GFXSET_x08_DOOR_FRAME_SIDE_D1_R)	{ // 07 & 08 Distance 1
						glbTempPicture.iXOffset -= 2;
						glbTempPicture.iYOffset += 2;
					}
					else if (iDoorFrameGfx == GDAT_GFXSET_x09_DOOR_FRAME_SIDE_D2_L 
						|| iDoorFrameGfx == GDAT_GFXSET_x0A_DOOR_FRAME_SIDE_D2_R)	{ // 09 & 0A Distance 2
		...
		if ((iDoorFrameDisplayFlags & DRAW_DOOR_FLAG_RIGHT_FRAME) != 0) {	// (0x04) right door frame (holding button)
			...
			if (iDoorFrameGfx != 0xFF) {
				QUERY_TEMP_PICST(1, 64, 64, 0, 0, 0, QUERY_CREATURE_BLIT_RECTI(iViewportCell, 14, 0), 3, colorkey, -1, GDAT_CATEGORY_x08_GRAPHICSSET, gfxset, iDoorFrameGfx); // door frame right
				// SPX: DM1 frame adjustment for pixel perfect
				if (SkCodeParam::bGFXFixModeDM1) {
					if (iDoorFrameGfx == GDAT_GFXSET_xD3_DOOR_FRAME_SIDE_D0_L 
						|| iDoorFrameGfx == GDAT_GFXSET_xD4_DOOR_FRAME_SIDE_D0_R) {	// D3 & D4 distance 0
				...
				if (door->Button() != 0) {
					...
					if (tblCellTilesRoom[iViewportCell].xsrd.tfoi[2] != 0xFF) {
						DRAW_DEFAULT_DOOR_BUTTON(GDAT_CATEGORY_x09_WALL_GFX, U8(tblCellTilesRoom[iViewportCell].xsrd.tfoi[2]) & 0xFF, U8(tblCellTilesRoom[iViewportCell].xsrd.tfoi[2] >> 8) +1, iViewportCell);
					}
				}
			}
		}
...
//^32CB:48D5
// SPX: _32cb_48d5 renamed _32cb_48d5_STRETCH
X16 SkWinCore::_32cb_48d5_STRETCH(U16 xx, i16 yy) 
{
	ENTER(0);
	i16 si = (((xx << 7) + 64) / yy) >> 1;
...
void SkWinCore::DRAW_DOOR(i16 iCellPos, X16 yy, X16 zz, X32 aa)	// i16 xx, X16 yy, X16 zz, X32 aa
{
		...
		if (iYDist <= 3) {
			...
			if (iInvertedYDist >= 0) {
				...
				if (iDoorPosRectno >= 0) {
					...
					X16 iDoorColorPassThrough = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0E_DOORS, iDoorGDATIndex, dtWordValue, GDAT_IMG_COLORKEY_1);	// X16 bp12
					//if (iDoorColorPassThrough != 0)
					{
						...
						if (iYDist == 0 || QUERY_GDAT_ENTRY_IF_LOADABLE(GDAT_CATEGORY_x0E_DOORS, iDoorGDATIndex, dtImage, iDoorImg) == 0) {	// 0xe
							if (iYDist != 0) {
						...
						if (iOrnateIndex != 0 || iDoorState == _DOOR_STATE_C05_DESTROYED) {	// If there is any ornate or door is destroyed
							...
							// Get door graphics
							//bp16 = QUERY_MULTILAYERS_PIC(&bp015c, GDAT_CATEGORY_x0E_DOORS, bp0e, bp0f, si, si, bp0a, 0, bp12, -1);	// 0xe
							iCacheNo = QUERY_MULTILAYERS_PIC(&xPicture, GDAT_CATEGORY_x0E_DOORS, iDoorGDATIndex, iDoorImg, iStretchDual, iStretchDual, iLightPalette, mirroredDoor, iDoorColorPassThrough, -1);	// 0xe
							DRAW_PICST(&xPicture);
							FREE_PICT_MEMENT(&xPicture);
							...
							if (iOrnateIndex != 0) {
								...
								// SPX: Get ornate
								X16 iColorTransparencyOverlay = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0B_DOOR_GFX, iOrnateGDATIndex, dtWordValue, GDAT_IMG_COLORKEY_1);	// X16 bp14
								...
								if (!SkCodeParam::bUseDM2ExtendedMode)
									QUERY_TEMP_PICST(mirroredDoor, iStretchHorizontal, iStretchVertical, 0, 0, iYDist, 
										(QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0B_DOOR_GFX, iOrnateGDATIndex, dtWordValue, GDAT_DOOR_x08_ORNATE__POSITION) << 2) + iInvertedYDist +0x7d0,
										-1, iColorTransparencyOverlay, -1, GDAT_CATEGORY_x0B_DOOR_GFX, iOrnateGDATIndex, GDAT_DOOR_DECORATION_MASK
										);
								...
								{	// SPX: use the colorkey 10 (by default) or what's in GDAT instead of -1 for activating 'pass through' bitmaps
									...
									iColorPassThrough = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0B_DOOR_GFX, iDoorGDATIndex, dtWordValue, GDAT_IMG_COLORKEY_1);
									if (iColorPassThrough == 0)
										iColorPassThrough = 10;
									QUERY_TEMP_PICST(mirroredDoor, iStretchHorizontal, iStretchVertical, 0, 0, iYDist, 
										(QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0B_DOOR_GFX, iOrnateGDATIndex, dtWordValue, GDAT_DOOR_x08_ORNATE__POSITION) << 2) + iInvertedYDist +0x7d0,
										-1, iColorTransparencyOverlay, iColorPassThrough, GDAT_CATEGORY_x0B_DOOR_GFX, iOrnateGDATIndex, GDAT_DOOR_DECORATION_MASK
										);
								}
							...
							if (iDoorState == _DOOR_STATE_C05_DESTROYED) {	// (iDoorState == 5) If door is destroyed .. then draw destroyed mask over any ornate
								...
								else if (SkCodeParam::bUseFixedMode)
								{
									...
									U16 iDoorDestroyedMask = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0E_DOORS, iDoorGDATIndex, dtImage, GDAT_DOOR_DESTROYED_MASK);
							...
							if (glbGlobalSpellEffects.SeeThruWalls > 0 && iYDist == 1 && yy == 7) {	// If Window spell is active, and display only if distance = 1 and just in front
								...
								
								{
									...
									U16 iDoorSeeThruMask = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0E_DOORS, iDoorGDATIndex, dtImage, GDAT_DOOR_SEE_THRU_MASK);
							...
							xPicture.colorKeyPassThrough = xPicture.iPal256[iDoorColorPassThrough];
							xPicture.iPaletteSize = C000_GDAT_PALETTE_SIZE_ZERO;
							...
							QUERY_TEMP_PICST(mirroredDoor, iStretchDual, iStretchDual, 0, 0, iLightPalette, -1, -1, iDoorColorPassThrough, -1, GDAT_CATEGORY_x0E_DOORS, iDoorGDATIndex, iDoorImg);
						}
	...
	if (aa != 0) {
		_32cb_2d8c_DRAW_TILE(tblCellTilesRoom[iCellPos].xsrd.w4, iCellPos, aa);
	}
	return;
}
...
void SkWinCore::DRAW_STAIRS_SIDE(i16 xx)
{
	...
	if (bp01 != 0xff) {
		DRAW_DUNGEON_GRAPHIC(GDAT_CATEGORY_x08_GRAPHICSSET, gfxset, bp01, _4976_4600[RCJ(9,di)][RCJ(2,si)], bp04, 0);
	}
	return;
}
...
void SkWinCore::DRAW_PIT_TILE(i16 xx)
{
	...
	if (si < 11 || tblCellTilesRoom[si].xsrd.tfoi[0] == 0) {
		...
		DRAW_DUNGEON_GRAPHIC(GDAT_CATEGORY_x08_GRAPHICSSET, bp03
...
X16 SkWinCore::DRAW_EXTERNAL_TILE(i16 xx)
{
	...
	if (bp18 > 3 && GET_TELEPORTER_DETAIL(&bp2a, U8(bp06), U8(bp08)) == 0)
		return 0;
	U16 bp02 = tblCellTilesRoom[si].xsrd.tfoi[2];
	X16 bp16 = bp02 >> 8;
	bp14 += X8(bp16);
	...
	if (glbTabXAxisDistance[RCJ(23,si)] == 0) {
		if (QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0A_FLOOR_GFX, iFloorOrnateID, dtWordValue, GDAT_FLOOR_ORNATE__DO_NOT_FLIP) == 0) {
	...
	X16 bp0e = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0A_FLOOR_GFX, iFloorOrnateID, dtWordValue, GDAT_IMG_COLORKEY_1); // 4
	if (bp0e == 0) {
		bp0e = glbSceneColorKey;
	}
	X16 bp0a = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0A_FLOOR_GFX, iFloorOrnateID, dtWordValue, GDAT_FLOOR_ORNATE__POSITION); // 5
	X16 bp12;
	if (bp0a == 0) {
	...
	X16 bp10 = 0x40; // bp10
	if (QUERY_GDAT_ENTRY_IF_LOADABLE(GDAT_CATEGORY_x0A_FLOOR_GFX, iFloorOrnateID, dtImage, bp14) == 0) {
		...
		{
			if (QUERY_GDAT_ENTRY_IF_LOADABLE(GDAT_CATEGORY_x0A_FLOOR_GFX, iFloorOrnateID, dtImage, bp14 + 0x80) == 1)
			{
				bp14 = bp14 + 0x80;
	...
	if (glbTabXAxisDistance[RCJ(23,si)] == 0 && bp18 != 0) {
		X16 bp1e = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0A_FLOOR_GFX, iFloorOrnateID, dtWordValue, GDAT_FLOOR_ORNATE__X63);
		if ((bp1c = GET_TELEPORTER_DETAIL(&bp2a, U8(bp06), U8(bp08))) != 0 || bp1e != 0) {
			QUERY_TEMP_PICST(bp04, bp10, bp10, 0, 0, bp18, bp0c, bp12, -3, -3, GDAT_CATEGORY_x0A_FLOOR_GFX, iFloorOrnateID, bp14);
			QUERY_TEMP_PICST(bp04, bp10, bp10, glbTempPicture.iXOffset, glbTempPicture.iYOffset, bp18, bp0c, bp12, bp0e, -1, GDAT_CATEGORY_x0A_FLOOR_GFX, iFloorOrnateID, bp14 +0xc8);
			DRAW_TEMP_PICST();
			bp06 = bp2a.b2;
			bp08 = bp2a.b3;
			bp1a = _4976_5aa0;
			...
			bp20 = QUERY_MULTILAYERS_PIC(
				&bp0164, GDAT_CATEGORY_x0A_FLOOR_GFX, iFloorOrnateID, bp14, bp10, bp10, bp18, bp04, bp0e,
				QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0A_FLOOR_GFX, iFloorOrnateID, dtWordValue, GDAT_IMG_FLOOR_COLORKEY_2)	// 0xa
				);
			if (bp1e != 0) {
				DRAW_RAIN_32cb_0c7d(&bp0164, bp20, bp0e);
			}
			else {
				FILL_ENTIRE_PICT(reinterpret_cast<U8 *>(QUERY_MEMENT_BUFF_FROM_CACHE_INDEX(bp20)), bp0164.iPal256[bp0e]);
			}
			bp0164.pb44 = reinterpret_cast<U8 *>(QUERY_MEMENT_BUFF_FROM_CACHE_INDEX(bp20));
			DRAW_PICST(&bp0164);
			FREE_PICT_MEMENT(&bp0164);
			bp0164.colorKeyPassThrough = bp0164.iPal256[bp0e];
			bp0164.pb44 = glbBackBuffViewport;
			bp0164.rectNo = bp0c;
			bp0164.w26 = bp12;
			bp0164.iPaletteSize = C000_GDAT_PALETTE_SIZE_ZERO;
	...
	QUERY_TEMP_PICST(bp04, bp10, bp10, 0, 0, bp18, bp0c, bp12, bp0e, -1, GDAT_CATEGORY_x0A_FLOOR_GFX, iFloorOrnateID, bp14);
	DRAW_TEMP_PICST();
	return 0;
}
//^32CB:2367
...
void SkWinCore::DRAW_PIT_ROOF(i16 xx)
{
	...
	if (SkCodeParam::bBWMode)
		return;	// SPX: Why ? because LOCATE currently moves +2 as player takes stairs, which is wrong for locating just some tile; then coords will be off and tile value will crash
	if (bp02 < 0)
		return;
	...
	DRAW_DUNGEON_GRAPHIC(GDAT_CATEGORY_x08_GRAPHICSSET, gfxset, tlbGraphicsPitRoof[RCJ(9,si)], _4976_425e[RCJ(9,si)], bp0a, di);
	return;
}
...
void SkWinCore::DRAW_FLYING_ITEM(ObjectID rl, i16 cellPos, X16 _5x5)
{
	...
	do {
        ...
        X16 iFlyingImageMethod = GET_MISSILE_FLYING_IMAGE_METHOD(rl);	// bp0c
		if (iFlyingImageMethod == 0xFFFF) {
			ObjectID bp18 = ObjectID(GET_ADDRESS_OF_RECORDE(rl)->GetMissileObject(), rl.Dir());
			DRAW_ITEM(bp18, cellPos, 0, 0xFFA4, -1, NULL, 1, 0, 0);
			continue;
		}
		Missile *pMissile = GET_ADDRESS_OF_RECORDE(rl);	// bp04
		U8 iMissileCls1Category = QUERY_CLS1_FROM_RECORD(rl);	// bp12
		U8 iMissileCls2MainItemId = QUERY_CLS2_FROM_RECORD(rl);	// bp11
		X16 bIsSpellMissile = (iMissileCls1Category == GDAT_CATEGORY_x0D_SPELL_MISSILES) ? 1 : 0;	// bp0e
		X16 bp10;
		i16 bp16;
		if ((bIsSpellMissile == 0 || pMissile->EnergyRemaining() == 0xFF) && bp1a == 0) {
			bp10 = bp16 = 0x40;
		}
		else {
			bp16 = (bp1a << 1) -(di >> 1);
			...
			if (bIsSpellMissile != 0) {
				bp10 = max_value(8, CALC_STRETCHED_SIZE((((pMissile->EnergyRemaining() << 7) / 255) +1) >> 1, bp10));
			}
		}
		si = 0;
		X8 bp13;
		if (iFlyingImageMethod == C03_GDAT_IMG_FLYING_METHOD_SIDE) {
			bp13 = 8;
		}
		else {
			X16 bp0a = glbTimersTable[pMissile->TimerIndex()].Direction();
			if ((bp0a & 1) != (_4976_5aa0 & 1)) {
				bp13 = 0xc;
				if (iFlyingImageMethod == 0) {
					if (di == 0 || di == 3) {
						si |= 1;
					}
					if (((bp1e +bp20) & 1) == 0) {
				...
				if (iFlyingImageMethod == C00_GDAT_IMG_FLYING_METHOD_WHIRLING) {
					if (((bp1e +bp20) & 1) != 0) {
						si |= 2;
						if (di < 2) {
							bp13 = 8;
				...
				else if (iFlyingImageMethod == C02_GDAT_IMG_FLYING_METHOD_ONLY_FRONT || (iFlyingImageMethod == C01_GDAT_IMG_FLYING_METHOD_BACK && bp0a != _4976_5aa0)) {
					bp13 = 8;
				}
				else {
					bp13 = 10;
				...
				if ((di & 1) != 0 && iMissileCls1Category == GDAT_CATEGORY_x0D_SPELL_MISSILES) {
					si |= 2;
				}
			}
		}
		X16 iMissileWord01;	// bp06
		if (iMissileCls1Category == GDAT_CATEGORY_x0D_SPELL_MISSILES) {
			iMissileWord01 = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0D_SPELL_MISSILES, iMissileCls2MainItemId, dtWordValue, 0x01);
		}
		else {
			iMissileWord01 = 3;
		}
...
void SkWinCore::DRAW_STATIC_OBJECT(i16 xx, X32 ss, X16 ww)
{
	...
	if (ww != 0) {
		//^32CB:3DF3
		_32cb_2d8c_DRAW_TILE(bp16, si, ss);	// can go here when there are stuff over door
	}
	//^32CB:3E04
	return;
}
...
void SkWinCore::DRAW_TELEPORTER_TILE(i16 iViewportCell, X16 iGDatCls1Category, X16 iGDatCls2MainItemId)
{
	ENTER(60);
	if (iViewportCell > 15)	// cells above 15 are on D4; teleporters are not displayed at that distance, and NO RECTNO exist for above
		return;
	X8 *bp04 = tblGraphicsTeleporterBytes4[RCJ(23,iViewportCell)];	// bp04		92 bytes (23*4)
	X16 iRectNo = tblGraphicsTeleporterWords[RCJ(16,iViewportCell)];	// bp0e		32 bytes (16*2)
	X16 bp10 = 0;
	X8 bp13 = bp04[1] & 0x7F;
	X16 bp16 = ((bp13 & 0x80) != 0) ? 1 : 0;	// SPX ? mistake ? is this bp04[1] & 0x80 ?
	i16 bp18 = bp04[2];
	i16 bp1a = bp04[3];
	SRECT bp22;
	if (QUERY_BLIT_RECT(NULL, &bp22, iRectNo, &bp18, &bp1a, -1) == NULL)
		return;
	U8 *bp08;	// bp08
	Picture bp3c;	// bp3c
	U8 *bp0c;	// bp0c
	if (bp04[1] == 0xff) {
		...
		bp3c.iGDatCls1Category = U8(iGDatCls1Category);
		bp3c.iGDatCls2MainItemId = U8(iGDatCls2MainItemId);
		bp3c.iGDatCls4EntryId = bp13;
    ...
    bp0c = QUERY_GDAT_IMAGE_ENTRY_BUFF(U8(iGDatCls1Category), U8(iGDatCls2MainItemId), 0x14);	// 0x14 = blue haze small pattern image
	if (bp04[1] != 0xFF) {
		bp08 = QUERY_PICT_BITS(&bp3c);
	}
	_44c8_20a4(
		...
		glbViewportWidth, 
		bp10, 
		_32cb_0649(U8(iGDatCls1Category), U8(iGDatCls2MainItemId), 0x14, bp10)
		);
	if (bp04[1] != 0xFF) {
		FREE_PICT_MEMENT(&bp3c);
	}
...
void SkWinCore::DRAW_WALL(i16 iViewportCell)	// i16 xx
{
	...
	X16 si = 0;
	X16 di = 0;
	U16 bp0a = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x08_GRAPHICSSET, glbMapGraphicsSet, dtWordValue, bp01);

	if (SkCodeParam::bDebugGFXNoWalls)
		return;
	...
	if (iViewportCell >= 0x10) { // wall at D4
		...
		if (QUERY_GDAT_ENTRY_IF_LOADABLE(GDAT_CATEGORY_x08_GRAPHICSSET, iMapGfx, dtImage, bp01) == 0) {
			if (iXDist == 0) {
				bFlip = 1;	// right
			}
...
#if (XDM1_EXTENDED_SEETHRUWALLS == 1)	// Check for possible thru wall gfx
	{
		...
		if (glbGlobalSpellEffects.SeeThruWalls > 0 && iViewportCell == 3)	// front D1 wall
		{
			...
			iPicture = QUERY_MULTILAYERS_PIC(&xExtPicWall, GDAT_CATEGORY_x08_GRAPHICSSET, iMapGfx, bp01, 0x40, 0x40, 0, bFlip, iColorkey1, -1);
			//iPicture = QUERY_MULTILAYERS_PIC(&xExtPicWall, GDAT_CATEGORY_x08_GRAPHICSSET, 0xFE, 0xF2, 0x40, 0x40, 0, bFlip, iColorkey1, -1); // test wall
			...
			if (1)
			{
				...
				U16 iWallSeeThruMask = QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x08_GRAPHICSSET, iMapGfx, dtImage, GDAT_GFXSET_xF1_SEE_THRU);
				...
				QUERY_TEMP_PICST(bFlip, bp20, bp22, 0, 0, 
					...
					GDAT_CATEGORY_x08_GRAPHICSSET, iWallSetIndex, GDAT_GFXSET_xF1_SEE_THRU
					);
			...
			xExtPicWall.colorKeyPassThrough = xExtPicWall.iPal256[iColorkey1];
			xExtPicWall.iPaletteSize = C000_GDAT_PALETTE_SIZE_ZERO;
			QUERY_PICST_IMAGE_FROM_MEMENT_CACHE(iPicture, &xExtPicWall);
			COPY_MEMORY(&xExtPicWall, &glbTempPicture, sizeof(ExtendedPicture));
			glbTempPicture.pb44 = glbBackBuffViewport;
			glbTempPicture.rectNo = 3 + RECTZONE_702_VIEWPORT_BASE;	// SPX ? ..
			DRAW_TEMP_PICST();
			if (iPicture >= 0)
		...
		{
			QUERY_TEMP_PICST(bFlip, 0x40, 0x40, 0, 0, (cd.pi.glbIsPlayerMoving != 0) ? -iYDist : 0, iViewportCell + RECTZONE_702_VIEWPORT_BASE, 0xFFFF, iColorkey1, -1, GDAT_CATEGORY_x08_GRAPHICSSET, iMapGfx, bp01);
...
/*
		SkD((DLV_DBG_CELL, "EXTTILE: @(%02d,%02d) w(%02X, %02X, %02X) tw(%02X, %02X, %02X, %02X) v(%02X)\n",
			tblCellTilesRoom[si].posx, tblCellTilesRoom[si].posy, 
			tblCellTilesRoom[si].xsrd.w0, tblCellTilesRoom[si].xsrd.w2, tblCellTilesRoom[si].xsrd.w4, 
			tblCellTilesRoom[si].xsrd.tfoi[0], tblCellTilesRoom[si].xsrd.tfoi[1], tblCellTilesRoom[si].xsrd.tfoi[2], tblCellTilesRoom[si].xsrd.tfoi[3],
			tblCellTilesRoom[si].xsrd.xvalue));
*/
		SkD((DLV_DBG_CELL, "EXTTILE: @(%02d,%02d) w(%02X, %02X, %02X) itw(%02X, %02X, %02X, %02X) tw(%02X, %02X, %02X, %02X) v(%02X)\n",
			tblCellTilesRoom[si].posx, tblCellTilesRoom[si].posy, 
			tblCellTilesRoom[si].xsrd.w0, tblCellTilesRoom[si].xsrd.w2, tblCellTilesRoom[si].xsrd.w4, 
			tblCellTilesRoom[si].xsrd.interwalls[0], tblCellTilesRoom[si].xsrd.interwalls[1], tblCellTilesRoom[si].xsrd.interwalls[2], tblCellTilesRoom[si].xsrd.interwalls[3],
			tblCellTilesRoom[si].xsrd.tfoi[0], tblCellTilesRoom[si].xsrd.tfoi[1], tblCellTilesRoom[si].xsrd.tfoi[2], tblCellTilesRoom[si].xsrd.tfoi[3],
			tblCellTilesRoom[si].xsrd.xvalue));
...
_51d7:
		...
		switch (bp06) {
			case 0://^520D // wall
				...
				DRAW_EXTENDED_FLOOR_INTERWALLS(si);	// SPX special addition
			...
			case 16://^5243	// seeing door frame slot in front from door tile ==> SPX: it seems to be something else
				if (si == 3) {
					...
					if (QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0E_DOORS, glbMapDoorType[GET_ADDRESS_OF_RECORD0(tblCellTilesRoom[3].xsrd.tfoi[2])->DoorType()], dtWordValue, GDAT_DOOR_NO_FRAMES) == 0) {
						QUERY_TEMP_PICST(
							glbGeneralFlipGraphics, 0x2B, 0x2B, 0, 0, 2, QUERY_CREATURE_BLIT_RECTI(3, 2, 0), 0xFFFF, 
							glbSceneColorKey, -1, GDAT_CATEGORY_x08_GRAPHICSSET, glbMapGraphicsSet, 0x06
							);
						DRAW_TEMP_PICST();
					}
				}
...
_52d1:
				...
				{
					DRAW_TELEPORTER_TILE(si, GDAT_CATEGORY_x18_TELEPORTERS, 0); // DRAW_TELEPORTER_TILE(si, 0x18, 0)
				}
				break;
...
{
	ENTER(2);
	X16 iWidth = READ_UI16(blitSrc,-4);	// si / width
	X16 iHeight = READ_UI16(blitSrc,-2);	// bp02 / height
	X16 bpp = READ_UI16(blitSrc,-6);	// di / bpp
	COPY_SKIMAGE_HEADER(blitSrc, blitDst);
	FIRE_BLIT_PICTURE(blitSrc, blitDst, ALLOC_TEMP_ORIGIN_RECT(iWidth, iHeight), 0, 0, iWidth, iWidth, -1, mirrorFlip, bpp, bpp, NULL);
	return;
}
...
void SkWinCore::DRAW_RAIN()
{
	...
	if (glbRainStrength == 0 || tlbRainScene[RCJ(5,glbRainSceneType)][2] == 0)
		return;
	i16 iViewportWidth = glbViewportWidth;	// bp0a
	i16 iViewportHeight = glbViewportHeight;	// bp0c
	SRECT bp14;
	if (QUERY_BLIT_RECT(NULL, &bp14, RECTZONE_702_VIEWPORT_BASE, &iViewportWidth, &iViewportHeight, -1) == NULL)
		return;
	U8 bp07 = 0;
	U16 bp06 = 0;
	QUERY_RAINFALL_PARAM(&bp07, &bp06);
	U8 *bp04 = QUERY_GDAT_IMAGE_ENTRY_BUFF(GDAT_CATEGORY_x17_ENVIRONMENT, glbMapGraphicsSet, bp07);
	ALLOC_IMAGE_MEMENT(GDAT_CATEGORY_x17_ENVIRONMENT, glbMapGraphicsSet, bp07);
	X16 si = 0;
	if (bp06 == 1) {
		si = ALLOC_TEMP_CACHE_INDEX();
		//ALLOC_NEW_PICT(si, READ_UI16(bp04,-4), READ_UI16(bp04,-2), 4);
		ALLOC_NEW_PICT(si, READ_IMGBUFF_WIDTH(bp04), READ_IMGBUFF_HEIGHT(bp04), IMG_4_BPP);
	...
	//_44c8_20a4(bp04, glbBackBuffViewport, NULL, &bp14, READ_UI16(bp04,-4) * READ_UI16(bp04,-2) -40 -(RAND() & 0x1f), RAND() & 255, glbViewportWidth, 0,
	_44c8_20a4(bp04, glbBackBuffViewport, NULL, &bp14, READ_IMGBUFF_WIDTH(bp04) * READ_IMGBUFF_HEIGHT(bp04) -40 -(RAND() & 0x1F), RAND() & 255, glbViewportWidth, 0,
		_32cb_0649(GDAT_CATEGORY_x17_ENVIRONMENT, glbMapGraphicsSet, bp07, 0)
		);
	if (bp06 == 1) {
		FREE_TEMP_CACHE_INDEX(si);
	}
	FREE_IMAGE_MEMENT(GDAT_CATEGORY_x17_ENVIRONMENT, glbMapGraphicsSet, bp07);
	return;
}
...
void SkWinCore::DRAW_PLAYER_TILE()
{
	...
	switch (bp0e) {
		case 0x10://^536D
			{
				Door *door = GET_ADDRESS_OF_RECORD0(tblCellTilesRoom->xsrd.tfoi[1]);	//*bp04
				if (QUERY_GDAT_ENTRY_DATA_INDEX(GDAT_CATEGORY_x0E_DOORS, glbMapDoorType[door->DoorType()], dtWordValue, GDAT_DOOR_NO_FRAMES) != 0)
					break;
				DRAW_DUNGEON_GRAPHIC(GDAT_CATEGORY_x08_GRAPHICSSET, glbMapGraphicsSet, GDAT_GFXSET_x06_DOOR_FRAME_FRONT_D1, QUERY_CREATURE_BLIT_RECTI(0, 2, 0), glbSceneColorKey, glbGeneralFlipGraphics);
				break;
			}
		case 0x13://^53CE	// on stairs tile (coming from stairs)
			si = tblCellTilesRoom->xsrd.tfoi[0];
			DRAW_DUNGEON_GRAPHIC(GDAT_CATEGORY_x08_GRAPHICSSET, glbMapGraphicsSet
//					, (si != 0) ? 0x39 : 0x4D
				, (si != 0) ? GDAT_GFXSET_x39_STAIRS_RAMP_UP_S0_L : GDAT_GFXSET_x4D_STAIRS_RAMP_DOWN_S0_L
				, (si != 0) ? 0x32B : 0x338, glbSceneColorKey, 0);
			DRAW_DUNGEON_GRAPHIC(GDAT_CATEGORY_x08_GRAPHICSSET, glbMapGraphicsSet
//					, (si != 0) ? 0x3a : 0x4e
				, (si != 0) ? GDAT_GFXSET_x3A_STAIRS_RAMP_UP_S0_R : GDAT_GFXSET_x4E_STAIRS_RAMP_DOWN_S0_R
	...
	if (bp06 == 5)
	{
		DRAW_TELEPORTER_TILE(0, GDAT_CATEGORY_x18_TELEPORTERS, 0);
	}
_5495:
#if defined (XDMX_EXTENDED_FEATURES)
	DRAW_EXTENDED_FLOOR_INTERWALLS(0);	// SPX special addition
#endif // XDMX_EXTENDED_FEATURES
	DRAW_RAIN();
	if (di != 0) {
		DRAW_EXTERNAL_TILE(0);
	}
	_32cb_2d8c_DRAW_TILE(tblCellTilesRoom->xsrd.w4, 0, 0x01ffffff);
	return;
}
...
{
	U16 iMemSizeCellTileDetail = sizeof(CellTileDetail); // 18 bytes SPX: added to control size change of structure (extended mode)
	U16 iMemAllocCellTiles = iMemSizeCellTileDetail * 23;	// originally 414 = 18 * 23
	// 23 = number of displayable cells in the viewport
	ENTER(6);
	IBMIO_USER_INPUT_CHECK();
	...
	_4976_5a9c = xx;
	_4976_5a9e = yy;
	tblCellTilesRoom = reinterpret_cast<CellTileDetail *>(ALLOC_MEMORY_RAM(sizeof(CellTileDetail) * 23, afDefault, 1024));	// std (16+2)*23 = 414
	_4976_5a84 = reinterpret_cast<U8 (*)[17]>(ALLOC_MEMORY_RAM(358, afZeroMem, 1024));		// 21 * 17 + 1 = 358
	_4976_5aa4 = reinterpret_cast<U8 (*)[17]>(ALLOC_MEMORY_RAM(358, afDefault, 1024));		// 21 * 17 + 1 = 358	// is it used only for debug with DLV_DBG_CEL2 ?!
	FILL_STR(*_4976_5aa4, 358, 255, 1);	// init buffer with (0xFF)
	_4976_5a7c = reinterpret_cast<U32 (*)[23]>(ALLOC_MEMORY_RAM(4*23, afZeroMem, 1024));	// 4 * 23 = 92
	_4976_5be2 = reinterpret_cast<U32 (*)[23]>(ALLOC_MEMORY_RAM(4*23, afZeroMem, 1024));	// 4 * 23 = 92
	_4976_4bc8 = 1;
	INIT_BACKBUFF();
	glbViewportClickableRectCount = 9;
	i16 si;
	...
	SkD((DLV_DBG_CELL, "DBG: U r at (%2u,%2u,%2u) %u\n"
		...
		"|       |       |%02X %04X|%02X %04X|%02X %04X|       |       |\n"
		"|-------|-------|-------|-------|-------|-------|-------|\n"
		, (Bitu)cd.pi.glbPlayerMap, (Bitu)cd.pi.glbPlayerPosX, (Bitu)cd.pi.glbPlayerPosY, (Bitu)cd.pi.glbPlayerDir
	...
	GET_SOME_VIEWPORT_INFO_TELEPORTER();
	SET_SOME_CELL_INFO_32cb_5a8f();
	DRAW_DUNGEON_TILES(); // render walls, flying items
	IBMIO_USER_INPUT_CHECK();
	DRAW_PLAYER_TILE(); // render current player tile
	IBMIO_USER_INPUT_CHECK();
//#if UseAltic
	DEALLOC_UPPER_MEMORY(92);	// 92 = 4 * 23 for _4976_5a7c
	DEALLOC_UPPER_MEMORY(92);	// 92 = 4 * 23 for _4976_5be2
	DEALLOC_UPPER_MEMORY(358);	// 358 = 21 * 17 + 1 for _4976_5a84
	DEALLOC_UPPER_MEMORY(358);	// 358 = 21 * 17 + 1 for _4976_5aa4
	DEALLOC_UPPER_MEMORY(iMemAllocCellTiles);	// 414
//#else
//	DEALLOC_UPPER_MEMORY(iMemAllocCellTiles+358+358+92+92);	// 414 + 358 + 358 + 92 + 92
//#endif
	return;
}

//^2481:0002
// _2481_0002 renamed DRAW_TITLE_MENU_SCREEN
void SkWinCore::DRAW_TITLE_MENU_SCREEN()
{
	ENTER(0);
	FIRE_SELECT_PALETTE_SET(0);
	if (_4976_3d2c != 0) {
...
//^0B36:0139
U8 *SkWinCore::QUERY_PICT_BITS(Picture* xPicture)
{
	ENTER(0);
	U16 iPictFlags = xPicture->w4;	// si
	if ((iPictFlags & 0x0004) != 0) {
		return xPicture->pb0 = QUERY_GDAT_IMAGE_ENTRY_BUFF(xPicture->iGDatCls1Category, xPicture->iGDatCls2MainItemId, xPicture->iGDatCls4EntryId);
	}
	else if ((iPictFlags & 0x0008) != 0) {
		return xPicture->pb0 = QUERY_MEMENT_BUFF_FROM_CACHE_INDEX(xPicture->w12);
	}
	else {
		return xPicture->pb0;
	}
...
//^0B36:11C0
// TODO: image related ?
// SPX: _0b36_11c0 renamed DRAW_SOME_PICTURE_0b36_11c0
void SkWinCore::DRAW_SOME_PICTURE_0b36_11c0(ExtendedPicture *xx, sk3f6c *yy, U16 ss, i16 colorkey2)
{
    ENTER(12);
	xx->pb44 = reinterpret_cast<U8 *>(QUERY_MEMENT_BUFF_FROM_CACHE_INDEX(yy->iCacheIndex0));
	SRECT bp08;
	i16 bp0a;
	i16 bp0c;
	if (ss == 0xffff) {
...
//^098D:0C45
// SPX: _098d_0c45 renamed DRAW_RESET_SOME_RECTANGLE
void SkWinCore::DRAW_RESET_SOME_RECTANGLE()
{
	ENTER(0);
	...
	if (cd.pi.glbNextChampionNumber == 0) {
		...
		if (xx != 0) {
			...
			if (glbRightPanelType == RIGHT_PANEL_MAGIC_MAP) {	// 3
				...
				Champion *pChampion = &glbChampionTable[cd.pi.glbChampionIndex];	//*bp04
				if ((glbMagicalMapFlags & 0x0200) != 0) {
					if (--glbMagicMapManaCounter == 0) {
						if (pChampion->manaCurrent != 0) {
							pChampion->manaCurrent--;
							pChampion->heroFlag |= CHAMPION_FLAG_0800;
				...
				else if (pChampion->manaCurrent != 0) {
					glbSomeChampionPanelFlag = 1;
					glbMagicalMapFlags = 0;
				}
			}
		...
		if (cd.pi.glbChampionIndex >= 0) {
			...
			for (iChampionIdx = 0; iChampionIdx < cd.pi.glbChampionsCount; iChampionIdx++) {
				Champion *pChampion = &tblChampionSquad[iChampionIdx];	// bp04
				bp1c[iChampionIdx] = 0;
				U8 bp07 = (pChampion->playerDir() +4 - cd.pi.glbPlayerDir) & 3;
				i8 bp08 = (pChampion->playerPos() +4 - cd.pi.glbPlayerDir) & 3;
				if (pChampion->curHP() != 0 && bp08 +1 == _4976_5dbc) {
					bp08 = -1;
				}
		...
		if (cd.pi.glbChampionIndex == 0) {
			...
			for (U16 iChampionIdx = 0; iChampionIdx < cd.pi.glbChampionsCount; iChampionIdx++) {	// bp06
				Champion *pChampion = &tblChampionSquad[iChampionIdx];
				if (pChampion->curHP() == 0 && GET_PLAYER_AT_POSITION(pChampion->playerPos()) >= 0) {
					continue;
				}
				bp0e = bp1c[iChampionIdx];
				if (glbChampionAlive[iChampionIdx] != ((pChampion->curHP() > 0) ? 1 : 0)) {
					glbChampionAlive[iChampionIdx] = ((pChampion->curHP() > 0) ? 1 : 0);
					bp0e = 1;
					glbPreviousRightPanelType = 0xFFFF;
				}
				...
				for (U16 iHandIdx = 0; iHandIdx < 2; iHandIdx++) {	// si
					...
					U8 bp17 = (pChampion->handCooldown[iHandIdx] != 0) ? 1 : 0;
					if (glbChampionHandCoolingDown[iChampionIdx][iHandIdx] != bp17) {
						glbChampionHandCoolingDown[iChampionIdx][iHandIdx] = bp17;
						bp10 = 1;
					}
					if (_4976_3de6[RCJ(4,iChampionIdx)][RCJ(2,iHandIdx)] != pChampion->Possess(iHandIdx) || bp0a != 0 || bp0e != 0) {
						_4976_3de6[RCJ(4,iChampionIdx)][RCJ(2,iHandIdx)] = pChampion->Possess(iHandIdx);
						bp10 = 1;
					}
				...
				U8 bp17 = (pChampion->herob44 != 0) ? 1 : 0;
