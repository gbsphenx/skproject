...
// SPX notes: 4 * 64 makes 256 MiB. For DOS/DJGPP, that's a bit too much. Then we give only 1 pool for DOS/DJGPP

#ifdef __DJGPP__
#define NUM_OF_FREEPOOLS 1
#else
#define NUM_OF_FREEPOOLS 4
#endif // __DJGPP__

#define SIZEOF_FREEPOOL  0x4000000	// 64 MiB
...
{
SPX_DEBUG_PRINT("MEMALLOC:Before Init\n");
  init(); // new!
SPX_DEBUG_PRINT("MEMALLOC:After Init\n");
  freepoollist_end = NULL; // clean it

  int iamount = SIZEOF_FREEPOOL;

  // get the memory in one piece
SPX_DEBUG_PRINT("RQMEM=%d*%d=%d\n", NUM_OF_FREEPOOLS, iamount, NUM_OF_FREEPOOLS * iamount);
  if (allocated_memory == NULL)
    allocated_memory = new i8[NUM_OF_FREEPOOLS * iamount];
SPX_DEBUG_PRINT("ALLMEM = %08X\n", allocated_memory);
  if (allocated_memory == NULL)
    throw(THROW_DMABORT);
  ...
  {
	  SPX_DEBUG_PRINT("AMTFP %02d/%02d - %08X + %02d*%06d\n", i, NUM_OF_FREEPOOLS, allocated_memory, i, iamount);
