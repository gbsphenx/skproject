#include <skwinapp.h> // link to main application launcher holding params and VRAM
...
{
	SPX_DEBUG_PRINT("GFX_SYSTEM:INIT\n");
...
unsigned char* xGLOBAL_CRAM = NULL;
...
//SPX_DEBUG_PRINT("blit_toscreen\n");
  ...
  blitter.blit(
    ...
    palette);
// SPX
DOS_VIDEO_UPDATE_SCREEN((unsigned char*) gfxsys.dm2screen);
	//update renderer
	{
		int DOS_SCREEN_HEIGHT = 200;
		int DOS_SCREEN_WIDTH = 320;
		unsigned char* pVideoSource = (unsigned char*) gfxsys.dm2screen;
		//unsigned char* pVideoTarget = skWinApplication->xSKVRAM;
		int ix, iy;
		/*
		for (iy = 0; iy < DOS_SCREEN_HEIGHT; ++iy) {
			for (ix = 0; ix < DOS_SCREEN_WIDTH; ++ix) {
				pVideoTarget[iy * DOS_SCREEN_WIDTH + ix] = pVideoSource[iy * DOS_SCREEN_WIDTH + ix];
			}
		}*/
		skWinApplication->renderScreen((X8*)pVideoSource, 0, 0, 320, 200);
		//SPX_DEBUG_PRINT("Screen Rendered gfxsys = %08X %08X\n", gfxsys.dm2screen, palette);
