...
#define OBJECT_BITS		16
#define MASK_ID	0x3FFF
#define SHIFT_DIR	OBJECT_BITS-2	
#define SHIFT_DB	SHIFT_DIR-4
	struct ObjectID {
		// FEDCBA98 76543210
		// -----------------
		// DDTTTTii iiiiiiii
		...
		explicit ObjectID(const ObjectID &src, Bit16u withNewDir) {
			//w = (withNewDir << 14) | (src.w & 0x3fff);
			w = (withNewDir << SHIFT_DIR) | ((t & 15) << SHIFT_DB) | (src.w & MASK_ID);
		}
		ObjectID(U16 recordLink): w(recordLink) { }
		ObjectID(U16 dir, U16 dbType, U16 index) {
			id = index;
			t = dbType;
			d = dir;
			w = (dir << SHIFT_DIR) | ((dbType & 15) << SHIFT_DB) | (index & MASK_ID);
		}
		...
		void ClearDir() {
			d = 0;
			w = (d << SHIFT_DIR) | ((t & 15) << SHIFT_DB) | (id & MASK_ID);
		}

		U8 Dir() const { return (U8)(d); }
		void Dir(U16 val) {
			d = val & 0x03;
			w = (d << SHIFT_DIR) | ((t & 15) << SHIFT_DB) | (id & MASK_ID);
		}

		U16 DBIndex() const { return t; }
		void DBIndex(U16 val) {
			t = val & 0x0F;
			w = (d << SHIFT_DIR) | ((t & 15) << SHIFT_DB) | (id & MASK_ID);
		}
		...
		U8 RealDBType() const { return (U8)(t); }
		void RealDBType(U16 val) {
			val &= 15;
			t = val;
			w = (d << SHIFT_DIR) | ((t & 15) << SHIFT_DB) | (id & MASK_ID);
...
namespace DM2Internal {
	...
	struct TELE_inf { // 5 bytes
		Bit8u b0; // @0 // cross_scene_test;
		Bit8u b1; // @1 // cross_scene_test_of_teleporter_target;
		Bit8u b2; // @2 // target_x_pos;
		Bit8u b3; // @3 // target_y_pos;
		Bit8u b4; // @4 // target_z;     // MAP number
	...
	struct GraphicFileEntry {
		Bit8u cls1, cls2, cls3, cls4, cls5, cls6;
		Bit16u data;
	};
	// 
	...
	struct skload_table_60 { // 56 bytes
		Bit32u dwGameTick;
		Bit32u dwRandomSeed;
		Bit16u wChampionsCount;	// player cnt
		Bit16u wPlayerPosX;	// column
		Bit16u wPlayerPosY;	// row
		Bit16u wPlayerDir;	// dir
		Bit16u wPlayerMap;	// map#
		Bit16u wChampionLeader;
		Bit16u wTimersCount;
		Bit32u dw22;
		Bit32u dw26;
		Bit16u w30;
		...
		Bit8u bRainStrength;
		Bit8u bRainLevelForSky;
		Bit8u bRainLevelForGround;
		Bit8u bRainMultiplicator;
		Bit16u wRainStormController;
		Bit8u bRainRelated3;
		Bit8u bRainRelated2;
		Bit32u dwRainSpecialNextTick;
	};
	// 
	struct shelf_memory {
		U32 val;
	...
	struct Timer {		// 10 bytes
		...
		void SetMap(Bit16u val) {
			val &= 0x00ff;
			dw00 &= 0x00ffffff;
			dw00 |= val << 24;
		}
		Bit32u GetTick() const { return dw00 & 0x00ffffff; }
		void SetTick(Bit32u val) {
			val &= 0x00ffffff;
			dw00 &= 0xff000000;
			dw00 |= val;
		}
		void TimerType(Bit8u val) {
			ttype = val;
		}
		Bit8u TimerType() const {
			return ttype;
		}
		U8 Xcoord() const {
		...
		void Xcoord(Bit16u val) {
			val &= 0x00ff;
			value &= 0xff00;
			value |= val;
		}
		void Ycoord(Bit8u val) {
			val &= 0x00ff;
			value &= 0x00ff;
			value |= val << 8;
		}

		void XcoordB(Bit8u val) { value &= 0xff00; value |= val; }	// (b6_0_f)
		U8 XcoordB() const { return U8(value); }	// (b6_0_f) SPX what difference with Xcoord ?
		void b6_0_4(Bit16u val) {
			val &= 0x001f;
			value &= 0xffe0;
			value |= val;
		}
		U8 b6_0_4() const { return (value&0x1f); }
		void w6_5_9(Bit16u val) {
			val &= 0x001f;
			value &= 0xfc1f;
			value |= (val << 5);
		}
		...
		void w0_0_f(Bit16u val) { dw00 &= 0xffff0000; dw00 |= val; }
		Bit16u w0_0_f() const { return (Bit16u)(dw00 & 0x0000ffff); }
		Bit8u ActionType() const { return (Bit8u)(w8 >> 8); }	// b9_0_f()
		void ActionType(Bit8u val) { // b9_0_f(Bit8u val) for tty04: actionType
			w8 &= 0x00ff;
			w8 |= (val << 8);
		}
	...
	struct Champion {		// 261 bytes
		Bit8u firstName[8];		// @0 // b0[8] first name
		Bit8u lastName[16];		// @8 // b8[16] family name
		...
		Bit8u absoluteDirection;			// @28 // b28_ absolute player dir (ignore squad dir). 0=North, 1=E, 2=S, 3=E
		Bit8u squadPosition;			// @29 // b29_ player position. 0=TL, 1=TR, 2=BR, 3=BL

		Bit8u playerDir() const { return absoluteDirection; }
		void playerDir(Bit8u val) {
			val &= 3;
			absoluteDirection = val;
		}

		Bit8u playerPos() const { return squadPosition; }
		void playerPos(Bit8u val) {
			val &= 3;
			squadPosition = val;
		}
		...
		Bit8u runesCount;			// @30 // b30

		U8 RuneCnt() const { return runesCount; }
		void RuneCnt(U8 val) { runesCount = val; }

		Bit8u PoisonValue; // b31;			// @31 // not 0 if poisoned
		Bit8u handCommand[2];		// @32 // b32[2] holds cm command

		Bit8u spelledRunes[4];		// @34	// b34_
		...
		U8 direction;				// @40	// b40
		U8 herob41;				// @41	// b41
		Bit8u handCooldown[2];		// @42	// b42[2]
		Bit8u herob44;			// @44	// b44

		X8 b45_;			// @45

		Bit16u timerIndex;			// @46 // w46 timer index?
        X16 damageSuffered;			// @48 // w48 current damage?
		Bit16u heroFlag;			// @50 // w50 update mask: 0x8000=all?
		...
		__int16 HPCurrent;		// @54 // w54_ cur hp
		Bit16u HPMax;			// @56 // w56 max hp
		Bit16u staminaCurrent;			// @58 // w58 cur stamina x10
		Bit16u staminaMax;			// @60 // w60 max stamina x10
		Bit16u manaCurrent;			// @62 // w62 cur mana
		Bit16u manaMax;			// @64 // w64 max mana
		i8 handDefenseClass[2];			// @66	// b66[2] SPX: has something to do with armor change?
		Bit16u food;			// @68 // w68 food
		Bit16u water;			// @70 // w70 water
		...
		Bit8u attributes[7][2];	//b72[7][2];	// @72

		//Bit8u w72;			// @72 // cur luck
		//Bit8u w73;			// @73 // max luck
		//Bit8u w74;			// @74 // cur str
		//Bit8u w75;			// @75 // max str
		//Bit8u w76;			// @76 // cur dex
		//Bit8u w77;			// @77 // max dex
		//Bit8u w78;			// @78 // cur wiz
		//Bit8u w79;			// @79 // max wiz
		//Bit8u w80;			// @80 // cur vit
		//Bit8u w81;			// @81 // max vit
		//Bit8u w82;			// @82 // cur anti-magic
		//Bit8u w83;			// @83 // max anti-magic
		//Bit8u w84;			// @84 // cur anti-fire
		//Bit8u w85;			// @85 // max anti-fire
		...
		DMEncyclopaedia::ObjectID Possess(Bit16u no) const { return DMEncyclopaedia::ObjectID(inventory[no]); }
		void Possess(Bit16u no, DMEncyclopaedia::ObjectID recordLink) { inventory[no] = recordLink; }
		...
		Bit16u weight;		// @253 // w253 weight x10
		Bit8u heroType;			// @255 // b255 hero no (in GDAT)
		Bit8u enchantmentAura;			// @256	// b256 protection enchantment number
...
//	};
	...
	struct SkImage {	// size=14?
		...
		__int16 Xoffset() const { return w0_ >> 10; }
		__int16 Yoffset() const { return w2_ >> 10; }
		Bit16u BitsPixel() const { return w4_; }
		Bit16u AllocLower() const { return w6_; }
	};
	// 
	struct PreEntry {
		__int32 dw0;
	...
	*/
		...
		U16 w20;		// @20
		Bit16u w22;		// @22
		Bit16u w24;		// @24		// Resistance & Fire resistance & Poison resistance & ??
		X16 w26;		// @26		
...
//		U16 w0_1_1() const { return (w0AIFlags >> 1)&1; } // 1 = reflector?
		...
		Bit16u GetPoisonResistance() const { return (w24 >> 8)&15; } // w24_8_b
...
//	};
	...
	struct skxxx3 { // 10 bytes
		Bit32u bp0a;	// @0 // tick for message invokation
		Bit8u bp06;		// @4 // ?
		Bit8u bp05;		// @5 // message type: 1=SET, 2=TOGGLE, 3=CLEAR
		Bit8u bp04;		// @6 // xpos
		Bit8u bp03;		// @7 // ypos
		Bit8u bp02;		// @8 // dir
		Bit8u bp01;		// @9 // actionType
	};
	// 
	struct sk5f06 { // 7 bytes
		Bit16u w0;		// @0
		Bit8u category;		// @2 // b2 cls1
		Bit8u index;		// @3 // b3 cls2
		Bit8u entry;		// @4 // b4 cls4
		Bit16u w5;		// @5
	};
	// SPX: sk5f0e
	...
	struct SoundStructure { // 12 bytes
		...
		__int8 b7;		// @7 // dist y
		Bit8u b8;		// @8 // volume
		Bit8u b9;		// @9 // SPX: left volume ?
		Bit8u b10;		// @10 // SPX: right volume ?
		Bit8u b11;		// @11
	};
	// 
	...
	struct sk5d00 { // ? bytes
		...
		Bit16u w16;		// @16	// poolflag: 0x800=Is4EMS
		Bit16u w18;		// @18	// allocflag?: 1=if used
		U32ptr dw20;	// @20
		X32 dw24;		// @24
	...
	// 
	struct sk3f6c { // ? bytes
		Bit16u w0;	// index. use with _4976_5d08[w0]
		SRECT rc2;
		//  w2 = w2.x
		//  w4 = w2.y
		//  w6 = w2.cx
	...
	struct Picture { // 24 bytes
		Bit8u *pb0;		// @0 // pic bits
		Bit16u w4;		// @4 // 4=Use (b8,b9,b11), 8=Use w12 for cacheIndex
		Bit16u w6;		// @6 // raw data index
		Bit8u b8;		// @8 // cls1
		Bit8u b9;		// @9 // cls2
		Bit8u b10;		// @10 // cls3
		Bit8u b11;		// @11 // cls4
		...
		Bit16u w14;		// @14
		Bit16u w16;		// @16
		__int16 width;	// @18 // (__int16 w18) width
		__int16 height;	// @20 // (__int16 w20) height
		__int16 w22;	// @20 // bpp
	};
	// 
	...
	//	__int16 w18;	// @18
	//	__int16 w20;	// @20
		Bit16u rectNo;		// @24 // (Bit16u w24) rectno
		Bit16u w26;		// @26
		__int16 w28;	// @28 // x-offset
		__int16 w30;	// @30 // y-offset
		__int16 w32;	// @32 // x?
		__int16 w34;	// @34 // y?
		SRECT rc36;		// @36
		Bit8u *pb44;	// @44 // picture buffer
		i16 colorKeyPassThrough;		// @48 // (i16 w48) color key
		Bit16u mirrorFlip;		// @50 // (w50) mirror flip
		Bit16u w52;		// @52 // horz stretch factor? (unstretch is 64)
		Bit16u w54;		// @54 // vert stretch factor? (unstretch is 64)
		Bit16u w56;		// @56 // palentcnt
		Bit8u b58[256];	// @58 // local palette 16 colors
	};
	...
	struct sk1891 { // 4 bytes
		Bit8u b0;		// @0
		Bit8u b1;		// @1	// game ended ? or used to compare with right panel
		Bit16u w2;		// @2
	...
	struct sk5358 { // 4 bytes
		Bit8u category;		// @0 // b0 cls1
		Bit8u index;		// @1 // b1 cls2
...
//	};
	...
	struct MousePosition { // 14 bytes
		Bit16u XPos;		// @0 // w0 mouse x-pos
		Bit16u YPos;		// @2 // w2 mouse y-pos
		Bit16u event;		// @4 // rectno?
		SRECT rc6;		// @6
		// rc6.x // @6
		// rc6.y // @8
		// rc6.cx // @10
