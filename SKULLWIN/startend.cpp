...
static void DM2_2481_0002(void)
{
...
static void DM2_SHOW_MENU_SCREEN(void)
{
  ...
  {
    //m_28802:
    ddat.v1e0954 = DM2_QUERY_GDAT_IMAGE_ENTRY_BUFF(5, 0, 4);
    t_resolution res = getbmpheader(BMPCAST(ddat.v1e0954))->res;
    if (res != BPP_8)
    {
      if (res != BPP_4)
        //m_28837:
        RG1W = getbmpheader(BMPCAST(ddat.v1e0954))->width;
      else
        RG1W = MK_EVEN(getbmpheader(BMPCAST(ddat.v1e0954))->width) / 2;
      //m_28840:
      paldat.glbl_pal1 = CHGCAST(t_palette, ddat.v1e0954 + RG1W * getbmpheader(BMPCAST(ddat.v1e0954))->height);
    }
  }
  else
  {
  ...
  if (res != BPP_8)
  {
    ...
    if (res != BPP_4)
      //m_28890:
      RG4W = getbmpheader(BMPCAST(ddat.v1e0950))->width;
    else
      RG4W = MK_EVEN(getbmpheader(BMPCAST(ddat.v1e0950))->width) / 2;
    //m_28896:
    paldat.glbl_pal2 = CHGCAST(t_palette, ddat.v1e0950 + RG4W * getbmpheader(BMPCAST(ddat.v1e0950))->height);
  }
  else
      ...
      if (ddat.savegamew8 != lcon(0x63))
      {
        if (ddat.savegamew8 != lcon(0xda))
        {
          ...
          if (ddat.v1d66f8 == 0)
            ...
            dm2_dballochandler.DM2_DEALLOC_BIGPOOL_STRUCT_BEFORE(CHGCAST(s_malloctail, ddat.v1e0954));
          //m_2891A:
          DM2_FREE_PICT_ENTRY(ddat.v1e0950);
          return;
        }
        break;
      }
...
DEBUG_DUMP_ULP();
  ...
  ddat.v1d70cb = 1;
  DM2_1031_07d6();
  RG1Blo = dm2_dballochandler.DM2_dballoc_3e74_2439(15, 11);
  ddat.v1e0414[17] = RG1Blo;
...
void DM2_SHOW_CREDITS(void)
{
  DM2_SELECT_PALETTE_SET(0);
  DM2_HIDE_MOUSE();
  R_C470(ddat.v1e0950, 2, -1, paldat.glbl_pal2);
  DM2_SELECT_PALETTE_SET(1);
  DM2_SHOW_MOUSE();
  DM2_1031_0541(1);
  //m_28970:
